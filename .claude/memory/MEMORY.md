...
- [列表残影：新旧文字完全重合](scroll-content-ghosting-transparent-backdrop.md) — 无 opaque 底衬的滚动内容 + `transform` 定位 + `scrollIntoView` 滚祖先；底衬要打在滚动内容(spacer)里
...
- [CLAUDE.md 15KB 硬预算守护](claude-md-size-guard.md) — `pnpm claude-md:check` 已接入 check；瘦身手法=拆子文档/删代码自证/去重；perforce 豁免待移除；**各文件常年贴顶（父文件 14992/15000），加一行前先想好从哪腾**
