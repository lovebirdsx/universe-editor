...
- [大文件三连修:定位/卡顿/tsserver 重启](largefile-reveal-dirtydiff-vscode-parity.md) — 34万行文件:①符号跳转打开不定位=rAF+50ms 定时轮询等挂载必超窗,waitForFileEditor 改事件驱动(registry onDidChange+onWillDispose+30s 兜底)并收敛 3 处私有轮询,命令层对 reveal 保持 fire-and-forget;②dirty-diff 每键主线程全文 diff 无豁免→对标 quickDiffModel:ThrottledDelayer(200,新加 platform)+50MB 双侧豁免+行级 diff(HEAD 预 split 缓存+getLinesContent);blame 加 500ms delayer;③文档同步全文→增量(Monaco deltas 降序转 LSP contentChanges+host 镜像 vscode-languageserver-textdocument 原地更新+sentVersion 防 replay 双 apply)+maxTsServerMemory 3072+semanticTokens 100K 门;④didOpen 曾 3×全文发送(冷启动 replay 竞态+重复 open 事件+prewarm pin 撞车)→lspClient 连接代际(sentGeneration)去重,靠临时 e2e 日志计数断言才现形;⑤真机15MB回灌:崩溃真因=语义project加载 OOM abort 134(3072不够该工程,8192可过)→maxTsServerMemory 配置化(每次restart重读)+OOM签名检测弹通知;复现=最小LSP stdio脚本,坑=cli.mjs按path.sep切tsserver.path,正斜杠Windows判invalid静默回退工作区旧TS;⑥didOpen IPC 4.4s=StdioFramingProtocol._ingest `+=`+全量indexOf O(n²)→分片累积器(4422→718ms);⑦waitForFileEditor 30s兜底挂e2e泄漏门禁→加宿主store参数+哨兵settle;坑=editor devDeps 漏 extension-host 边致 e2e 跑 stale bundle;⑧⑨tab 切回卡>1s(CDP CPU profile 定位)=activeEditor DTO 带 15MB text 过 RPC+mergeConflict 每切重扫全文→DTO 去 text 走 ExtHostDocuments 镜像(get/whenOpen+generation,$getActiveTextEditor 先 PendingDocumentSync.flush 防 200ms 旧文本致 markdown 命令错编辑)+findNextMatch 预筛+WeakMap 版本缓存(723→93ms,longtask 清零);⑩看门狗 TabSwitchPerfContribution=切tab超200ms写warn到 tabSwitchPerf.log(双rAF首帧+longtask 汇总+recordTabSwitchPhase 命名相位归因,连切截断收尾不丢报告)
...
- [computeLineDiff 须保持 Myers O(ND)](linediff-myers-perf.md) — dirty-diff 复用它对大文件切换做全文 diff，勿退回 O(m·n)；二修=V 数组曾按 n+m 而非 maxD 分配致 trace 每轮拷 5.4MB(D 大时 4s 纯 memcpy+GC)→按 2*maxD+1 收窄+100ms 墙钟预算超时粗粒度回退(4073→172ms)
