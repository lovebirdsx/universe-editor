...
**判例**：`agentContextActions.ts` 的 `SendCommitToAgentChatAction` / `AddSelectionToExistingAgentChatAction` / `AddSelectionToNewAgentChatAction` 用 `captureRevealServices(accessor)`（外加 `IQuickInputService`）先把 service 同步快照成 `RevealServices`，再传给 `resolveExistingChatTarget` / `createChatTarget` / `resolveActiveOrNewSession` / `revealChat`。
