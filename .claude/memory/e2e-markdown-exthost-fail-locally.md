...
**⚠️ 修正（2026-06-29）**：本条不可一概套用。后查明 `smoke.markdownLsp` / `smoke.peekNavigation` 的稳定失败是**真回归**（URI 跨 IPC 没 revive，详见 [[realpath-uri-ipc-revive]]）。关键鉴别：**先 `pnpm build` 重建 `out/` 产物再下结论**——本条原结论很可能是基于陈旧产物得出的。`received 恒 []` + 重建后稳定复现 = 真回归，不是 exthost 环境噪音。
