...
**2026-08-09 e2e 串行趟清空**：曾因此根因把 20 个「打开 workspace」用例钉在 @serial 串行趟（`--workers=1`）。根因修复后标签已全部摘除并回 8-worker 并行主趟（实验验证并行全绿），@serial 现仅剩 OS 剪贴板两用例。见 apps/editor/e2e/CLAUDE.md「何时打 @serial」。
