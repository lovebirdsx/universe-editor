...
- **冷挂载 drain 竞态（inbox 消费了但 setText 静默丢弃）**：Monaco 首载是异步的（`MonacoLoader.peek()` 未命中走 `ensureInitialized().then(mount)`），而 PromptInput 的三个 inbox drain effect 在 React commit 后同步执行——此时 `setText`/`insertRef` 内 `modelRef.current === null` 静默 return，但 `drain()` 已把 inbox 清掉 → deposit **永久丢失**。deep link 落输入框（首个"session 未 mount 即 deposit"场景）稳定复现；同病的还有 git graph "Send to Agent Chat"、"Add Selection to Chat"、Rewind replace inbox 发到未打开 session 的场景。修法：PromptInput 加 `editorReady` state（`onEditorReady` 时翻转），三个 drain effect `if (!editorReady) return` 提前退出（留队列不消费）+ 依赖加 editorReady，ready 后统一 drain；先 pull 后订阅无缝隙。回归锁：`smoke.deepLinkAgent.spec.ts`（inbox 链路首个 e2e 覆盖）。
