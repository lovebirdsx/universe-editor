...
**教训**:allotment 的 onChange/onDragEnd 回调内读 props 一律走 ref,勿信闭包;守护测试须模拟真实时间线(`isMaximized:true` 重启)而非稳定后 live maximize —— 三条时间线(live maximize / 最大化重启 / maximize→unmaximize)都在 `smoke.maximizedSecondarySidebarRestore.spec.ts`。本机 markdown 预览 cursor 对齐 e2e 失败是既有环境 flake,与此无关。
