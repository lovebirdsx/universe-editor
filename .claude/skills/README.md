...
- 默认加 `disable-model-invocation: true`（手动 `/xxx` 调用）；高频流程类可去掉以开放模型自动调用。codex 侧策略独立维护在该 skill 的 `agents/openai.yaml`（codex 原生读取，默认全手动 `false`；新增 skill 跑 `pnpm skills:policy` 补齐，想开放单个 skill 的 codex 自动调用就手改为 `true`）。
