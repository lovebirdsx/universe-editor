...
2. **改了扩展代码，别裸 `pnpm --filter <ext> e2e`**：子包 `e2e` script 只 `playwright test`，**不 build**——扩展 `dist/` 和 editor `out/` 都可能过期，跑旧产物（假绿/假红）。走 `pnpm e2e:ext <包>`（turbo 自动 build 宿主+扩展）。
...
5. **子包级 playwright 不 rebuild**：诊断单 spec 前先 `pnpm build`（或走 turbo）。根 `pnpm e2e` 走 turbo 会自动 build 依赖链。
