...
- `Execution context was destroyed ... navigation`（fire 后页面 reload/导航再 evaluate）→ 案例 6；冷启动首个裸 evaluate、无任何导航/崩溃（utility world 销毁，挂 CDP 后不复现）→ 案例 58
...
- 大 fixture 多次 print 往返+裸 toBeVisible 等满不出现、initial+retry 同形态=「纯慢 vs 静默 '' 误路由」双假设同修（失败结构化传播+断言先 poll 任一终态）；artifact `if-no-files-found: ignore` 把上传路径错位静默吞掉致无现场 → 案例 68
