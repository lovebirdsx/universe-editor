...
全局 keybinding handler 命中 CommitInlineCompletionAction
  （when: inlineSuggestionVisible && editorTextFocus && !suggestWidgetVisible，权重 200 抢占）
  ▼
...
- **ghost text 出来了但 Tab 不接受**：集成层三件套——确认 `bridgeInlineSuggestionVisible` 有把全局 `inlineSuggestionVisible` 置 true、`CommitInlineCompletionAction` 权重 > 50、when 子句成立（`!suggestWidgetVisible` 等）。逐步诊断走 [fix-keybinding-not-firing]。
...
3. **commit 命令权重必须 > MonacoDefault(50)**：否则全局 handler defer 给 Monaco 自己 dispatch，又回到不可靠路径。WorkbenchContrib 200 是对的。
