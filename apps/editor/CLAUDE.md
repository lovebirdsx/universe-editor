...
1. 在 `main/services/ai/providers/` 写 `XxxProvider implements IAiModelProvider`（`provideModels` / `sendRequest` / `provideTokenCount`）。构造注入 `AiProviderContext`：经 `context.secrets`（`ISecretStorageService`）读密钥、经 `context.getVendorConfig(vendor)` 读 baseUrl/默认模型。`sendRequest` 用 `AsyncIterableSource` + `DeferredPromise` 产流，监听 `token.onCancellationRequested` 中止 fetch，HTTP 状态映射到 `AiErrorCode`。参考 `ollamaProvider.ts`。
