...
test.describe('@p1 deep link — agent workspace routing', () => {
  test('cwd with no matching window opens a new workspace window and creates the session there', async ({
    electronApp,
    workbench,
    page,
  }) => {
    ...
    // Window B reads the echo-agent config from settings.json AT STARTUP; the
    // update above resolves before the write necessarily hits disk. Poll the
    // file itself — sending the link earlier races window B onto the real
    // default agent (observed: a genuine Claude reply instead of the echo).
      ...
      .poll(
        () => {
          try {
            ...
            const agents = parsed['acp.agents'] as Array<{ id?: string }> | undefined
            return (
              parsed['acp.defaultAgentId'] === 'echo' &&
              Boolean(agents?.some((a) => a.id === 'echo'))
            )
          } catch {
            return false
          }
        },
        { timeout: 10_000, message: 'echo agent config should be flushed to settings.json' },
      )
      .toBe(true)
