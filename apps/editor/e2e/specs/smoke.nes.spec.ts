...
test.describe('@p1 next edit suggestion', () => {
  ...
  test('renders an inline edit and Tab jumps then accepts', async ({ page, workbench }) => {
    await workbench.waitForRestored()
    ...
    await expect
      ...
      .toBe(true)
    expect(await page.evaluate(() => window.__E2E__!.getActiveInlineEditText())).toBe('LINE THREE')
    ...
    await page.evaluate(() => void window.__E2E__!.runCommand('editor.action.inlineSuggest.commit'))
