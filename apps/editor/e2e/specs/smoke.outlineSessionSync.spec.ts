...
test.describe('@p1 outline ↔ agent session selection sync', () => {
  test('moving the session selection moves the outline highlight @regression', async ({
    ...
    //
    // Note the navigable rows are the FULL timeline — the first user message
    // (`alpha`) renders in the sticky bar above the scroll container, but it
    // stays keyboard-reachable, so navigation runs over
    ...
    // Alt+Home → first navigable row (the sticky-bar user message); the outline
