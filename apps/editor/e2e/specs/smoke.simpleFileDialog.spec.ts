...
test.describe('@p1 simple file dialog', () => {
  ...
  test('openFile navigates into a folder and selecting a file opens it', async ({
    ...
    // Open File is single-select: clicking a file row opens it immediately.
    await note.click()
    await workbench.quickInput.waitForHidden()
  ...
  test('dragging a selection out of the input past the backdrop keeps the dialog open', async ({
    page,
    workbench,
  }) => {
    const tmpDir = await makeTree()
