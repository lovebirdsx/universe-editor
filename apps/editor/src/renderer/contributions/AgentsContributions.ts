        ...
        properties: {
          ...
          'acp.mcpServers': {
            ...
            description: localize(
              ...
              'MCP servers forwarded to the agent on session/new, keyed by server name. stdio: `{ "fs": { "command": "npx", "args": ["-y", "@modelcontextprotocol/server-filesystem", "."], "env": {} } }`. http/sse: `{ "docs": { "type": "http", "url": "https://…", "headers": {} } }`. A `.mcp.json` in the workspace root overrides same-named entries for that project. The default on/off switch is not part of these entries — it is kept per user/workspace in the MCP settings panel or the session picker. Transports the agent does not support (http/sse) are skipped with a warning; env/header values are stored in plain text — keep secrets in real environment variables.',
            ),
          },
