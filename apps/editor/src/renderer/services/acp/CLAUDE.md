...
- **新会话的默认启用集 = 池中全部非 `disabled` 条目**（`resolveMcpServerSelection(pool, null)`）。`disabled` 标志是**唯一**的默认开关，两个入口同源：AI 设置"MCP 服务器"面板的 checkbox + 会话 picker 弹窗行内的「默认」开关（`setMcpServerDefaultEnabled` 按 winning 层写回 settings；VSCode 兼容层只读则写对应可写高层覆盖；`.mcp.json` 与 `extension` 来源返回 `false`，UI 禁用控件——扩展条目要关请禁用扩展或用其 `whenConfiguration` 设置项）。
