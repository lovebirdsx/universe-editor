...
| `acpSessionFilterService.ts` / `acpSessionStatus.ts` / `acpAuthError.ts` | 列表过滤/状态枚举/auth 错误判定；`acpSessionStatus.ts` 另含共享判定 **`isResidentLive(session, reader?)`**（`status!=='closed' \|\| isDormant`——「这个实例还该被当成活会话用吗」的唯一真相，facade 与 UI 共用） | — |
...
| `acpSessionProviderContext.ts` | 会话的**费率归属上下文**：按 `agentId + authority` 复合键缓存「这个会话在那台主机上实际用哪个 provider 条目」。解析走 `claudeConfig/codexConfig.resolveActiveAuth(authority)` **反查 agent 自己的配置文件**（不读任何编辑器声明值），只有 `kind==='provider' && providerId` 才产出 context——外部/手写凭据刻意不归属（开销显示「—」）。懒解析（`_known` 冷键触发，`refresh()` 重算已知键）+ 订阅两个 agent 的 `onDidChangeConfig`/`onDidChangeAuth` 实时失效 | 会话开销归属、本地/远端计费分区 |
...
- **加/改内置 agent skill（随编辑器分发、`/` 弹层可见的用户 skill）**：skill 本体放 `apps/editor/resources/agent-skills/.claude/skills/<name>/SKILL.md`，**加文件即生效，代码零改动**。内置 skill 默认应声明 `disable-model-invocation: true`（仅 `/` 手动调用，不进模型自动隐式调用）；codex 端由 codex-acp fork 桥接该 frontmatter 生成 `agents/openai.yaml`，无需手工维护。机制：main 的 `environmentSnapshotMainService` 暴露 `builtinAgentSkillsRoot`（packaged=`<resources>/agent-skills`，dev=`resolveFromRepo('apps/editor/resources/agent-skills')`，目录不存在则 undefined）→ `acpSessionService._builtinAgentDirs()`（memoized）在 new/load/resume/fork 四条 wire 路径 spread 官方 `additionalDirectories` 字段 → 两个 fork 都从该 root 的 `.claude/skills` 发现 skill（codex `refreshSkills` 扫 roots；claude SDK 把 additionalDirectories 当 working-dir root）。**remote authority 会话不注入**（本地路径对远端 agent 无意义，`_builtinAgentDirs(authority)` 内 guard）。打包：`runtime-resources.mjs` stage 整个 `agent-skills/` 树 + 每个 skill 一条 `REQUIRED_SOURCE_FILES` sentinel（新增 skill 记得补 sentinel）。测试：`AcpSessionService.test.ts` 的 'builtin agent skills injection' describe。命名别撞本仓库开发者 skill（`.claude/skills/`——开发者在编辑器里打开本仓库时两边同时可见）。
...
- **空闲进程回收**（`acp.idleProcessTimeoutMs`，默认 5min，<=0 禁用）：与 stall watchdog 同 tick 的 `_reclaimIdleConnections` 按 (agentId,cwd) 分组，**组内全部** session 满足 status∈{idle,errored,closed} + 无 recovery/pendingElicitation/pendingPermission/compaction/background（`backgroundTaskCount>0`，它独立于 status）+ 静音超时才 `killConnectionFor` 停进程释放内存；但**全 closed 的组跳过**（无活 lease——用户关的交给池 30s 宽限驱逐，已 seal 的池条目已 evict，跳过同时防止静默 seal 组每 tick 重复打点）；可复活约束：非 readOnly 且非 closed 的 session 须 `hasMessages !== false`（否则 `session/resume` 没有可恢复的 transcript）。杀后不碰任何 session 状态——走坑 #11 的静默 seal + 统一唤醒抽象复活，集成测试 `AcpSession.recovery.integration.test.ts` 已覆盖该路径。拿不到 cwd 的 session（未 attach/历史行无 cwd）让**整个 agentId** 本轮跳过——池 key 不能猜。
- **休眠会话的唤醒策略分两档**（明确区分「必须唤醒」与「不该唤醒」——为改个标题拉起一个进程会抵消回收本身的意义）：
...
- **`lastActivityAt` 防抖动**：`_handleConnectionLost` 内 bump `_lastActivityAt`——唤醒是用户活动，成功唤醒的 session 因此拿到完整 idleMs 宽限。若不 bump：唤醒耗时十几秒且期间无 wire 流量 → `lastActivityAt` 陈旧 → 下一 tick（≤60s）就被再杀，用户观感是「点开又睡回去」。不会无限续命（唤醒后无交互，idleMs 后照常回收）；唤醒全程 `recovery.state !== undefined` 已被 `_isIdleReclaimable` 排除，reaper 本身无需改。
...
5. **FakeSession stub 漏新接口成员**：`IAcpSession` 加方法（如 `whenConnected`）后，`ConfigOptionsBar.test.tsx` / `PromptInput.test.tsx` / `SessionChangesView.test.tsx` 等的本地 stub 要同步补，否则 typecheck 红。**加数据字段（如 `authority`）时分两类**：走 `as unknown as IAcpSession` 双重转型的 stub 不会报错、但会静默读到 `undefined`（多数 stub 属此类）；显式 `FakeSession implements/extends IAcpSession` 的（ConfigBarOverflowMenu / ConfigOptionsBar / McpServerPicker / PromptInput 四处）会 typecheck 红。**真正断言分区行为的测试（`AccountUsageService` / `SubscriptionUsageService` / `UsageIndicator`）必须显式给 authority**，否则静默 undefined 会让用例假过。
...
11. **`status:'closed'` 有两义，判别式已收口成三个符号，别再自己拼**：agent 进程在闲置期退出时，onClose 的 idle 分支只做**静默 seal**（`status='closed'`、无 `[error]`、无 recovery state），phase 仍是 `'connected'`、死 lease 仍绑在 `_conn` 上、session 未 dispose、history 行保留；用户主动 `close()` 则 phase 转 `'closed'` 且 session dispose，绝不该被复活。三个符号**一套判定不留两份**：
