...
> 本文从 `services/acp/session/CLAUDE.md` 拆出，范围是：ACP agent 会话的「回退（rewind，截断对话+可选回滚文件+回填输入框做 edit-and-retry）」与「分叉（fork，以某消息之前的历史新建独立会话）」纵切案例——SDK 能力边界、消息锚点机制、claude/codex 双实现、已修 bug 根因与测试套路。
...
>
> 协议层全景（双 id、applyUpdate 状态机、连接池、恢复）见 [CLAUDE.md](CLAUDE.md) 会话子系统部分，本节只讲 rewind/fork 纵切，不重复。
...
- **相关**：[CLAUDE.md](CLAUDE.md) 会话子系统（本节是其纵切）；skill `update-claude-agent-acp`（vendor fork 构建/升级流程）、`docs/user/CLAUDE.md`（用户文档）

### codex 版（与 claude 架构根本不同，已实现）
