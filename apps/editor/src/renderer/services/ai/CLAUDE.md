...
- 加新 AI provider（让模型列表多出可选项）属于 AI 模型层，见 apps/editor/CLAUDE.md **套路 I（加一个 AI provider（协议））**——密钥明文存在 `aiSettings.json` 的 provider 条目 `apiKey` 字段（renderer 经门面读同一份配置）；仍有效的红线是密钥绝不进日志 / AI Debug、UI 掩码显示。
