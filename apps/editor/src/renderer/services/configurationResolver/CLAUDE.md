...
本目录是配置变量替换 renderer 数据源门面的家：本仓库对齐 VSCode 的 IConfigurationResolverService，把 `${workspaceFolder}`/`${env:NAME}`/`${userHome}`/`${file...}`/`${config:...}`/`${pathSeparator}` 等变量在真正使用时展开成当前上下文的实际值，并承担终端 cwd 的「相对拼根 + 无效目录安全回退」。本文是配置变量替换子系统的上下文地图（处理相关任务前通读）。
...
- 相关：另一个用 workspace/editor 数据源的子系统见 `apps/editor/src/renderer/services/explorer/CLAUDE.md`；跨进程服务套路见 `apps/editor/CLAUDE.md` 套路 C
