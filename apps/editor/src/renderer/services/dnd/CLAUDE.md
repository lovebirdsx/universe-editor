# apps/editor/src/renderer/services/dnd/CLAUDE.md

资源拖放（DnD）应用侧的读取/落点逻辑在本目录（`resourceDropTransfer` / `openDroppedResource` / `importDroppedFiles`），核心抽象（uri-list 线格式、`resourceDragProps` / `useDragHandle` / `useDropTarget`）在 `packages/workbench-ui/src/dnd/`（纯 React，无 Electron）。本文是拖放的上下文地图（处理相关任务前通读）。
