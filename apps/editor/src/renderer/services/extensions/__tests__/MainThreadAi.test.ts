...
const fakeAi: IAiModelService = {
  ...
  updateProviders: (_providers: readonly AiProviderInstance[]) => Promise.resolve(),
  getProviderTypes: () => Promise.resolve({}),
  updateProviderTypes: (_types: Readonly<Record<string, AiProviderType>>) => Promise.resolve(),
  getProviderTypeDescriptors: () => Promise.resolve([]),
  verifyProvider: (_input: AiProviderVerifyInput) => Promise.resolve({ ok: true, modelCount: 0 }),
  setApiKey: (_typeId: string, _instanceName: string, _key: string) => Promise.resolve(),
  deleteApiKey: (_typeId: string, _instanceName: string) => Promise.resolve(),
  hasApiKey: (_typeId: string, _instanceName: string) => Promise.resolve(false),
  getRateTables: () => Promise.resolve([] as AiRateTableSnapshot[]),
  getAccountUsage: (_providerKey: string) =>
    Promise.resolve(undefined as AiAccountUsage | undefined),
  refreshRemote: (_providerKey?: string) => Promise.resolve(),
}
