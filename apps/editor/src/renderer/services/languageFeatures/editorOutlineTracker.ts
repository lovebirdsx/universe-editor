 ...
 *  EditorOutlineTracker — derives ONE editor's document-symbol tree (and the
 *  symbol under its cursor) from the language features facade, exposing them as
 *  observables. It re-pulls symbols on content change (debounced) and on
 *  provider registration, and recomputes only the active symbol on cursor
 *  movement.
 *
 *  There is one tracker per editor group (owned by OutlineService), so a split
 *  view's breadcrumbs each show their own group's navigation path instead of the
 *  focused editor's.
 *--------------------------------------------------------------------------------------------*/
...
/**
 * The outline of one editor, as consumed by the Outline view, the breadcrumbs
 * and the symbol quick access. Implemented by an editor group's
 * `EditorOutlineTracker` (and, for the active group, by `OutlineService`).
 */
export interface IOutlineScope {
...
/** Cached symbol trees kept across tab switches (see `OutlineSymbolCache`). Small
 *  on purpose: a huge file's tree is itself megabytes of retained objects, and tab
...
/** A settled symbol pull: what came back, and how long the wire took. */
interface SymbolPull {
  readonly roots: monaco.languages.DocumentSymbol[] | null | undefined
  readonly pullMs: number
}

/**
 * The symbol trees already pulled, plus the pulls currently on the wire. Shared
 * by every group's tracker: split views showing the same file must not each
 * cross the wire for it — a large file's symbol tree is a multi-MB RPC payload.
 */
export class OutlineSymbolCache {
  private readonly _trees = new Map<string, CachedSymbols>()
  private readonly _pulls = new Map<string, Promise<SymbolPull>>()
  /** Bumped by `clear()`: a pull started before a provider change must not be joined. */
  private _epoch = 0

  /** The tree for an unchanged model, or undefined when it must be (re)pulled. */
  get(uri: string, modelVersionId: number, languageId: string): CachedSymbols | undefined {
    const cached = this._trees.get(uri)
    if (!cached || cached.modelVersionId !== modelVersionId || cached.languageId !== languageId) {
      return undefined
    }
    this._trees.delete(uri)
    this._trees.set(uri, cached) // refresh LRU order
    return cached
  }

  put(uri: string, entry: CachedSymbols): void {
    this._trees.delete(uri)
    this._trees.set(uri, entry)
    if (this._trees.size > SYMBOL_CACHE_MAX) {
      const oldest = this._trees.keys().next().value
      if (oldest !== undefined) this._trees.delete(oldest)
    }
  }
  ...
  /**
   * Run `start` for `key`, or join the pull already running for it. Two groups
   * displaying the same file (and its shared model) therefore pull once per model
   * state — version AND language — not once per group. `start` must always
   * settle — `pullWithTimeout` guarantees that — so a stuck pull can't wedge
   * every tracker waiting on it.
   *
   * `key` must carry every dimension the cached tree is validated against
   * (see `get`): a language switch keeps the model version, so a key of
   * `uri@version` alone would hand a joiner the previous language's tree.
   *
   * The bookkeeping is a second handler on the SAME promise rather than
   * `.finally()`: `finally` resolves one microtask later, which would push a
   * caller's publish past the single `await flush()` the tests rely on (and with
   * it, past the cache lookups of a tab switch happening in the same turn).
   */
  pullOnce(key: string, start: () => Promise<SymbolPull>): Promise<SymbolPull> {
    const poolKey = `${this._epoch}:${key}`
    const running = this._pulls.get(poolKey)
    if (running) return running
    const pull = start()
    const done = (): void => {
      if (this._pulls.get(poolKey) === pull) this._pulls.delete(poolKey)
    }
    void pull.then(done, done)
    this._pulls.set(poolKey, pull)
    return pull
  }
}

export interface IOutlineTrackerOptions {
  /** The editor group this tracker belongs to; disambiguates split mounts of the
   *  same input in `FileEditorRegistry`. */
  readonly groupId: number
  /** The editor this tracker follows — its group's current active editor. */
  readonly activeEditor: IObservable<EditorInput | undefined>
  readonly languageFeatures: ILanguageFeaturesService
  readonly logger: ILogger
  readonly cache: OutlineSymbolCache
}

export class EditorOutlineTracker extends Disposable implements IOutlineScope {
  // Debug names carry the group id, so they can only be built once the
  // constructor has read its options.
  private readonly _outline: ISettableObservable<OutlineModel | undefined>
  private readonly _activeSymbol: ISettableObservable<monaco.languages.DocumentSymbol | undefined>
  private readonly _sourceKind: ISettableObservable<OutlineSourceKind | undefined>
  readonly outline: IObservable<OutlineModel | undefined>
  readonly activeSymbol: IObservable<monaco.languages.DocumentSymbol | undefined>
  readonly sourceKind: IObservable<OutlineSourceKind | undefined>
  ...
  /** `uri@version@language` of the pull currently on the wire; duplicate triggers
   *  for the same state (re-attach, marker burst) coalesce instead of stacking pulls. */
  private _pullInFlightKey: string | undefined
  /** Set by `dispose()`. A disposed tracker must not restart the retry chain: the
   *  in-flight pull still lands, and its handler would otherwise re-arm a timer
   *  that keeps pulling the removed group's file for the whole retry budget. */
  ...
  private readonly _cache: OutlineSymbolCache

  constructor(options: IOutlineTrackerOptions) {
    super()

    this._groupId = options.groupId
    this._activeEditor = options.activeEditor
    this._languageFeatures = options.languageFeatures
    this._logger = options.logger
    this._cache = options.cache

    this._outline = observableValue(
      `EditorOutlineTracker.outline.${this._groupId}`,
      undefined as OutlineModel | undefined,
    )
    this._activeSymbol = observableValue(
      `EditorOutlineTracker.activeSymbol.${this._groupId}`,
      undefined as monaco.languages.DocumentSymbol | undefined,
    )
    this._sourceKind = observableValue(
      `EditorOutlineTracker.sourceKind.${this._groupId}`,
      undefined as OutlineSourceKind | undefined,
    )
    this.outline = this._outline
    this.activeSymbol = this._activeSymbol
    this.sourceKind = this._sourceKind

    // Re-attach whenever this group's active editor changes.
    this._register(
      autorun((r) => {
        this._activeEditor.read(r)
  ...
  private _attachActiveEditor(): void {
    const input = this._activeEditor.get()
    ...
    const editor = FileEditorRegistry.get(fileInput, this._groupId)
    ...
    // This group's editor may mount AFTER its symbols arrived (a background group
    // renders one editor per group, not only the focused one): retrack the cursor
    // symbol so its breadcrumbs fill in without waiting for a cursor move. Skip it
    // while the published tree still belongs to the previous file.
    if (editor && this._outline.get()?.uri === model.uri.toString()) {
  ...
  private _recomputeSymbols(retry?: { generation: number; delay: number; elapsed: number }): void {
    ...
    if (this._disposed || !model || model.isDisposed()) return
    ...
    const cached = this._cache.get(uri, versionId, languageId)
    if (cached) {
      const current = this._outline.get()
    ...
    // The language is part of the key because a language switch keeps the version.
    const pullKey = `${uri}@${versionId}@${languageId}`
    if (this._pullInFlightKey === pullKey) return
    this._pullInFlightKey = pullKey

    void this._cache
      .pullOnce(pullKey, () => this._pullWithTimeout(provider, model))
      .then((pull) => {
        ...
        const roots = pull.roots ?? []
        if (pull.pullMs > 500) {
          this._logger.info(
            `document-symbol pull ${uri} took ${pull.pullMs.toFixed(0)}ms roots=${
              roots.length
        ...
        if (roots.length > 0 && model.getVersionId() === versionId) {
          this._cache.put(uri, { modelVersionId: versionId, languageId, roots })
        }
   ...
   * underlying worker RPC never resolves. Timing it here (rather than in a
   * wrapper `.then`) keeps the caller's publish exactly one microtask behind the
   * provider's answer.
   */
  private _pullWithTimeout(
    ...
    return new Promise((resolve, reject) => {
      let settled = false
      const finish = (result: monaco.languages.DocumentSymbol[] | null | undefined): void => {
        settled = true
        resolve({ roots: result, pullMs: performance.now() - started })
      }
  ...
  private _recomputeActiveSymbol(): void {
    ...
    if (readerUri !== undefined) {
      ...
      const editor = this._currentInput
        ? FileEditorRegistry.get(this._currentInput, this._groupId)
        : undefined
  ...
  revealSymbol(symbol: monaco.languages.DocumentSymbol): void {
    ...
    if (!input) return
    const editor = FileEditorRegistry.get(input, this._groupId)
    if (!editor) return
