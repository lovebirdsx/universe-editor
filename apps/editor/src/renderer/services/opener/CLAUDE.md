...
本目录是 opener 子系统的家：对等 VSCode 的 IOpenerService 机制——发一个地址（文件路径、URL、command: URI、或 OS 级 universe-editor:// 深链）就打开对应资源并定位行列。renderer 三档 opener 实现在本目录（`OpenerService.ts`），契约在 platform，OS 级深链在 shared/main。本文收录案例「链接打开机制 IOpenerService + 深链接」（处理相关任务前通读）。

## 案例：链接打开机制 IOpenerService + 深链接

对等 VSCode 的"发一个地址就打开对应文件并定位/执行配置命令"。核心思路：**统一入口 `IOpenerService.open(target, options)`，行列编码进 URI fragment（`#L{行},{列}`，1-based），三档 opener first-wins 分发**（External → Command → File）。区别于 markdown 预览/成链（链接点击分发在 MarkdownView/SafeLink，见 `apps/editor/src/renderer/workbench/markdown/CLAUDE.md`）：本案例是"地址→打开+定位"的统一机制本身。
...
>
> ⚠️ 第二原则：**检测端 ≠ 打开端**。"把文本里的裸路径识别成可点链接"是 `filePathLink.ts`（另有 markdown 的 SafeLink、终端 linkProvider），那是**检测**；本案例讲的是识别出地址**之后怎么打开并定位**。两端别混。
...
- **markdown 里点链接的分发逻辑**（相对路径解析、目录/图片/预览分流、Ctrl+点击到侧边）：**不在本案例**，在 `useMarkdownFileLink.ts` + `MarkdownView.tsx` 的 `SafeLink`（见 `apps/editor/src/renderer/workbench/markdown/CLAUDE.md` 线③相邻）。本案例只负责它们最终调到的"打开+定位"。
...
- 检测端（非本案例，但常一起出现）：`apps/editor/src/renderer/services/acp/filePathLink.ts`（裸路径识别单一真相源，`splitFilePathLocation` 被 opener 复用）、`workbench/markdown/useMarkdownFileLink.ts` + `MarkdownView.tsx` SafeLink（markdown 链接分发）
...
- 相关 skill：[fix-disposable-leak]（坑1）、[register-monaco-command]（命令注册对照）；markdown 链接检测/分发/成链见 `apps/editor/src/renderer/workbench/markdown/CLAUDE.md`
