...
export function TerminalOutput({ text, contentKey }: { text: string; contentKey?: string }) {
  ...
  // size-change correction loop returns. See contentOverflow.ts for why.
  const [overflows, setOverflows] = useState(() =>
    initialOverflow(contentKey, () => estimateTerminalOverflow(text)),
  )
  ...
  // changes to avoid a redundant re-render; the measured truth is remembered per
  // contentKey so the next remount seeds from it directly.
  useLayoutEffect(() => {
    const el = innerRef.current
    if (!el) return
    const measure = () => {
      const next = el.scrollHeight > TERMINAL_COLLAPSED_MAX_PX + 1
