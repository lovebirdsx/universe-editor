  ...
  return useCallback(
    (e: KeyboardEvent<HTMLDivElement>) => {
      ...
      switch (e.key) {
        ...
        case 'Enter': {
          if (e.altKey || currentKey === undefined) return
          e.preventDefault()
          if (e.ctrlKey || e.metaKey) openMenu(currentKey)
          else openCommitChanges()
          return
        }
        default:
          return
    ...
    [rows, selectionRef, select, openMenu, openCommitChanges, scrollRef, rowAttribute, rowHeight],
  )
}
