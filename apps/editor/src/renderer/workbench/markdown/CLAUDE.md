...
markdown 已是**内置插件** `extensions/markdown`（按 `extend-language-plugin` 的「迁新语言」套路从 TS 样板迁成）。它和 TS 共用同一套**句柄路由 + 四条数据流 + renderer 句柄壳/转换层**——这些**别在这里找差异**，去 [extend-language-plugin]。本文只讲 **markdown 独有的形态**：它和 TS 的不同点、两条线的地图、为什么这么设计。
...
- `extensions/markdown/e2e/specs/markdownLsp.spec.ts` —— 语言特性 + 粘贴成链冒烟
- 相关 skill：[extend-language-plugin]（通用语言插件套路）；TS 子系统对照见 `extensions/typescript/CLAUDE.md`
