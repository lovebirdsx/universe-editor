...
outline（大纲）子系统的视图层在本目录（OutlineView/OutlineViewToolbar/outlineViewState），服务主干 OutlineService 在 `services/languageFeatures/`（因其语言无关、与 symbolTree 等纯函数同域）。本文是 outline 子系统的上下文地图（处理相关任务前通读）。
...
> 预览这条线的形态属于 markdown 子系统，详见 `apps/editor/src/renderer/workbench/markdown/CLAUDE.md` 线②。大纲只通过 `IMarkdownPreviewController` 与它解耦协作——**给预览加新交互能力时，优先扩 `IMarkdownPreviewController` 接口**（如当初加 `focus()`），三处同步：接口（MarkdownPreviewRegistry.ts）、实现（MarkdownPreviewEditor.tsx 的 controller 对象）、调用（OutlineService.revealSymbol / _recomputeActiveSymbol）。
...
- **改大纲项点击/回车的跳转行为**：`OutlineService.revealSymbol`——**三个分支都要改**（file / preview / session），保持对称（尤其 focus）。
...
- 相关 skill：[extend-language-plugin]（符号来源/语言插件套路）；预览线②形态见 `apps/editor/src/renderer/workbench/markdown/CLAUDE.md`
