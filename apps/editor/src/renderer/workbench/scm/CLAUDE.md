...
- **会话更改的噪声过滤是「委托 ∪ 内建」两条腿**（`services/scm/P4IgnoreService.ts` + `services/scm/p4Ignore.ts`）：`SessionWatchedChangesContribution._ignoredPaths` 把 `<providerId>.checkIgnore` 与内建 `.p4ignore` 解析**并行取并集**。必须是并集而非「无归属才兜底」——委托给不出答案有三态且都长得像「没有忽略」：`sourceControls` 全空、扩展在线但**离线**（`checkIgnore` 在连接守卫后返回 `[]`，`_goOffline` 不移除 source control）、命令尚未注册（`undefined`，宿主还把整批缓存成「未忽略」）。并集也不得串行：`checkIgnore` 超时是 20s。`resolveScmProviderId` 保持单一仲裁不改（改成复数会每批对多个 provider 发命令，且与「选中 repo 决定判定规则」这条已文档化行为冲突）。内建层的 armed 分档（有 p4 但路径不在任何 client root → 不套规则；完全没有 p4 → 套）与向上遍历的 ceiling 见该文件头注释。
