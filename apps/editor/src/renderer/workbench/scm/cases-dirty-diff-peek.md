# cases-dirty-diff-peek.md

本文从 `workbench/scm/CLAUDE.md` 拆出，范围是 dirty-diff 内联 peek 的完整实现复盘（骨架/布局公式/交互接线/E2E 套路）。CLAUDE.md 里只留入口与结论，做 dirty-diff peek 相关改动时通读本文。
