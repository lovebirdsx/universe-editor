...
webview 的 iframe 宿主（`WebviewElement.tsx`）在本目录。本文是 webview / 自定义编辑器预览**基建**的上下文地图（五层架构 + 全部已知坑）；**写一个新预览扩展**（最常见的任务，照抄 pdf 范例）见 `extensions-external/pdf/CLAUDE.md`。
...
- `extensions-external/pdf/`（**范例扩展，照抄它**，authoring 套路见其 CLAUDE.md）：`src/extension.ts`（provider + asWebviewUri + dirUri 修复）/ `esbuild.config.mjs`（out-of-workspace 构建 + text loader）/ `scripts/pack.mjs`（.vsix 打包）/ `README.md`（构建流程）
