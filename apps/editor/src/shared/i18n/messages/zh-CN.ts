...
export const ZH_CN_MESSAGES: MessageMap = {
  ...
  'aiSettings.help.mcpServers': [
    '## MCP 服务器',
    '',
    'MCP 服务器为 Agent 提供额外工具。配置存放在 `acp.mcpServers`，这里按作用域分组展示：',
    '',
    '- **用户（全局）**：在所有工作区可用。',
    '- **工作区**：存放在 `.universe-editor/settings.json`，仅对当前文件夹生效。',
    '- **.mcp.json**：工作区根目录的 Claude Code 兼容文件（此处只读，请直接编辑文件）。',
    '',
