import type { MessageMap } from '@universe-editor/platform'

export const ZH_CN_MESSAGES: MessageMap = {
  // --- App ---
  'about.detail':
    '版本: {version}\nExtension API: {extensionApi}\nElectron: {electron}\nChromium: {chromium}\nNode: {node}\nV8: {v8}',
  'app.description': '一个遵循 VSCode 范式的游戏内容编辑器。',
  'app.name': 'Universe Editor',

  // --- Agent settings ---
  'agentSettings.showFable': '在模型选择器中显示 Fable',
  'agentSettings.showFable.desc':
    '将 Claude Fable 5 追加为可选模型，不改变默认模型（env.ANTHROPIC_CUSTOM_MODEL_OPTION）。能否使用仍取决于你的账号 / 套餐。',

  // --- Common ---
  'common.cancel': '取消',
  'common.close': '关闭',
  'common.copy': '复制',
  'common.delete': '删除',
  'common.deleteWithEllipsis': '删除…',
  'common.details': '详情',
  'common.dismiss': '忽略',
  'common.edit': '编辑',
  'common.loading': '正在加载…',
  'common.name': '名称',
  'common.ok': '确定',
  'common.open': '打开',
  'common.paste': '粘贴',
  'common.refresh': '刷新',
  'common.replace': '替换',
  'common.reset': '重置',
  'common.restart': '重启',
  'common.save': '保存',

  // --- Command categories ---
  'command.category.agents': 'Agents',
  'command.category.ai': 'AI',
  'command.category.developer': '开发人员',
  'command.category.diffEditor': '差异编辑器',
  'command.category.editor': '编辑器',
  'command.category.extensions': '扩展',
  'command.category.file': '文件',
  'command.category.gitGraph': 'Git Graph',
  'command.category.go': '转到',
  'command.category.help': '帮助',
  'command.category.html': 'HTML',
  'command.category.json': 'JSON',
  'command.category.markdown': 'Markdown',
  'command.category.merge': '合并编辑器',
  'command.category.mergeConflict': '合并冲突',
  'command.category.perforceGraph': 'Perforce Graph',
  'command.category.preferences': '首选项',
  'command.category.remoteSsh': 'Remote-SSH',
  'command.category.search': '搜索',
  'command.category.scm': '源代码管理',
  'command.category.swarm': 'Swarm',
  'command.category.terminal': '终端',
  'command.category.view': '视图',
  'command.category.workspaceTrust': '工作区信任',
  'command.category.wsl': 'WSL',

  // --- Extensions ---
  'action.extensions.checkForUpdates': '检查扩展更新',
  'action.extensions.checkForUpdates.done': '已更新 {count} 个扩展。',
  'action.extensions.checkForUpdates.none': '所有扩展均为最新。',
  'action.extensions.disable.none': '没有已启用的扩展。',
  'action.extensions.disable.placeholder': '选择要禁用的扩展',
  'action.extensions.disableForWorkspace': '禁用扩展（工作区）',
  'action.extensions.disableGlobally': '禁用扩展',
  'action.extensions.disableWorkspace.placeholder': '选择要在本工作区禁用的扩展',
  'action.extensions.enable.none': '没有被禁用的扩展。',
  'action.extensions.enable.placeholder': '选择要启用的扩展',
  'action.extensions.enableForWorkspace': '启用扩展（工作区）',
  'action.extensions.enableGlobally': '启用扩展',
  'action.extensions.enableWorkspace.placeholder': '选择要在本工作区启用的扩展',
  'action.extensions.installFromVSIX': '从 VSIX 安装…',
  'action.extensions.installFromVSIX.done': '已安装扩展"{name}"（{version}）。',
  'action.extensions.installFromVSIX.failed': '安装扩展失败：{error}',
  'action.extensions.installFromVSIX.notVsix': '所选文件不是 .vsix 包。',
  'action.extensions.installFromVSIX.open': '安装',
  'action.extensions.installFromVSIX.title': '从 VSIX 安装',
  'action.extensions.restartHost': '重启扩展宿主',
  'action.extensions.show': '显示已安装的扩展',
  'action.extensions.uninstall': '卸载扩展…',
  'action.extensions.uninstall.confirm': '卸载"{name}"？',
  'action.extensions.uninstall.confirm.yes': '卸载',
  'action.extensions.uninstall.done': '已卸载"{name}"。',
  'action.extensions.uninstall.failed': '卸载扩展失败：{error}',
  'action.extensions.uninstall.none': '没有已安装的用户扩展。',
  'action.extensions.uninstall.placeholder': '选择要卸载的扩展',
  'action.extensions.update.failed': '更新 {name} 失败：{error}',
  'action.extensions.workspace.none': '请先打开文件夹以更改工作区启用状态。',
  'extensions.development': '开发中',
  'extDev.statusBar': '扩展开发宿主 ({count})',
  'extDev.statusBar.tooltip': '扩展开发宿主：从源码目录加载了 {count} 个开发中的扩展\n{paths}',
  'extDev.autoReload.restarting': '正在重启扩展宿主…',
  'extDev.autoReload.restarting.tooltip': '开发中扩展的产物发生变更，正在重启扩展宿主。',
  'extDev.autoReload.debuggerDetached':
    '扩展宿主已因开发中扩展的产物变更而自动重启。已附加的调试器会断开——继续调试请重新 attach。将 "extensions.autoRestartOnChange" 设为 false 可关闭自动重启。',
  'settings.extensions': '扩展',
  'settings.extensions.autoRestartOnChange':
    '开发中扩展的产物变更时自动重启扩展宿主。仅在扩展开发宿主窗口中生效。',
  'windowTitle.extDevHost': '[扩展开发宿主]',
  'extensions.activation.failed': '扩展"{name}"激活失败：{error}',
  'extHost.crash.restarting': '扩展宿主已崩溃（代码 {code}），正在重启…',
  'extHost.crash.noMoreRestarts': '扩展宿主持续崩溃（代码 {code}），将不再自动重启。',
  'extManagement.error.noMarketplace': '扩展市场不可用。',
  'extManagement.error.packageMismatch':
    '下载的扩展包（{downloadedId}@{downloadedVersion}）与市场条目（{expectedId}@{expectedVersion}）不一致。',
  'extManagement.error.unsigned': '市场条目 {id}@{version} 未签名——拒绝安装。',
  'extManagement.error.engineMismatch':
    '该扩展需要 universe {required}，而当前宿主 API 为 {actual}。',
  'extManagement.error.remoteUnavailable': '{authority} 的远程扩展管理不可用。',
  'extManagement.error.remoteOperation': '在 {authority} 上的扩展操作失败：{message}',
  'extGallery.error.downloadFailed': '下载 {id} 失败：HTTP {status}',
  'extensions.installInRemote': '安装到远程',
  'extensions.installInRemote.label': '安装到 {label}',
  'extensions.installInRemote.hint':
    '此扩展已安装在本机，但需安装到 {label} 才能在此工作区中运行。',
  'extensions.installInRemote.failed': '在 {label} 上安装 {name} 失败：{error}',
  'extensions.installInRemote.unavailable': '该扩展在市场中不可用，因此无法安装到远程工作区。',
  'extensions.installInRemote.unavailable.badge': '远程不可用',
  'extensions.group.local': '本机',
  'extensions.noneInstalled.local': '没有已安装的本机扩展',
  'extensions.activationFailed': '激活失败',
  'extensions.builtin': '内置',
  'extensions.detail.capabilityWarning':
    '该扩展对你的文件和网络拥有近乎原生的访问权限。请只安装来自你信任的发布者的扩展。',
  'extensions.detail.commands': '命令',
  'extensions.detail.contributions': '贡献点',
  'extensions.detail.contributionsAfterInstall': '安装该扩展后可查看它提供的贡献点',
  'extensions.detail.installs': '{count} 次安装',
  'extensions.detail.keybindings': '快捷键',
  'extensions.detail.missing': '扩展 {id} 不可用',
  'extensions.detail.noReadme': '暂无详细信息',
  'extensions.detail.readme': '详情',
  'extensions.detail.settings': '设置',
  'extensions.disable': '禁用',
  'extensions.disableWorkspace': '禁用（工作区）',
  'extensions.disabled': '已禁用',
  'extensions.disabledWorkspace': '已禁用（工作区）',
  'extensions.drop.notVsix': '只能通过拖放到此处安装 .vsix 包。',
  'extensions.enable': '启用',
  'extensions.enableWorkspace': '启用（工作区）',
  'extensions.group.builtin': '内置',
  'extensions.group.installed': '已安装',
  'extensions.group.marketplace': '市场扩展',
  'extensions.install': '安装',
  'extensions.install.failed': '安装 {name} 失败：{error}',
  'extensions.uninstall.failed': '卸载 {name} 失败：{error}',
  'extensions.installCompatibleVersion': '将安装版本 {version}',
  'extensions.installIncompatible': '与当前版本不兼容',
  'extensions.installVsix.done': '已安装"{name}"（{version}）。',
  'extensions.installVsix.failed': '安装扩展失败：{error}',
  'extensions.list': '扩展',
  'extensions.manage': '管理',
  'extensions.noResults': '未找到扩展',
  'extensions.noneInstalled': '没有已安装的扩展',
  'extensions.quarantined': '已禁用 {count} 个被标记为恶意的扩展：{ids}',
  'extensions.search.label': '搜索扩展',
  'extensions.search.placeholder': '搜索扩展（@builtin 用于内置扩展）',
  'extensions.trust.confirm': '信任发布者并安装',
  'extensions.trust.detail':
    '该扩展对你的文件和网络拥有近乎原生的访问权限。请只安装来自你信任的发布者的扩展。',
  'extensions.trust.message': '安装来自 {publisher} 的"{name}"？',
  'extensions.uninstall': '卸载',
  'extensions.versionIncompatible': '已禁用（需要 universe {reason}）',
  'extensions.versionIncompatible.disabled': '已禁用 {count} 个与此版本不兼容的扩展：{ids}',
  'extensions.viewDetails': '查看详情',

  // --- Menus ---
  'menu.activityBar': '活动栏',
  'menu.edit': '编辑',
  'menu.file': '文件',
  'menu.help': '帮助',
  'menu.view': '视图',
  'menuBar.ariaLabel': '菜单栏',

  // --- Workbench structure ---
  'panel.maximize': '最大化底栏大小',
  'panel.output': '输出',
  'panel.restore': '还原底栏大小',
  'sidebar.empty': '没有注册任何视图。',
  'titleBar.empty': '（空）',
  'view.aiDebug': 'AI 调试',
  'view.commitChanges': '提交变更',
  'view.files': '文件',
  'view.outline': '大纲',
  'view.output': '输出',
  'view.scm': '源代码管理',
  'view.search': '搜索',
  'view.sessionChanges': '会话更改',
  'view.sessions.main': 'Sessions',
  'view.sessions.mcp': 'MCP 服务器',
  'view.extensions': '扩展',
  'view.swarm.changes': 'Swarm 更改',
  'view.swarm.reviews': '审核',
  'view.terminal': '终端',
  'view.timeline': '时间线',
  'viewContainer.aiDebug': 'AI 调试',
  'viewContainer.close': '关闭',
  'viewContainer.explorer': '资源管理器',
  'viewContainer.extensions': '扩展',
  'viewContainer.generated': '自定义视图',
  'viewContainer.outline': '大纲',
  'viewContainer.output': '输出',
  'viewContainer.remote': '远程资源管理器',
  'viewContainer.scm': 'SCM',
  'viewContainer.search': '搜索',
  'viewContainer.sessionChanges': '会话更改',
  'viewContainer.sessions': 'Sessions',
  'viewContainer.swarm': 'Swarm 审核',
  'viewContainer.terminal': '终端',

  // --- Editor tabs ---
  'editorTab.close': '关闭 {name}',
  'editorTab.tooltip.pinned': '已固定',
  'editorTab.tooltip.preview': '预览',
  'editorTab.tooltip.readonly': '只读',
  'editorTab.tooltip.unsavedChanges': '未保存的更改',
  'editorTab.tooltip.untitled': '未保存文件',
  'editorTab.unpin': '取消固定 {name}',
  'editorTabs.scrollLeft': '向左滚动标签页',
  'editorTabs.scrollRight': '向右滚动标签页',

  // --- Layout, status bar & window controls ---
  'agentStatus.idleTooltip': '切换会话',
  'agentStatus.runningTooltip': '{0} 个会话正在运行 — 切换会话',
  'agentStatus.waitingTooltip': '{0} 个会话等待输入 — 切换会话',
  'commandCenter.tooltip': '搜索 {name} — {title}',
  'layoutControls.configureLayout': '配置布局',
  'status.cursorPosition': '光标位置',
  'status.editorEncoding': '编辑器编码',
  'status.selectLanguageMode': '选择语言模式',
  'status.notifications': '通知',
  'status.cursorPositionText': '行 {line}，列 {column}',
  'status.unreadNotifications': '{count} 条未读通知',
  'statusbar.label': '状态栏',
  'windowControls.maximize': '最大化',
  'windowControls.minimize': '最小化',
  'windowControls.restore': '向下还原',

  // --- Welcome ---
  'welcome.agent.desc':
    'Agents 是此编辑器的核心 - 描述你的目标，让 AI 和你一起编辑、搜索和运行任务。',
  'welcome.agent.openView': '打开 Sessions 面板',
  'welcome.agent.select': '选择 Agent…',
  'welcome.agent.start': '开始第一个 Agent 会话',
  'welcome.agent.title': '从 Agent 开始',
  'welcome.agentGuide': 'AGENT 工具使用说明',
  'welcome.commandPalette': '打开命令面板',
  'welcome.docs.center': '浏览文档中心',
  'welcome.docs.title': '文档',
  'welcome.editorGuide': '编辑器使用说明',
  'welcome.outputPanel': '切换输出底栏',
  'welcome.recent': '最近打开',
  'welcome.splitEditor': '拆分编辑器',

  // --- Actions ---
  'action.about.title': '关于 Universe Editor',
  'action.agent.activateConfigEntry': '聚焦会话配置项',
  'action.agent.addSelectionToExistingChat': '将选区添加到已有 Agent 聊天',
  'action.agent.addSelectionToNewChat': '将选区添加到新建 Agent 聊天',
  'action.agent.aiFixProblem': '使用 AI 修复',
  'action.agent.archiveSession': '归档 Agent 会话',
  'action.agent.cancelTurn': '取消 Agent 回合',
  'action.agent.clearBookmarks': '清除全部会话书签',
  'action.agent.clearHistory': '清除 Agent 会话历史',
  'action.agent.collapseCard': '折叠卡片',
  'action.agent.collapseCardSubtree': '折叠卡片及子项',
  'action.agent.copyContextText': '复制文本',
  'action.agent.copyFocusedMessage': '复制消息',
  'action.agent.copyImage': '复制图片',
  'action.agent.copyReference': '复制引用',
  'action.agent.copyResourcePath': '复制路径',
  'action.agent.copySubAgentTranscript': '复制子 Agent 记录',
  'action.agent.cycleTimelineCollapse': '循环切换时间线折叠状态（全部）',
  'action.agent.expandCard': '展开卡片',
  'action.agent.expandCardSubtree': '展开卡片及子项',
  'action.agent.find': '在会话中查找',
  'action.agent.findClose': '关闭查找',
  'action.agent.findNext': '查找下一个',
  'action.agent.findPrevious': '查找上一个',
  'action.agent.focusBottomTimelineItem': '聚焦时间线底部项',
  'action.agent.focusDeeperTimelineItem': '展开卡片或进入子时间线',
  'action.agent.focusInput': '聚焦 Agent 输入框',
  'action.agent.focusNextTimelineItem': '聚焦下一个时间线项',
  'action.agent.focusOuterTimelineItem': '折叠卡片或返回父级',
  'action.agent.focusPreviousTimelineItem': '聚焦上一个时间线项',
  'action.agent.focusTopTimelineItem': '聚焦时间线顶部项',
  'action.agent.forkSession': '分叉 Agent 会话',
  'action.agent.goToParentSession': '转到父会话',
  'action.agent.jumpToBookmark0': '跳转到会话书签 0',
  'action.agent.jumpToBookmark1': '跳转到会话书签 1',
  'action.agent.jumpToBookmark2': '跳转到会话书签 2',
  'action.agent.jumpToBookmark3': '跳转到会话书签 3',
  'action.agent.jumpToBookmark4': '跳转到会话书签 4',
  'action.agent.jumpToBookmark5': '跳转到会话书签 5',
  'action.agent.jumpToBookmark6': '跳转到会话书签 6',
  'action.agent.jumpToBookmark7': '跳转到会话书签 7',
  'action.agent.jumpToBookmark8': '跳转到会话书签 8',
  'action.agent.jumpToBookmark9': '跳转到会话书签 9',
  'action.agent.jumpToPlan': '跳转到计划',
  'action.agent.listBookmarks': '列出会话书签',
  'action.agent.newSession': '新建 Agent 会话',
  'action.agent.newSessionInCurrentEditor': '在当前编辑器中新建 Agent 会话',
  'action.agent.newSessionInFolder': '在此处新建 Agent 会话',
  'action.agent.newSessionWithScope': '新建会话（选择作用域）…',
  'action.agent.newSideTask': '新建侧边任务',
  'action.agent.openClaudeConfig': '打开 Claude 配置（JSON）',
  'action.agent.openCodexConfig': '打开 Codex 配置（TOML）',
  'action.agent.openMcpSettings': '打开 MCP 设置',
  'action.agent.openSettings': '打开 Agent 设置',
  'action.agent.openSideTask': '打开侧边任务…',
  'action.agent.openView': '打开 Sessions 视图',
  'action.agent.pinSession': '固定 Agent 会话',
  'action.agent.prompt.acceptSuggestion': '接受建议',
  'action.agent.prompt.hideSuggestion': '隐藏建议',
  'action.agent.prompt.selectNextSuggestion': '选择下一个建议',
  'action.agent.prompt.selectPreviousSuggestion': '选择上一个建议',
  'action.agent.refreshSessions': '刷新 Agent 会话列表',
  'action.agent.renameSession': '重命名 Agent 会话…',
  'action.agent.resumeSession': '恢复 Agent 会话…',
  'action.agent.revealSessionInOS': '在文件管理器中显示会话记录',
  'action.agent.rewindSession': '回退到此处',
  'action.agent.scrollTimelineDown': '向下滚动时间线',
  'action.agent.scrollTimelinePageDown': '时间线向下翻页',
  'action.agent.scrollTimelinePageUp': '时间线向上翻页',
  'action.agent.scrollTimelineToBottom': '滚动时间线到底部',
  'action.agent.scrollTimelineToTop': '滚动时间线到顶部',
  'action.agent.scrollTimelineUp': '向上滚动时间线',
  'action.agent.selectAgent': '选择 Agent…',
  'action.agent.selectMode': '选择 Agent 模式…',
  'action.agent.selectModel': '选择 Agent 模型…',
  'action.agent.selectThoughtLevel': '选择 Agent 思考级别…',
  'action.agent.configureAiFix': '配置 AI 修复…',
  'action.agent.sendCommitToChat': '发送到 Agent 聊天',
  'action.agent.showSessionChanges': '显示会话更改',
  'action.agent.switchSession': '切换会话…',
  'action.agent.switchSessionReverse': '切换到上一个会话…',
  'action.agent.toggleBookmark0': '切换会话书签 0',
  'action.agent.toggleBookmark1': '切换会话书签 1',
  'action.agent.toggleBookmark2': '切换会话书签 2',
  'action.agent.toggleBookmark3': '切换会话书签 3',
  'action.agent.toggleBookmark4': '切换会话书签 4',
  'action.agent.toggleBookmark5': '切换会话书签 5',
  'action.agent.toggleBookmark6': '切换会话书签 6',
  'action.agent.toggleBookmark7': '切换会话书签 7',
  'action.agent.toggleBookmark8': '切换会话书签 8',
  'action.agent.toggleBookmark9': '切换会话书签 9',
  'action.agent.toggleCardSubtreeCollapse': '切换卡片及子项折叠状态',
  'action.agent.toggleTimelineItemCollapse': '切换时间线项折叠状态',
  'action.agent.unarchiveSession': '取消归档 Agent 会话',
  'action.agent.unpinSession': '取消固定 Agent 会话',
  'action.ai.clearApiKey': '清除 AI 提供方 API 密钥',
  'action.ai.commitMessage.pickModel': '选择提交信息模型',
  'action.ai.inlineCompletion.commit': '采纳内联补全',
  'action.ai.inlineCompletion.jump': '跳转到下一个编辑建议',
  'action.ai.inlineCompletion.pickModel': '选择内联补全模型',
  'action.ai.inlineCompletion.toggleInEditor': '在编辑器中开关内联补全',
  'action.ai.inlineCompletion.toggleInSession': '在会话输入框中开关内联补全',
  'action.ai.inlineCompletion.trigger': '触发内联补全',
  'action.ai.openSettings': '打开 AI 与 Agent 设置',
  'action.ai.openSettingsJson': '打开 AI 设置（JSON）',
  'action.ai.pickModel': '选择 AI 模型',
  'action.ai.revealScm': '显示源代码管理',
  'action.ai.sessionTitle.pickModel': '选择会话标题模型',
  'action.ai.setApiKey': '设置 AI 提供方 API 密钥',
  'action.changeLanguageMode': '更改语言模式',
  'action.clearHistory.title': '清除导航历史',
  'action.clearOutput.title': '清空输出',
  'action.clearRecentFiles.title': '清除最近打开的文件',
  'action.clearRecentWorkspaces.title': '清除最近打开的项',
  'action.closeActiveEditor.title': '关闭编辑器',
  'action.closeActivePinnedEditor.title': '关闭固定的编辑器',
  'action.closeAllEditors.title': '关闭所有编辑器',
  'action.closeEditorsInGroup.title': '关闭组内所有编辑器',
  'action.closeEditorsToTheLeft.title': '关闭左侧编辑器',
  'action.closeEditorsToTheRight.title': '关闭右侧编辑器',
  'action.closeFolder.title': '关闭文件夹',
  'action.closeOtherEditors.title': '关闭其他编辑器',
  'action.closeQuickInput.title': '关闭快速输入',
  'action.closeUnmodifiedEditors.title': '关闭已保存的编辑器',
  'action.closeWindow.title': '关闭窗口',
  'action.completeMerge.title': '完成合并',
  'action.configureDisplayLanguage.title': '配置显示语言',
  'action.selectForCompare.title': '选择以进行比较',
  'action.compareWithSelected.title': '与所选项进行比较',
  'action.compareFiles.title': '比较所选文件',
  'action.copy.title': '复制',
  'action.copyFilePath.title': '复制路径',
  'action.copyName.title': '复制名称',
  'action.copyRelativeFilePath.title': '复制相对路径',
  'action.cut.title': '剪切',
  'action.decreaseViewHeight.title': '减小当前视图高度',
  'action.decreaseViewWidth.title': '减小当前视图宽度',
  'action.deleteFile.title': '删除',
  'action.filesExplorer.cancelCut': '取消剪切',
  'action.filesExplorer.copy': '复制',
  'action.filesExplorer.cut': '剪切',
  'action.filesExplorer.duplicate': '复制副本…',
  'action.filesExplorer.move': '移动…',
  'action.filesExplorer.paste': '粘贴',
  'action.filesExplorer.undo': '撤销',
  'action.filesExplorer.redo': '重做',
  'action.diffEditor.nextChange.title': '转到下一处更改',
  'action.diffEditor.previousChange.title': '转到上一处更改',
  'action.editor.nextChange.title': '转到下一处更改',
  'action.editor.previousChange.title': '转到上一处更改',
  'action.exit.title': '退出',
  'action.exportDiagnostics.title': '导出诊断包...',
  'action.openHeapSnapshotsFolder.title': '打开内存快照目录',
  'action.startHeapSnapshotDiagnostics.title': '开始内存诊断',
  'action.stopHeapSnapshotDiagnostics.title': '停止内存诊断',
  'action.find.title': '查找',
  'action.findInFiles.title': '在文件中查找',
  'action.findInFolder.title': '在文件夹中查找...',
  'action.findNext.title': '查找下一个',
  'action.findPrevious.title': '查找上一个',
  'action.findWordAtCursor.next.title': '查找光标处的单词：下一个',
  'action.findWordAtCursor.previous.title': '查找光标处的单词：上一个',
  'action.firstEditorInGroup.title': '打开组内第一个编辑器',
  'action.focusScope.addFolder.title': '添加到聚焦',
  'action.focusScope.addFolders.title': '添加到聚焦…',
  'action.focusScope.clear.title': '退出聚焦模式',
  'action.focusScope.focusFolder.title': '聚焦此项',
  'action.focusScope.manage.title': '管理聚焦项',
  'action.focusScope.removeFolder.title': '从聚焦中移除',
  'action.focusAboveGroup.title': '聚焦上方编辑器组',
  'action.focusActiveEditorGroup.title': '聚焦当前编辑器组',
  'action.focusBelowGroup.title': '聚焦下方编辑器组',
  'action.focusFirstGroup.title': '聚焦第一个组',
  'action.focusLastGroup.title': '聚焦最后一个组',
  'action.focusLeftGroup.title': '聚焦左侧编辑器组',
  'action.focusNextGroup.title': '聚焦下一个组',
  'action.focusNextPart.title': '聚焦下一部分',
  'action.focusOutline.title': '聚焦大纲视图',
  'action.focusPreviousGroup.title': '聚焦上一个组',
  'action.focusPreviousPart.title': '聚焦上一部分',
  'action.focusRightGroup.title': '聚焦右侧编辑器组',
  'action.goBack.title': '后退',
  'action.goForward.title': '前进',
  'action.goToFile.title': '转到文件…',
  'action.goToImplementation.title': '转到实现',
  'action.goToReferences.title': '转到引用',
  'action.goToTypeDefinition.title': '转到类型定义',
  'action.gotoSymbol.title': '转到编辑器中的符号…',
  'action.html.openPreview.title': '打开预览',
  'action.html.openPreviewToSide.title': '在侧边打开预览',
  'action.html.showSource.title': '打开源代码',
  'action.increaseViewHeight.title': '增大当前视图高度',
  'action.increaseViewWidth.title': '增大当前视图宽度',
  'action.json.showSchema.title': '显示 JSON Schema',
  'action.lastEditorInGroup.title': '打开组内最后一个编辑器',
  'action.lockEditorGroup.title': '锁定组',
  'action.markBugRecordingStep.title': '标记 Bug 录制步骤',
  'action.markdown.findFileReferences': '查找文件引用',
  'action.markdown.openPreview.title': '打开预览',
  'action.markdown.openPreviewToSide.title': '在侧边打开预览',
  'action.markdown.showSource.title': '打开源文件',
  'action.markdownPreview.find.title': '在预览中查找',
  'action.markdownPreview.findClose.title': '关闭查找',
  'action.markdownPreview.findNext.title': '查找下一个',
  'action.markdownPreview.findPrevious.title': '查找上一个',
  'action.markdownPreview.help.title': '键盘快捷键',
  'action.markdownPreview.linkHints.title': '显示链接提示',
  'action.markdownPreview.linkHintsToSide.title': '显示链接提示（侧边）',
  'action.moveEditorLeftInGroup.title': '向左移动编辑器',
  'action.moveEditorRightInGroup.title': '向右移动编辑器',
  'action.moveEditorToAboveGroup.title': '将编辑器移动到上方组',
  'action.moveEditorToBelowGroup.title': '将编辑器移动到下方组',
  'action.moveEditorToLeftGroup.title': '将编辑器移动到左侧组',
  'action.moveEditorToNextGroup.title': '将编辑器移动到下一组',
  'action.moveEditorToPreviousGroup.title': '将编辑器移动到上一组',
  'action.moveEditorToRightGroup.title': '将编辑器移动到右侧组',
  'action.newFile.title': '新建文件…',
  'action.newFolder.title': '新建文件夹…',
  'action.newTerminal.title': '新建终端',
  'action.newUntitledFile.title': '新建文件',
  'action.newWindow.title': '新建窗口',
  'action.nextEditor.title': '打开下一个编辑器',
  'action.nextViewContainer.title': '打开下一个视图容器',
  'action.notifications.clearAll': '清除所有通知',
  'action.notifications.test': '测试通知',
  'action.notifications.test.message': '这是一条测试通知。',
  'action.notifications.toggleList': '切换通知',
  'action.openActiveLogFile.title': '开发人员：打开活跃日志文件',
  'action.openAgentGuide.title': 'AGENT 工具使用说明',
  'action.openConfigLocationFolder.title': '打开配置目录',
  'action.openContainingFolder.title': '打开所在文件夹',
  'action.openContainingFolder.remoteNotSupported':
    '该文件位于远程主机，无法在本地文件管理器中打开。',
  'action.openDocs.title': '文档',
  'action.openEditorGuide.title': '编辑器使用说明',
  'action.openExtensionDocs.title': '扩展开发文档',
  'action.openFile.title': '打开文件…',
  'action.openFolder.title': '打开文件夹…',
  'action.openFolderInNewWindow.title': '在新窗口中打开文件夹…',
  'action.openInTerminal.title': '在外部终端中打开',
  'action.openKeybindings.title': '打开键盘快捷方式',
  'action.openKeybindingsJson.title': '打开键盘快捷方式（JSON）',
  'action.openLogFile.title': '开发人员：打开日志文件...',
  'action.openLogsFolder.title': '开发人员：打开日志文件夹',
  'action.openRecent.title': '打开最近打开的项…',
  'action.openSettings.title': '打开设置',
  'action.openSettingsJson.title': '打开设置（JSON）',
  'action.openUpdateConfigJson.title': '打开更新配置（JSON）',
  'action.openUserDataFolder.title': '开发人员：打开用户数据文件夹',
  'action.openInstallFolder.title': '开发人员：打开安装目录',
  'action.openVSCodeKeybindingsJson.notFound':
    '未找到 VS Code 键盘快捷方式文件（可能未安装 VS Code）。',
  'action.openVSCodeKeybindingsJson.title': '打开 VS Code 键盘快捷方式（JSON）',
  'action.openVSCodeSettingsJson.notFound': '未找到 VS Code 设置文件（可能未安装 VS Code）。',
  'action.openVSCodeSettingsJson.title': '打开 VS Code 设置（JSON）',
  'action.openWithDefaultApplication.title': '使用默认应用打开',
  'action.openWorkspaceInVSCode.failed': '打开 VS Code 失败。请确认 `code` 命令已加入 PATH。',
  'action.openWorkspaceInVSCode.remoteNotSupported':
    '暂不支持在 VS Code 中打开该类型的远程工作区。',
  'action.openWorkspaceInVSCode.title': '在 VS Code 中打开工作区',
  'action.openWorkspaceSettings.title': '打开工作区设置',
  'action.openWorkspaceSettingsJson.title': '打开工作区设置（JSON）',
  'action.openProcessExplorer.title': '打开进程资源管理器',
  'action.openTimeline.title': '打开时间线',
  'action.openWorkspaceInCurrentWindow.title': '在当前窗口打开',
  'action.openWorkspaceInNewWindow.title': '在新窗口打开',
  'action.outline.navigateDown': '大纲：选择下一项',
  'action.outline.navigateLeft': '大纲：折叠或选择父项',
  'action.outline.navigateRight': '大纲：展开或选择第一个子项',
  'action.outline.navigateUp': '大纲：选择上一项',
  'action.paste.title': '粘贴',
  'action.peekDeclaration.title': '速览声明',
  'action.peekDefinition.title': '速览定义',
  'action.peekImplementation.title': '速览实现',
  'action.pinEditor.title': '固定标签',
  'action.peekReferences.title': '速览引用',
  'action.peekTypeDefinition.title': '速览类型定义',
  'action.previousEditor.title': '打开上一个编辑器',
  'action.previousViewContainer.title': '打开上一个视图容器',
  'action.quickOpenRecentEditor.title': '打开最近使用的编辑器或视图',
  'action.quickOpenRecentEditorReverse.title': '打开最久未使用的编辑器或视图',
  'action.quickTextSearch.title': '快速搜索',
  'action.redo.title': '重做',
  'action.refresh.title': '刷新资源管理器',
  'action.refreshLogOutput.title': '开发人员：刷新日志输出',
  'action.reloadWindow.title': '重载窗口',
  'action.rename.title': '重命名…',
  'action.remote.closeConnection.title': '关闭连接',
  'action.remote.connectToHost.title': '连接到主机…',
  'action.remote.connectToWsl.title': '连接到 WSL…',
  'action.remote.openFolder.title': '在主机上打开文件夹…',
  'action.remote.removeManualHost.title': '遗忘',
  'action.remote.retryConnection.title': '重试连接',
  'action.remote.stopServer.title': '停止远程服务器',
  'action.removeRecent.title': '从最近打开中移除',
  'action.reopenClosedEditor.title': '重新打开已关闭的编辑器',
  'action.reopenWith.placeHolder': '选择编辑器类型',
  'action.reopenWith.title': '重新打开方式...',
  'action.replace.title': '替换',
  'action.reportIssue.title': '报告问题...',
  'action.resetConfigLocation.title': '将配置目录重置为默认值',
  'action.resetViewLocations.title': '重置视图位置',
  'action.revealActiveFileInExplorer.title': '在资源管理器中定位当前文件',
  'action.revealInExplorer.title': '在资源管理器视图中显示',
  'action.revealDeclaration.title': '转到声明',
  'action.revealDefinition.title': '转到定义',
  'action.revealDefinitionAside.title': '在侧边打开定义',
  'action.save.title': '保存',
  'action.saveAs.title': '另存为…',
  'action.saveAll.title': '全部保存',
  'action.scm.openChanges.title': '打开更改',
  'action.scm.openFile.title': '打开文件',
  'action.scm.openPreview.title': '打开预览',
  'action.scm.switchRepo.placeholder': '选择仓库',
  'action.scm.blame.toggleEditorDecoration.title': '切换编辑器的 Blame 装饰',
  'action.scm.blame.toggleStatusBarItem.title': '切换状态栏 Blame 项',
  'action.scm.switchRepo.title': '切换仓库…',
  'action.selectAll.title': '全选',
  'action.selectIconTheme.title': '文件图标主题',
  'action.selectProductIconTheme.title': '产品图标主题',
  'action.selectTheme.title': '颜色主题',
  'action.setConfigLocation.title': '设置配置目录…',
  'action.setLogLevel.title': '开发人员：设置日志级别...',
  'action.showAllCommands.title': '显示所有命令',
  'action.showAllSymbols.title': '转到工作区中的符号…',
  'action.showExplorer.title': '显示资源管理器',
  'action.showExtensions.title': '显示扩展',
  'action.showLogs.title': '开发人员：显示日志...',
  'action.showOutputChannel.title': '输出：显示输出通道...',
  'action.showScm.title': '显示源代码管理',
  'action.showStartupPerformance.title': '启动性能',
  'action.showInteractionPerformance.title': '交互性能',
  'action.splitEditorDown.title': '向下拆分编辑器',
  'action.splitEditorLeft.title': '向左拆分编辑器',
  'action.splitEditorRight.title': '向右拆分编辑器',
  'action.splitEditorUp.title': '向上拆分编辑器',
  'action.splitTerminal.title': '拆分终端',
  'action.startBugRecording.title': '开始 Bug 录制',
  'action.stopBugRecording.title': '停止 Bug 录制并导出证据包',
  'action.switchWindow.title': '切换窗口…',
  'action.terminal.focusPanel.title': '聚焦终端',
  'action.terminalInEditor.title': '在编辑器中打开终端',
  'action.toggleActivityBar.title': '切换活动栏',
  'action.toggleDeveloperTools.title': '切换开发者工具',
  'action.toggleEditorGroupLock.title': '切换编辑器组锁定',
  'action.toggleKeybindingsLog.title': '开发人员：切换键盘快捷方式故障排查',
  'action.toggleMaximizedPanel.title': '切换最大化底栏',
  'action.toggleMinimap.title': '切换小地图',
  'action.toggleOutput.title': '视图：切换输出',
  'action.toggleOutputAutoScroll.title': '输出：切换自动滚动',
  'action.togglePanel.title': '切换底栏',
  'action.togglePrimarySideBar.title': '切换主侧边栏',
  'action.toggleSecondarySideBar.title': '切换辅助侧边栏',
  'action.toggleTerminal.title': '切换终端',
  'action.toggleWordWrap.title': '切换自动换行',
  'action.undo.title': '撤销',
  'action.unlockEditorGroup.title': '解锁组',
  'action.unpinEditor.title': '取消固定标签',
  'action.trust.grant': '信任工作区',
  'action.trust.manage': '管理工作区信任',
  'action.trust.revoke': '限制工作区（撤销信任）',
  'action.zoomIn.title': '放大',
  'action.zoomOut.title': '缩小',
  'action.zoomReset.title': '重置缩放',
  'action.diff.openDiff': '打开差异对比',
  'action.diff.openWebviewDiff': '打开 Webview 差异对比',
  'action.editor.closeChange.title': '关闭更改速览',
  'action.editor.showChange.title': '显示更改',
  'action.emitTestErrorLog.title': '开发人员：输出测试错误日志',
  'action.extensionApi.getActiveEditorFile': '获取活动编辑器文件',
  'action.extensionApi.getConfiguration': '获取配置',
  'action.extensionApi.openFile': '打开文件',
  'action.extensionApi.openFileAt': '在指定位置打开文件',
  'action.extensionApi.openFolder': '打开文件夹',
  'action.extensionApi.openFolderInNewWindow': '在新窗口中打开文件夹',
  'action.extensionApi.updateConfiguration': '更新配置',
  'action.extensionApi.writeClipboard': '写入剪贴板',
  'action.commitChanges.show': '显示提交变更',
  'action.commitChanges.focus': '聚焦提交变更视图',
  'action.sessionChanges.focus': '聚焦会话更改视图',
  'action.gitGraph.focusSearch': '聚焦搜索',
  'action.gitGraph.open': '打开 Git Graph',
  'action.gitGraph.refresh': '刷新',
  'action.gitGraph.toggleRemoteBranches': '切换远程分支显示',
  'action.gitGraph.view': '查看 Git Graph',
  'action.merge.openMergeEditor': '打开合并编辑器',
  'action.perforceGraph.focusSearch': '聚焦搜索',
  'action.perforceGraph.open': '打开 Perforce Graph',
  'action.perforceGraph.refresh': '刷新',
  'action.perforceGraph.view': '查看 Perforce Graph',
  'action.perforceGraph.viewFileHistory': '查看文件历史',
  'action.reopenWith': '重新打开方式…',
  'action.swarm.jumpToChanges': '转到 Swarm 更改',
  'action.swarm.openReview': '打开 Swarm 审核',
  'action.swarm.openReviewById': '按 ID 打开 Swarm 审核…',
  'action.swarm.pollTick': 'Swarm 轮询',
  'action.swarm.refreshReviews': '刷新 Swarm 审核',
  'action.swarm.showReviews': '显示 Swarm 审核',
  'action.swarmChanges.focus': '聚焦 Swarm 更改视图',
  'swarm.applyToLocal': '应用到本地',

  // --- Dialogs ---
  'dialog.default.cancel': '取消',
  'dialog.default.ok': '确定',
  'dialog.closeEditor.message': '是否保存对 {label} 的更改？',
  'dialog.closeEditor.detail': '如果不保存，你的更改将丢失。',
  'dialog.closeEditor.dontSave': '不保存',
  'dialog.displayLanguage.detail': '重启应用后，所选显示语言才会生效。',
  'dialog.displayLanguage.message': '显示语言已更新。',
  'dialog.file.create.error.file': '创建文件失败',
  'dialog.file.create.error.folder': '创建文件夹失败',
  'dialog.file.delete.confirm.detail.directory': '这会永久删除该文件夹及其所有内容。',
  'dialog.file.delete.confirm.detail.file': '这会永久删除该文件。',
  'dialog.file.delete.confirm.detail.trash': '你可以从{trash}中恢复它。',
  'dialog.file.delete.moveToTrash': '移到{trash}',
  'dialog.file.delete.confirm.message': '确定要删除"{name}"吗？',
  'dialog.file.delete.confirm.message.multiple': '确定要删除所选的 {count} 个项目吗？',
  'dialog.file.delete.error': '删除失败',
  'dialog.file.delete.permanent.button': '永久删除',
  'dialog.file.delete.permanent.retry.detail': '你也可以选择永久删除。',
  'dialog.file.clipboard.confirm.detail': '在其他应用中粘贴时，将使用这些临时副本。',
  'dialog.file.clipboard.confirm.message':
    '需要先将 {size} 的远端文件下载到临时位置，才能放入系统剪贴板。要继续吗？',
  'dialog.file.clipboard.refused.detail':
    '超过 2GB 或 100,000 个项目的内容无法放入系统剪贴板。请减少选择后再试。',
  'dialog.file.clipboard.refused.message': '所选内容过大，无法复制到系统剪贴板',
  'dialog.file.compare.error': '比较失败',
  'dialog.file.duplicate.error': '复制副本失败',
  'dialog.file.move.error': '移动失败',
  'dialog.file.move.overwrite.detail': '替换会覆盖目标位置中已有的项目。',
  'dialog.file.move.overwrite.message':
    '目标文件夹中已存在名为"{name}"的文件或文件夹。要替换它吗？',
  'dialog.file.open.error': '无法打开文件',
  'dialog.file.paste.error': '粘贴失败',
  'dialog.file.prompt.duplicate': '复制副本',
  'dialog.file.prompt.newFile': '新建文件',
  'dialog.file.prompt.newFolder': '新建文件夹',
  'dialog.file.prompt.rename': '重命名',
  'dialog.file.rename.error': '重命名失败',
  'doc.localeFallback': '此页面暂无你所用显示语言的版本，当前显示中文版。',
  'fileDialog.move.openLabel': '移动',
  'fileDialog.move.title': '选择目标文件夹',
  'dialog.largeFile.detail': '大文件可能导致编辑器无响应。',
  'dialog.largeFile.message': '该文件大小为 {size}。仍要打开吗？',
  'dialog.binaryFile.detail': '该文件无法在文本编辑器中正常显示。',
  'dialog.binaryFile.message': '文件是二进制文件或使用了不受支持的文本编码。',
  'dialog.binaryFile.openAnyway': '仍然打开',

  // --- Quick input ---
  'quickAccess.commands.placeholder': '输入命令名称…',
  'quickAccess.file.placeholder': '转到文件…',
  'quickAccess.fileSymbol.placeholder': '转到编辑器中的符号…',
  'quickAccess.fileSymbolByCategory.placeholder': '按类别转到编辑器中的符号…',
  'quickAccess.workspaceSymbol.placeholder': '转到工作区中的符号…',
  'quickInput.colorTheme.placeholder': '选择颜色主题',
  'quickInput.commandPalette.placeholder': '输入命令名称…',
  'quickInput.iconTheme.placeholder': '选择文件图标主题',
  'quickInput.productIconTheme.placeholder': '选择产品图标主题',
  'quickInput.displayLanguage.auto.description': '使用操作系统的显示语言。',
  'quickInput.displayLanguage.en-US.description': '将编辑器界面显示为英文。',
  'quickInput.displayLanguage.placeholder': '选择显示语言',
  'quickInput.displayLanguage.zh-CN.description': '将编辑器界面显示为简体中文。',
  'quickInput.goToFile.placeholder': '转到文件…',
  'quickInput.logLevel.placeholder': '选择日志级别',
  'quickInput.logs.placeholder': '选择日志文件',
  'quickInput.openLogFile.placeholder': '选择要在编辑器中打开的日志文件',
  'quickInput.openRecent.placeholder': '打开最近打开的项',
  'quickInput.openRecentFile.placeholder': '打开最近的文件…',
  'quickInput.outputChannel.placeholder': '选择要显示的输出通道',
  'quickInput.quickSearch.enterTerm': '输入要在文件中搜索的内容。',
  'quickInput.quickSearch.failed': '搜索失败。',
  'quickInput.quickSearch.moreResults': 'Search 视图中还有更多结果。',
  'quickInput.quickSearch.noResults': '没有匹配结果',
  'quickInput.quickSearch.noWorkspace': '打开文件夹后才能搜索文件。',
  'quickInput.quickSearch.placeholder': '在工作区文件中搜索文本。',
  'quickInput.showAllSymbols.placeholder': '转到工作区中的符号…',
  'quickInput.switchWindow.placeholder': '选择要切换到的窗口',
  'quickOpenRecentEditor.group': '组 {id}',
  'quickOpenRecentEditor.placeholder': '最近使用的编辑器和视图',
  'quickInput.button.ariaLabel': '快速选择按钮',
  'quickInput.filter.placeholder': '输入以筛选…',
  'quickInput.input.ariaLabel': '快速选择输入框',
  'quickInput.inputBox.ariaLabel': '输入框',
  'quickInput.noResults': '没有结果',
  'quickInput.prefixHint': "输入 '{prefix}' 后接命令名称",
  'quickInput.quickNavigateHint': '松开 {modifier} 打开选中项 · 按 Enter 输入过滤',
  'quickInput.removeFromList': '从列表中移除',

  // --- Settings ---
  'settings.acp.agents':
    '自定义 ACP 兼容 Agent 命令。每项都需要 `id`、`command`；`args`、`env`、`cwd` 可选。Env 值会以明文存储 - 请将 API 密钥保存在真实环境变量中。',
  'settings.acp.claude.executablePath':
    'Claude 可执行文件的绝对路径。仅当 `acp.claude.source` 为 "custom" 时使用。',
  'settings.tocAriaLabel': '设置分类',
  'settings.acp.claude.source':
    '如何获取内置 Agent 使用的原生 Claude 二进制文件："download" 会在首次使用时下载到用户数据目录，"system" 会复用 PATH 中找到的 `claude` 安装，"custom" 会使用 `acp.claude.executablePath` 中的路径。',
  'settings.acp.codex.apiKey':
    '作为 OPENAI_API_KEY 传给 Codex 的 OpenAI API 密钥。会以明文存储 - 更建议设置真实的 OPENAI_API_KEY / CODEX_API_KEY 环境变量；此项留空时 Agent 会自动继承。',
  'settings.acp.codex.executablePath':
    'codex-acp 可执行文件的绝对路径。仅当 `acp.codex.source` 为 "custom" 时使用。',
  'settings.acp.codex.source':
    '如何获取内置 Codex Agent 使用的 codex-acp 适配器二进制文件："download" 会在首次使用时下载到用户数据目录，"system" 会复用 PATH 中找到的 `codex-acp` 安装，"custom" 会使用 `acp.codex.executablePath` 中的路径。',
  'settings.acp.defaultAgentId': '"新建 Agent 会话"使用的默认 Agent。',
  'settings.acp.aiFix.agentId':
    '「使用 AI 修复」（错误悬停快速修复）使用的 Agent。每次修复都新建专用会话；非法 ID 回退到默认 Agent。请通过「配置 AI 修复」命令选择。',
  'settings.acp.aiFix.model':
    '「使用 AI 修复」会话使用的模型。留空跟随 Agent 默认。请通过「配置 AI 修复」命令选择，不要手填 ID。',
  'settings.acp.aiFix.thoughtLevel':
    '「使用 AI 修复」会话的思考深度（如 low/medium/high）。留空跟随 Agent 默认；Agent 不提供的值会被忽略并告警。请通过「配置 AI 修复」命令选择。',
  'settings.acp.aiFix.mode':
    '「使用 AI 修复」会话的模式。留空不固定模式（AI 修复需要写权限来改文件，默认不固定）。',
  'settings.acp.defaultCollapseModes':
    '按 Agent ID 设置新会话的默认时间线折叠模式。可选值："default"（按类型智能折叠）、"collapsed"（全部折叠）、"expanded"（全部展开，但子 Agent 卡片最多保留一张展开）。示例：{ "claude-code": "default", "codex": "collapsed" }。',
  'settings.acp.mcpCard.defaultExpanded':
    'MCP 工具调用卡片默认展开哪些区域："both"（输入和输出）、"output"（仅输出）或 "none"（全部折叠）。',
  'settings.acp.fontFamily': '控制 Agent 聊天面板的字体。留空则继承工作台 UI 字体。',
  'settings.acp.fontSize':
    '控制 Agent 聊天面板的基础字体大小（像素）。代码块和其他内容会相对缩放。',
  'settings.acp.idleProcessTimeoutMs':
    '所有会话闲置该时长（毫秒）后自动停止对应的 Agent 进程以释放内存，下次发送消息时自动重连恢复。设为 0 表示禁用。',
  'settings.acp.mcpServers':
    '转发给 Agent session/new 的 MCP 服务器，以服务器名为键。stdio：`{ "fs": { "command": "npx", "args": ["-y", "@modelcontextprotocol/server-filesystem", "."], "env": {} } }`。http/sse：`{ "docs": { "type": "http", "url": "https://…", "headers": {} } }`。工作区根目录的 `.mcp.json` 会按同名条目覆盖此处的定义。默认启停开关不属于这些条目——在 MCP 设置面板或会话选择器中按用户/工作区两级设置。Agent 不支持的传输（http/sse）会跳过并发出警告；env/header 值会以明文存储 - 请将密钥保存在真实环境变量中。',
  'settings.acp.notifications.enabled':
    '当编辑器窗口未聚焦且 Agent 需要注意（权限请求、问题、任务完成或错误）时，显示系统桌面通知。点击通知会聚焦窗口并跳转到该会话。会自动继续的计划确认卡片（见 `acp.plan.autoExecute`）不会通知。',
  'settings.acp.permissions': '无需提示即可自动批准的工具调用类型（例如 "fs.read"）。',
  'settings.acp.plan.autoExecute':
    '计划模式的会话完成计划（"Ready to code?"）后，经确认卡片上的短暂倒计时自动以所选模式继续——悬停或与卡片交互即可接管。"off" 表示始终等待手动选择。计划确认卡片上的复选框会把此设置切换为 "bypassPermissions"。',
  'settings.acp.plan.autoExecute.off': '关闭',
  'settings.acp.plan.autoExecute.bypassPermissions': 'Bypass 权限',
  'settings.acp.plan.autoExecute.auto': '自动',
  'settings.acp.plan.autoExecute.acceptEdits': '接受编辑',
  'settings.acp.plan.autoExecute.default': '手动批准编辑',
  'settings.acp.prefetchBinaries':
    '在编辑器空闲时后台下载最新 Claude / codex-acp 二进制文件（仅受管下载），这样升级可立即完成。在远程工作区下会在远端主机上预下载。关闭可节省带宽。',
  'settings.acp.startupTimeoutMs':
    '等待 Agent 响应 `initialize` + `session/new` 的时长（毫秒），超时后放弃。仅在你确认 Agent 启动很快时才调低。',
  'settings.acp.projectRoots': '常驻子项目目录清单（支持绝对路径或相对工作区根路径）。',
  'settings.acp.subProject.detectEnabled': '是否在创建新会话时自动探测活动文件所属的项目根。',
  'settings.acp.subProject.detectMarkers': '子项目特征标记文件清单。',
  'settings.acp.subProject.maxDepth': '向上探测子项目标记时的最大层级。',
  'settings.acp.subscriptionUsage.refreshIntervalMs':
    '官方订阅用量指示器（Claude 套餐额度窗口 / Codex 限额）的刷新频率（毫秒）。每次刷新都要打到正在运行的 Agent 进程，因此轮询频率远低于网关用量指示器。',
  'settings.acp.subscriptionUsage.staleAfterMs':
    '一次订阅用量读数在多长时间内仍视为可信（毫秒）。超过后指示器会弱化数值，并在悬浮提示里标注数据读取时间。',
  'settings.agents': 'Agents',
  'settings.ai': 'AI',
  'settings.ai.accountUsage.refreshIntervalMs':
    '网关账号用量指示器的刷新间隔（毫秒）。每次刷新先读缓存；读数缺失或已过期（5 分钟）才向网关发起一次网络拉取。',
  'settings.ai.inlineCompletion.debounceDelay': '自动触发内联补全请求前的延迟（毫秒）。',
  'settings.ai.inlineCompletion.disabledLanguages':
    '禁用 AI 内联补全的语言 id，例如 ["json", "log"]。',
  'settings.ai.inlineCompletion.enabledInEditor': '在文本编辑器中启用 AI 内联补全（幽灵文本）。',
  'settings.ai.inlineCompletion.enabledInSession': '在会话输入框中启用 AI 内联补全（幽灵文本）。',
  'settings.ai.inlineCompletion.maxContextPrefixChars': '作为上下文发送的光标前最大字符数。',
  'settings.ai.inlineCompletion.maxContextSuffixChars': '作为上下文发送的光标后最大字符数。',
  'settings.ai.inlineCompletion.maxTokens': '单次内联补全生成的最大 token 数。',
  'settings.ai.inlineCompletion.multiline': '允许内联补全跨多行。关闭时补全会截断为单行。',
  'settings.ai.nes.contextLines':
    '为下一次编辑建议发送的光标上下文行数（启用 ai.nes.includeFullDocument 时忽略）。',
  'settings.ai.nes.debounceDelay': '自动触发下一次编辑建议请求前的延迟（毫秒）。',
  'settings.ai.nes.enabled':
    '启用下一次编辑建议：根据你最近的编辑预测文件中其他位置的下一次编辑，以内联 diff 展示，可跳转并用 Tab 接受。',
  'settings.ai.nes.fallbackToCompletion':
    '当下一次编辑建议没有结果时，回退为光标位置的幽灵文本补全。',
  'settings.ai.nes.includeFullDocument':
    '为下一次编辑建议发送整个文档作为上下文，而不是仅发送光标附近窗口。',
  'settings.ai.nes.maxTokens': '单次下一次编辑建议生成的最大 token 数。',
  'settings.ai.nes.recentEditsCount': '每个文件发送多少条最近编辑作为下一次编辑建议的上下文。',
  'settings.editor': '编辑器',
  'settings.editor.detectIndentation.description':
    '控制在打开文件时是否根据文件内容自动检测 `editor.tabSize` 和 `editor.insertSpaces`。',
  'settings.editor.disableMonospaceOptimizations.description':
    '禁用 Monaco 等宽字体渲染优化，强制对每一行逐字符精确测量宽度。包含 CJK（如中文）文本的行已经使用精确测量，因此除非你在纯 ASCII 行看到水平抖动，否则保持关闭（默认）。启用后可能让当前行在 IME 组合输入期间显得更粗/更暗。',
  'settings.editor.fontFamily.description': '控制编辑器字体。',
  'settings.editor.fontSize.description': '控制编辑器字体大小（像素）。',
  'settings.editor.fontWeight.description':
    '控制字体粗细。接受 "normal"、"bold" 或 100 到 900 之间的数字。',
  'settings.editor.insertSpaces.description':
    '按 Tab 键时插入空格。当 `editor.detectIndentation` 开启时，此设置会根据文件内容被覆盖。',
  'settings.editor.languageFonts.description':
    '按语言覆盖字体。键为 Monaco 语言 id（例如 "markdown"、"typescript"）。每个值可包含 "fontFamily"（字符串）和/或 "fontSize"（数字）。',
  'settings.editor.letterSpacing.description': '控制字母间距（像素）。',
  'settings.editor.lineHeight.description':
    '控制行高。使用 0 根据字体大小计算；0-8 的值会作为字体大小倍数；大于等于 8 的值会作为像素。',
  'settings.editor.lineHighlightBackground.description':
    '当前行高亮的背景色（CSS 颜色，支持 8 位 alpha）。留空则使用主题默认值。',
  'settings.editor.lineHighlightBorder.description':
    '当前行高亮的边框色。留空则使用主题默认值（透明，即无边框）。',
  'settings.editor.minimap.description': '控制是否显示小地图。',
  'settings.editor.largeFileOptimizations.description':
    '针对大文件的特殊处理：关闭部分高内存占用的功能。需要重新打开文件才能生效。',
  'settings.editor.occurrencesHighlight.description':
    '控制是否自动高亮光标下单词的出现位置（无需选区）。"off" 会禁用。',
  'settings.editor.renderLineHighlight.description':
    '控制当前行如何高亮。"line" 填充整行，"gutter" 只高亮行号区域，"all" 同时高亮两者，"none" 禁用。',
  'settings.editor.tabSize.description':
    '一个制表符等于的空格数。当 `editor.detectIndentation` 开启时，此设置会根据文件内容被覆盖。',
  'settings.editor.wordWrap.bounded': '在视口宽度与 `editor.wordWrapColumn` 中的较小值处换行。',
  'settings.editor.wordWrap.description': '控制行的换行方式。',
  'settings.editor.wordWrap.off': '永不换行。',
  'settings.editor.wordWrap.on': '在视口宽度处换行。',
  'settings.editor.wordWrap.wordWrapColumn': '在 `editor.wordWrapColumn` 指定的列处换行。',
  'settings.alsoModifiedIn': '也在{scope}设置中修改',
  'settings.clearSearch': '清除搜索',
  'settings.control.ariaLabel': '设置值',
  'settings.count': '找到 {count} 项设置',
  'settings.defaultBadge': '默认',
  'settings.empty': '没有匹配的设置。',
  'settings.enum.afterDelay': '延迟后',
  'settings.enum.auto': '跟随系统语言',
  'settings.enum.dark': '深色',
  'settings.enum.en-US': 'English',
  'settings.enum.light': '浅色',
  'settings.enum.off': '关闭',
  'settings.enum.onFocusChange': '失焦时',
  'settings.enum.terminal.cmd': '命令提示符',
  'settings.enum.terminal.powershell': 'PowerShell',
  'settings.enum.terminal.pwsh': 'PowerShell Core',
  'settings.enum.terminal.wt': 'Windows Terminal',
  'settings.enum.update.default': '自动检查',
  'settings.enum.update.manual': '手动',
  'settings.enum.update.none': '从不检查',
  'settings.enum.update.start': '启动时检查',
  'settings.enum.wordWrap.bounded': '视口与列宽较小值',
  'settings.enum.wordWrap.off': '关闭',
  'settings.enum.wordWrap.on': '开启',
  'settings.enum.wordWrap.wordWrapColumn': '按列宽换行',
  'settings.enum.zh-CN': '简体中文',
  'settings.explorer': '资源管理器',
  'settings.explorer.autoReveal.description':
    '控制资源管理器是否在打开文件时自动定位并选中该文件。',
  'settings.files.enableTrash': '删除文件/文件夹时移到系统回收站（废纸篓）。关闭后删除将无法恢复。',
  'settings.explorer.confirmDelete': '从资源管理器删除文件或文件夹前弹出确认。',
  'settings.explorer.enableUndo':
    '为资源管理器的文件操作（新建、重命名、移动、复制、删除）启用撤销/重做（Ctrl+Z / Ctrl+Y）。',
  'trash.recycleBin': '回收站',
  'trash.trash': '废纸篓',
  'settings.files': '文件',
  'settings.files.autoSave.description': '控制未保存文件的自动保存行为。',
  'settings.files.autoSaveDelay.description':
    '延迟后自动保存的等待时长（毫秒），当 autoSave 设为 afterDelay 时生效。',
  'settings.files.exclude.description':
    '配置要从资源管理器中排除的文件和文件夹 glob 模式。也会读取 .vscode/settings.json。',
  'settings.files.watcherExclude.description': '配置要从文件监听中排除的文件路径 glob 模式。',
  'settings.gear.copyId': '复制设置 ID',
  'settings.gear.copyJson': '复制设置为 JSON',
  'settings.gear.label': '更多操作',
  'settings.gear.reset': '重置设置',
  'settings.group.agents': 'Agents',
  'settings.group.ai': 'AI',
  'settings.js/ts.tsserver.maxMemory':
    'TypeScript 服务器可用的最大内存（单位 MB）。在大型项目中若 TypeScript 语言服务因内存不足而崩溃（会弹出崩溃通知），可调大此值。服务器（重新）启动时生效。',
  'settings.js/ts.experimental.useTsgo':
    '使用实验性的 Go 原生 TypeScript 语言服务器（tsgo），替代内置的 typescript-language-server（驱动 JS tsserver）。该设置在扩展宿主启动时由主进程读取，修改后需重启窗口才生效。',
  'settings.js/ts.prewarm.projects':
    '要提前预热的工作区相对 tsconfig 路径，使对应 TypeScript 项目的符号在打开文件前即可搜索。单 tsconfig 项目会自动预热；多 tsconfig 工作区除非列在此处，否则不会预热任何项目。',
  'settings.language.description': '控制界面显示语言。更改后需要重启应用。',
  'settings.nav': '设置分类',
  'settings.noWorkspace': '打开文件夹后才能编辑工作区设置。',
  'settings.origin.default': '默认',
  'settings.origin.memory': '运行时',
  'settings.origin.user': '用户',
  'settings.origin.vscodeUser': 'VSCode 用户',
  'settings.origin.vscodeWorkspace': 'VSCode 工作区',
  'settings.origin.workspace': '工作区',
  'settings.output': '输出',
  'settings.output.fontFamily.description': '控制输出底栏的字体。',
  'settings.output.fontSize.description': '控制输出底栏的字体大小（像素）。',
  'settings.performance': '性能',
  'settings.performance.startupWarning.developmentThresholdMs.description':
    '仅当开发构建启动超过此毫秒数时显示启动警告。',
  'settings.performance.startupWarning.enabled.description': '控制是否在状态栏显示慢启动警告。',
  'settings.performance.startupWarning.releaseThresholdMs.description':
    '仅当发布构建启动超过此毫秒数时显示启动警告。',
  'settings.performance.responsiveness.enabled.description':
    '持续监控交互响应（键入、点击、切标签页），交互变慢时在窗口日志中留下可归因的记录。开销接近零。',
  'settings.performance.responsiveness.warnThresholdMs.description':
    '交互耗时超过此毫秒数时记录警告日志。',
  'settings.performance.responsiveness.statusWarning.enabled.description':
    '当慢交互频繁发生时在状态栏显示警告。',
  'performance.responsiveness.statusbar.tooltip': '最近一分钟发生 {count} 次慢交互——点击查看报告',
  'settings.readonly': '在表单视图中不可编辑',
  'settings.search': '搜索',
  'settings.search.exclude.description':
    '配置搜索和快速打开时要排除的文件和文件夹 glob 模式。继承 files.exclude。',
  'settings.search.threads.description': '全局搜索使用的线程数。0 表示自动（CPU 内核数减 2）。',
  'settings.search.useIgnoreFiles.description':
    '搜索文本和文件名时是否遵循 .gitignore 与 .ignore 文件。关闭后会连同被忽略的文件一起搜索，例如构建产物和 .git 目录。',
  'settings.search.searchOnType.description':
    '输入时即搜索全部文件。关闭后仅在按下 Enter 时执行搜索。',
  'settings.search.searchOnTypeDebouncePeriod.description':
    '停止输入后等待多久（毫秒）再执行搜索。几乎匹配一切的正则表达式会按比例延长等待时间。仅在启用「输入时搜索」时生效。',
  'settings.search.collapseResults.description':
    '搜索结果树是否默认展开文件。折叠可让超大结果集保持流畅。',
  'settings.search.collapseResults.auto': '匹配少于 10 条的文件展开，其余折叠。',
  'settings.search.collapseResults.alwaysCollapse': '折叠所有文件。',
  'settings.search.collapseResults.alwaysExpand': '展开所有文件。',
  'settings.search.placeholder': '搜索设置（{count}）',
  'settings.swarm.notifications.enabled':
    '当有新审核进入「需我处理」列表且编辑器窗口未聚焦时，显示系统桌面通知。点击通知会打开该审核。',
  'settings.tab.user': '用户',
  'settings.tab.workspace': '工作区',
  'settings.terminal': '终端',
  'settings.terminal.external.windowsExec.description':
    '运行"在外部终端中打开"时，要在 Windows 上启动的终端。',
  'settings.terminal.integrated.cwd.description':
    '新建集成终端的默认工作目录。支持 ${workspaceFolder}、${userHome}、${env:NAME} 等变量。留空则使用工作区根目录。',
  'settings.terminal.integrated.fontFamily.description': '控制终端字体。',
  'settings.terminal.integrated.fontSize.description': '控制终端字体大小（像素）。',
  'settings.terminal.integrated.scrollback.description':
    '终端滚动缓冲（历史）中保留的最大行数。切回终端编辑器时会恢复。设为 0 表示无限制。',
  'settings.terminal.integrated.profiles.windows.description':
    'Windows 上的终端 profile，例如 { "Git Bash": null, "My Shell": { "path": "C:\\\\bin\\\\sh.exe", "args": ["-l"] } }。值为 null 表示删除同名自动检测 profile；对象只覆盖其声明的字段。',
  'settings.terminal.integrated.profiles.osx.description':
    'macOS 上的终端 profile。值为 null 表示删除同名自动检测 profile；对象只覆盖其声明的字段。',
  'settings.terminal.integrated.profiles.linux.description':
    'Linux 上的终端 profile。值为 null 表示删除同名自动检测 profile；对象只覆盖其声明的字段。',
  'settings.terminal.integrated.defaultProfile.windows.description':
    'Windows 上默认终端 profile 的名称（来自 profile 列表）。留空则使用 PowerShell。',
  'settings.terminal.integrated.defaultProfile.osx.description':
    'macOS 上默认终端 profile 的名称（来自 profile 列表）。留空则使用登录 shell。',
  'settings.terminal.integrated.defaultProfile.linux.description':
    'Linux 上默认终端 profile 的名称（来自 profile 列表）。留空则使用登录 shell。',
  'settings.terminal.integrated.useWslProfiles.description':
    '控制是否在终端 profile 列表中显示 WSL 发行版（仅 Windows）。',
  'settings.title': '设置',
  'settings.title.workbench': '工作台',
  'settings.update': '更新',
  'settings.update.checkIntervalMinutes.description':
    '运行时检查更新的频率（分钟）。0 表示禁用周期检查。',
  'settings.update.mode.description':
    '控制编辑器是否自动检查更新。none：从不；manual：仅通过命令；start：启动后不久检查一次；default：启动时检查并周期检查。',
  'settings.workbench.chat.virtualizationThreshold.description':
    'Agent 会话时间线项超过此数量时启用虚拟滚动。',
  'settings.workbench.colorTheme.description': '工作台颜色主题。',
  'settings.workbench.fontFamily.description': '控制工作台 UI 字体。',
  'settings.workbench.tree.virtualizationThreshold.description':
    '可见树项超过此数量时启用虚拟滚动。',
  'settings.acp.deepLink.allowAutoSubmit':
    '允许 `universe-editor://agent/new?prompt=…` 深链接自动发送提示词。默认启用；关闭后链接只会填充输入框，便于你先检查再发送。链接可用 `autoSubmit=false` 强制只填充。',
  'settings.acp.prompt.confirmShortFirstMessageLength':
    '会话中第一条消息的最小字符数。若消息更短，发送前会显示确认对话框。设为 0 可禁用。',
  'settings.acp.prompt.historyMaxEntries':
    '全局保存的提示词历史条数上限（所有工作区共享）。达到上限后会丢弃较早的条目。',
  'settings.acp.prompt.image.maxCount': '单条提示词可附加的图片数量上限。',
  'settings.acp.prompt.image.maxSizeMB':
    '粘贴、拖放或附加到提示词的单张图片大小上限（MB）。超过的图片会被拒绝。',
  'settings.acp.sessions.confirmDelete': '删除会话前是否请求确认。',
  'settings.acp.sessions.historyScope.all': '全部',
  'settings.acp.sessions.historyScope.description':
    '控制 Agent 会话历史中列出哪些会话。"当前工作树"包含同一仓库的兄弟 git 工作树中的会话；"全部"列出所有项目的会话。',
  'settings.acp.sessions.historyScope.workspace': '当前工作区',
  'settings.acp.sessions.historyScope.worktree': '当前工作树',
  'settings.acp.turnStallTimeoutMs':
    '运行中的回合超过此时长（毫秒）没有任何输出即判定为卡死：会结束 agent 进程并让会话热重连，恢复被中断的回合。设为 0 可关闭此看门狗。等待你回答问题或批准权限的会话不受影响。',
  'settings.editor.codeActionsOnSave.description':
    '保存时要运行的代码操作，按操作类型（如 "source.organizeImports"、"source.fixAll"）配置。"always" 表示每次保存都运行，"explicit" 表示仅在显式保存时运行，"never" 表示禁用该类型。',
  'settings.editor.semanticHighlighting.description': '控制对支持该特性的语言是否显示语义高亮。',
  'settings.files.nativeDialog.enable.description':
    '打开文件/文件夹及保存时使用操作系统文件对话框，而不是内置对话框。',
  'settings.issueReporter': '问题报告器',
  'settings.issueReporter.provider.description':
    'Report Issue 命令提交到的目标。tracker 支持附加诊断包；GitHub 只会预填问题正文。',
  'settings.issueReporter.tracker.appUrl.description':
    'tracker Web 应用的基础 URL。Report Issue 会打开其预填好的 addPost 页面。',
  'settings.issueReporter.tracker.board.description':
    '问题报告帖子预填到哪个 tracker 看板（名称或 ID）。',
  'settings.issueReporter.tracker.category.description': '问题报告帖子预填到哪个 tracker 分类。',
  'settings.issueReporter.tracker.serverUrl.description':
    '带附件报告问题时，诊断包上传到哪个 tracker 文件服务器。',
  'settings.languageServices': '语言服务',
  'settings.languageServices.prewarm':
    '工作区空闲后预热的语言 id，使打开该语言的首个文件时没有启动延迟。设为 [] 可禁用。',
  'settings.markdown': 'Markdown',
  'settings.markdown.preview.renderYamlFrontmatter':
    '在 markdown 预览中将 YAML frontmatter（文件顶部的 `---` 块）渲染为表格。关闭时 frontmatter 会被隐藏。',
  'settings.scm': '源代码管理',
  'settings.scm.blame.editorDecoration.disableHover':
    '禁用指向行内 blame 标注时显示完整提交信息的悬浮提示。',
  'settings.scm.blame.editorDecoration.enabled': '在行尾显示光标所在行的 blame 信息。',
  'settings.scm.blame.editorDecoration.template':
    '行内 blame 标注的模板。可用占位符：${TEMPLATE_TOKENS}。',
  'settings.scm.blame.ignoreWhitespace': '计算 blame 信息时忽略空白变更。',
  'settings.scm.blame.statusBarItem.enabled': '在状态栏中显示光标所在行的 blame 信息。',
  'settings.scm.blame.statusBarItem.template':
    'blame 信息状态栏项的模板。可用占位符：${TEMPLATE_TOKENS}。',
  'settings.scm.diffDecorations': '控制编辑器中的差异装饰。',
  'settings.scm.diffDecorations.all': '在所有可用位置显示差异装饰。',
  'settings.scm.diffDecorations.gutter': '仅在编辑器装订线中显示差异装饰。',
  'settings.scm.diffDecorations.minimap': '仅在小地图中显示差异装饰。',
  'settings.scm.diffDecorations.none': '不显示差异装饰。',
  'settings.scm.diffDecorations.overview': '仅在概览标尺中显示差异装饰。',
  'settings.scm.ignoreFiles.searchCeiling':
    '在过滤推测的会话更改时，编辑器从文件所在目录向上查找 Perforce 忽略文件（.p4ignore / p4ignore.txt）的范围。仅对 Perforce 工作区生效。',
  'settings.scm.ignoreFiles.searchCeiling.filesystem':
    '一直向上查找到 Perforce client root；若该路径不属于任何 client，则查找到文件系统根目录。',
  'settings.scm.ignoreFiles.searchCeiling.workspace': '不查找到打开的文件夹之上。',
  'settings.scm.mergeEditor':
    '在三方合并编辑器中打开有冲突的文件。设为 false 时，冲突以工作区差异的形式打开，并通过冲突标记就地解决。',
  'settings.semanticHighlighting.configuredByTheme': '由主题配置',
  'settings.semanticHighlighting.configuredByTheme.description':
    '语义高亮由当前颜色主题的 `semanticHighlighting` 设置配置。',
  'settings.semanticHighlighting.false': '已禁用',
  'settings.semanticHighlighting.false.description': '对所有颜色主题禁用语义高亮。',
  'settings.semanticHighlighting.true': '已启用',
  'settings.semanticHighlighting.true.description': '对所有颜色主题启用语义高亮。',
  'settings.swarm.authoredHideApproved': '在"我发起的"分组中隐藏已批准的审核。',
  'settings.swarm.autoRefresh.enabled':
    '每分钟自动刷新已打开的 Swarm 审核（详情、评论、文件列表）。标题栏按钮的手动刷新始终可用。',
  'settings.swarm.inlineComments.enabled':
    '在 Swarm 审核差异上叠加 GitHub PR 风格的行内评论。关闭时差异只显示代码。默认关闭。',
  'settings.swarm.needsActionApprovableOnly': '在"需要我处理"中只显示你当前可以批准的审核。',
  'settings.swarm.needsActionAuthors':
    '在"需要我处理"中只显示作者在此集合内的审核。留空表示显示所有作者。',
  'settings.swarm.reviewsView': 'Swarm 审核',
  'settings.telemetry': '遥测',
  'settings.telemetry.errorCollection.enabled.description':
    '将结构化错误记录（指纹、脱敏堆栈、出现次数）收集到本地 errors.jsonl 日志以便诊断。不会通过网络发送任何内容。',
  'settings.timeline': '时间线',
  'settings.timeline.pageSize': '时间线视图中每页显示的条目数。',
  'settings.typescript': 'TypeScript',
  'settings.window': '窗口',
  'settings.window.autoDetectColorScheme.description':
    '启用后，将根据系统颜色模式自动选择颜色主题。若系统为深色模式，使用 "workbench.preferredDarkColorTheme"，否则使用 "workbench.preferredLightColorTheme"。',
  'settings.window.zoomLevel':
    '整个窗口 UI 的缩放系数。`1` 为 100%，`1.25` 为 125%。在操作系统缩放不会传递给应用的平台上（例如 WSLg），用它来匹配宿主显示缩放。Ctrl+= / Ctrl+- 会为当前会话临时调整缩放；本设置是启动时恢复的值。',
  'settings.workbench.colorCustomizations.description':
    '覆盖当前颜色主题中的颜色。键为颜色 id（如 "editor.background"），值为颜色字符串，或 "default" 以恢复主题值。嵌套在 "[主题名称]" 下可将覆盖限定到某个主题。',
  'settings.workbench.iconTheme.description':
    '指定工作台使用的文件图标主题。`null` 选择内置的 Universe Material 图标。',
  'settings.workbench.preferredDarkColorTheme.description':
    '指定系统颜色模式为深色且启用 "window.autoDetectColorScheme" 时使用的颜色主题。',
  'settings.workbench.preferredLightColorTheme.description':
    '指定系统颜色模式为浅色且启用 "window.autoDetectColorScheme" 时使用的颜色主题。',
  'settings.workbench.productIconTheme.description': '指定工作台使用的产品图标主题。',
  'settings.workspace': '聚焦',
  'settings.workspace.focusEnabled.description':
    '将编辑器限制为 `workspace.focusFolders` 中列出的子文件夹。它会收窄资源管理器、搜索、快速打开和文件监视范围——在超大工作区中，这是逐文件夹排除噪声之外的一种白名单式替代方案。',
  'settings.workspace.focusFolders.description':
    '要聚焦的工作区相对文件夹或单个文件，例如 `{ "Client": true, "Source/Client/Run.bat": true }`。这里填的是路径而非 glob 模式——它们会直接交给文件扫描器与监视器；命名文件的条目会被当作文件处理（直接监视、纳入搜索，并由 Perforce 逐文件协调）。将条目设为 false 可取消从较低设置层继承的条目。仅在开启 `workspace.focusEnabled` 时生效。',
  'settings.workspace.focusShowRootFiles.description':
    '聚焦模式开启时，保持直接位于工作区根目录下的文件可见。指向聚焦文件夹的路径上的文件夹始终显示，但它们自身的文件会被隐藏。',

  // --- Explorer ---
  'explorer.collapseAll': '全部折叠',
  'explorer.context.delete': '删除',
  'explorer.context.newFile': '新建文件',
  'explorer.context.newFolder': '新建文件夹',
  'explorer.context.openWithDefaultApplication': '使用默认应用打开',
  'explorer.context.refresh': '刷新',
  'explorer.context.rename': '重命名',
  'explorer.context.revealInFileExplorer': '在文件管理器中显示',
  'explorer.empty.noFolder': '你尚未打开文件夹。',
  'explorer.newFile': '新建文件...',
  'explorer.newFolder': '新建文件夹...',
  'explorer.refresh': '刷新资源管理器',
  'explorer.symbolicLink': '符号链接',

  // --- File dialog ---
  'fileDialog.create': '创建',
  'fileDialog.createFile': '文件"{name}"不存在。要创建它吗？',
  'fileDialog.createFolder': '文件夹"{name}"不存在。要创建它吗？',
  'fileDialog.hideHidden': '隐藏隐藏文件',
  'fileDialog.ok': '确定',
  'fileDialog.open': '打开',
  'fileDialog.openFile.title': '打开文件',
  'fileDialog.openFolder.title': '打开文件夹',
  'fileDialog.openFolderButton': '打开',
  'fileDialog.overwrite': '名为"{name}"的文件已存在。要替换它吗？',
  'fileDialog.replace': '替换',
  'fileDialog.save': '保存',
  'fileDialog.saveAs.title': '另存为',
  'fileDialog.showHidden': '显示隐藏文件',

  // --- Search ---
  'search.clear': '清除搜索结果',
  'search.collapseAll': '全部折叠',
  'search.filesToExclude': '要排除的文件',
  'search.filesToInclude': '要包含的文件',
  'search.input.placeholder': '搜索',
  'search.matchCase': '区分大小写',
  'search.matchCount': '{count} 个匹配项',
  'search.matchWholeWord': '全字匹配',
  'search.refresh': '刷新',
  'search.replace': '替换',
  'search.replace.placeholder': '替换',
  'search.replaceAllInFile': '替换文件中的所有匹配项',
  'search.replaceAllInNamedFile': '替换 {name} 中的所有匹配项',
  'search.replaceMatchAtLine': '替换第 {line} 行的匹配项',
  'search.results': '搜索结果',
  'search.toggleDetails': '切换搜索详情',
  'search.toggleNode': '切换 {name}',
  'search.toggleReplace': '切换替换',
  'search.useExcludeSettings': '使用排除设置和忽略文件',
  'search.useRegex': '使用正则表达式',
  'search.viewAsList': '以列表查看',
  'search.viewAsTree': '以树查看',
  'search.menu.copy': '复制',
  'search.menu.copyAll': '全部复制',
  'search.menu.copyPath': '复制路径',
  'search.menu.remove': '移除',
  'search.staleBanner': '结果可能已过期',
  'search.rerun': '重新搜索',
  'search.searching': '搜索中…',
  'search.searchingProgress': '搜索中… {matched}/{scanned} 文件',
  'search.noResults': '未找到结果',
  'search.summary': '{matches} 个匹配，分布在 {files} 个文件',
  'search.replaceAll': '全部替换',
  'search.replaceAll.confirm': '在 {files} 个文件中替换 {changes} 处。继续？',
  'search.replaceAll.replace': '替换',
  'search.failed': '搜索失败',
  'search.statusSearching': '$(search) 搜索中…',
  'search.statusProgress': '$(search) 搜索中… {matched}/{scanned} 文件，{matches} 匹配',

  // --- Outline ---
  'outline.collapse': '折叠',
  'outline.collapseAll': '全部折叠',
  'outline.collapseSubtree': '折叠子树',
  'outline.empty': '未找到符号。',
  'outline.expand': '展开',
  'outline.expandAll': '全部展开',
  'outline.expandSubtree': '展开子树',
  'outline.filterOnType': '按类型过滤',
  'outline.filterPlaceholder': '过滤',
  'outline.followCursor': '跟随光标',
  'outline.goTo': '跳转',
  'outline.goToDefinition': '转到定义',
  'outline.goToImplementation': '转到实现',
  'outline.goToReferences': '转到引用',
  'outline.goToSymbol': '跳转到符号',
  'outline.goToTypeDefinition': '转到类型定义',
  'outline.label': '大纲',
  'outline.moreActions': '更多操作...',
  'outline.noMatches': '没有匹配的符号。',
  'outline.sortByKind': '排序方式：类别',
  'outline.sortByName': '排序方式：名称',
  'outline.sortByPosition': '排序方式：位置',

  // --- SCM ---
  'commitChanges.empty': '从 Blame、时间线或图谱中选择一个提交以查看其变更。',
  'commitChanges.expandAll': '全部展开',
  'commitChanges.openInGraph': '在图谱中显示',
  'commitChanges.treeLabel': '变更的文件',
  'scm.collapseAll': '全部折叠',
  'scm.expandAll': '全部展开',
  'scm.commit': '提交',
  'scm.commitActions': '提交操作...',
  'scm.empty': '没有注册源代码管理提供方。',
  'scm.moreActions': '更多操作...',
  'scm.openFile': '打开文件',
  'scm.viewAsList': '以列表查看',
  'scm.viewAsTree': '以树查看',
  'scm.commitAmend': '提交（修正）',
  'scm.commitAndPush': '提交并推送',
  'scm.commitAndSync': '提交并同步',
  'scm.unstageChanges': '取消暂存更改',
  'scm.discardChanges': '放弃更改',
  'scm.stageChanges': '暂存更改',
  'scm.blame.notCommittedYet': '尚未提交',
  'scm.blame.openCommitTooltip': '打开提交',
  'scm.blame.openInGraph': '在图谱中显示',
  'scm.blame.openInGraphTooltip': '在图谱中显示',
  'scm.blame.statusBarTooltip': '追溯（Blame）',
  'dirtyDiff.peek.close': '关闭',
  'dirtyDiff.peek.next': '下一处更改',
  'dirtyDiff.peek.openChanges': '打开更改',
  'dirtyDiff.peek.previous': '上一处更改',
  'dirtyDiff.peek.revert': '还原更改',
  'dirtyDiff.peek.stage': '暂存更改',
  'dirtyDiff.peek.title': '第 {0} / {1} 处更改',
  'dirtyDiff.peek.titleOne': '1 处更改',

  // --- Git graph ---
  'gitGraph.checkout': '检出',
  'gitGraph.checkoutCommit': '检出此提交…',
  'gitGraph.checkoutCommit.confirm': '检出提交 {hash}？',
  'gitGraph.checkoutCommit.detail': '这会让 HEAD 进入分离状态。',
  'gitGraph.checkoutLocalBranch': '检出为本地分支…',
  'gitGraph.cherryPick': '拣选…',
  'gitGraph.cherryPickToBranch': '拣选到分支…',
  'gitGraph.cherryPickToBranch.title': '将 {hash} 拣选到分支',
  'gitGraph.cherryPickToBranch.noBranches': '没有可用的分支。',
  'gitGraph.cherryPickToBranch.noTargets': '没有其它可拣选的分支。',
  'gitGraph.commitCount': '{count} 次提交{more}',
  'gitGraph.comparing': '正在比较 {left} ↔ {right}',
  'gitGraph.copyBranchName': '复制分支名字',
  'gitGraph.copyHash': '复制提交哈希',
  'gitGraph.copyMessage': '复制提交信息',
  'gitGraph.createBranchHere': '在此处创建分支…',
  'gitGraph.createTagHere': '在此处创建标签…',
  'gitGraph.deleteBranch.confirm': '删除分支"{name}"？',
  'gitGraph.deleteBranch.detail': '强制删除会移除此分支，即使它尚未完全合并。',
  'gitGraph.deleteRemoteBranch': '删除远程分支…',
  'gitGraph.deleteRemoteBranch.confirm': '删除远程分支"{name}"？',
  'gitGraph.deleteRemoteBranch.detail': '这会从远程服务器删除该分支。',
  'gitGraph.deleteTag': '删除标签…',
  'gitGraph.deleteTag.confirm': '删除标签"{name}"？',
  'gitGraph.forceDelete': '强制删除',
  'gitGraph.forcePush.button': '强制推送',
  'gitGraph.forcePush.confirm': '强制推送"{name}"到 origin？',
  'gitGraph.forcePush.detail': '这会覆盖远程分支历史，并可能丢弃他人的提交。',
  'gitGraph.header.author': '作者',
  'gitGraph.header.commit': '提交',
  'gitGraph.header.date': '日期',
  'gitGraph.header.description': '描述',
  'gitGraph.hideRemoteBranches': '隐藏远程分支',
  'gitGraph.loadMore': '加载更多提交',
  'gitGraph.localBranchName': '本地分支名称',
  'gitGraph.mergeCurrent': '合并到当前分支…',
  'gitGraph.newBranchName': '新分支名称',
  'gitGraph.newTagName': '新标签名称',
  'gitGraph.noCommitDetails': '没有提交详情。',
  'gitGraph.noFileChanges': '没有文件更改。',
  'gitGraph.openCommitChanges': '打开提交更改',
  'gitGraph.openFile': '打开文件',
  'gitGraph.openFile.failed': '无法打开 {path}：{error}',
  'gitGraph.openFile.noRepository': '无法打开 {path}：当前未选中 Git 仓库。',
  'gitGraph.openFile.notFound':
    '无法打开 {path}：该文件不在当前工作树中，可能已被删除、重命名，或只存在于所选提交里。',
  'gitGraph.noUncommittedChanges': '没有未提交的更改。',
  'gitGraph.onBranch': ' · 位于 {branch}',
  'gitGraph.onlyFirstParent': '仅跟随第一父提交',
  'gitGraph.order': '顺序',
  'gitGraph.order.authorDate': '作者日期',
  'gitGraph.order.date': '日期',
  'gitGraph.order.topology': '拓扑',
  'gitGraph.parents': '父提交：',
  'gitGraph.pull': '拉取',
  'gitGraph.pullAutostash': '拉取（自动储藏）',
  'gitGraph.pullRebase': '拉取（变基）',
  'gitGraph.push': '推送…',
  'gitGraph.pushForce': '推送（强制）…',
  'gitGraph.pushTag': '推送标签…',
  'gitGraph.rebaseCurrentBranch': '将当前分支变基到此分支…',
  'gitGraph.rebaseCurrentCommit': '将当前分支变基到此提交…',
  'gitGraph.rename': '重命名…',
  'gitGraph.renameBranch': '重命名分支',
  'gitGraph.repository': '仓库',
  'gitGraph.reset.hard': 'Hard',
  'gitGraph.reset.mixed': 'Mixed',
  'gitGraph.resetCurrentCommit': '将当前分支重置到此提交…',
  'gitGraph.resetCurrentCommit.confirm': '将当前分支重置到 {hash}？',
  'gitGraph.resetCurrentCommit.detail': 'Mixed 会保留你的更改但不暂存。Hard 会丢弃所有工作区更改。',
  'gitGraph.revert': '还原…',
  'gitGraph.search.placeholder': '搜索提交…',
  'gitGraph.showRemoteBranches': '显示远程分支',
  'gitGraph.stash.apply': '应用 stash…',
  'gitGraph.stash.drop': '丢弃 stash…',
  'gitGraph.stash.dropButton': '丢弃',
  'gitGraph.stash.dropConfirm': '丢弃 {selector}？',
  'gitGraph.stash.dropDetail': '已 stash 的更改将会丢失。',
  'gitGraph.stash.pop': '弹出 stash…',
  'gitGraph.status.added': '已添加',
  'gitGraph.status.copied': '已复制',
  'gitGraph.status.deleted': '已删除',
  'gitGraph.status.modified': '已修改',
  'gitGraph.status.renamed': '已重命名',
  'gitGraph.status.typeChanged': '类型已更改',
  'gitGraph.status.unmerged': '未合并',
  'gitGraph.title': 'Git 图谱',
  'gitGraph.unavailable': 'Git 图谱不可用 - 此文件夹是 git 仓库吗？',
  'gitGraph.uncommittedChanges': '未提交的更改',
  'gitGraph.uncommittedCount': '未提交的更改（{count}）',
  'gitGraph.viewSettings': '查看设置',
  'gitGraph.worktree.tooltip': '工作树 {name} · {branch}\n{path}',
  'gitGraph.worktree.tooltipDetached': '工作树 {name}（已分离）\n{path}',
  'gitGraph.worktree.open': '打开工作树',
  'gitGraph.worktree.openNewWindow': '在新窗口打开工作树',
  'gitGraph.worktree.copyPath': '复制工作树路径',
  'gitGraph.worktree.delete': '删除工作树…',
  'gitGraph.worktree.deleteConfirm': '删除工作树"{name}"？',
  'gitGraph.worktree.deleteDetail': '这会移除位于 {path} 的工作树文件夹，但保留其分支本身。',
  'gitGraph.worktree.syncToThis': '将工作树同步到 {branch}…',
  'gitGraph.worktree.forceSyncToThis': '强制将工作树同步到 {branch}…',
  'gitGraph.worktree.sync.title': '将工作树同步到 {branch}',
  'gitGraph.worktree.forceSync.title': '强制将工作树同步到 {branch}',
  'gitGraph.worktree.sync.selectAll': '全选',
  'gitGraph.worktree.sync.clearSelection': '取消全选',
  'gitGraph.worktree.sync.confirm': '同步（{count}）',
  'gitGraph.worktree.forceSync.confirm': '强制同步（{count}）',
  'gitGraph.worktree.sync.progress': '正在将 {count} 个工作树同步到 {branch}…',
  'gitGraph.worktree.sync.summaryTitle': '工作树同步到 {branch}',
  'gitGraph.worktree.sync.summarySynced': '已同步：{names}',
  'gitGraph.worktree.sync.summarySkipped': '已跳过（有未提交更改）：{names}',
  'gitGraph.worktree.sync.summaryUnmerged': '已跳过（存在不在 {branch} 中的提交）：{names}',
  'gitGraph.worktree.sync.summaryUnmatched': '已跳过（在 {branch} 中找不到相同提交消息）：{names}',
  'gitGraph.worktree.sync.summaryFailed': '失败：{items}',
  'gitGraph.worktree.sync.summaryNone': '无可执行的操作。',
  'gitGraph.checkoutRemote.exists': '分支"{name}"在本地已存在。',
  'gitGraph.checkoutRemote.offerReset': '你可以改为将它重置到"{remote}"的最新提交。',
  'gitGraph.checkoutRemote.resetToRemote': '重置到远程…',
  'gitGraph.commitList': '提交',
  'gitGraph.pickMenuContext': '为哪个目标显示上下文菜单？',
  'gitGraph.ref.branch': '分支 {name}',
  'gitGraph.ref.commit': '提交 {hash}',
  'gitGraph.ref.remote': '远程 {name}',
  'gitGraph.ref.tag': '标签 {name}',
  'gitGraph.ref.worktree': '工作树 {name}',
  'gitGraph.resetToRemote.button': '重置',
  'gitGraph.resetToRemote.confirm': '将"{name}"重置到"{remote}"？',
  'gitGraph.resetToRemote.detail': '不在远程上的本地提交将被丢弃。',
  'gitGraph.sendToAgentChat': '发送到 Agent 聊天',

  // --- Perforce Graph ---
  'perforceGraph.title': 'Perforce Graph',
  'perforceGraph.changeCount': '{count} 个变更{more}',
  'perforceGraph.changeList': '变更',
  'perforceGraph.onClient': ' · {client}',
  'perforceGraph.haveBadge': '已同步',
  'perforceGraph.haveBadge.tooltip':
    '图谱标记的是本地已同步到的最新变更；比本行更新的变更尚未同步（来源与生效时间见工具栏的“已同步至”）。',
  'perforceGraph.haveBadge.coveredTooltip':
    '同步点 #{point} 没有修改这个范围，所以这里没有它那一行；本行是该范围内被这次同步覆盖到的最新变更，比它更新的变更尚未同步。',
  'perforceGraph.syncedTo': ' · 已同步至 ',
  'perforceGraph.syncedToUnknown': '#?（点击查询）',
  'perforceGraph.syncPoint.query': '查询同步点',
  'perforceGraph.syncPoint.queryTooltip':
    '尚不知道工作区拉取到哪里。点击向 Perforce 查询——范围大时可能要等一会儿。',
  'perforceGraph.syncPoint.queryButton':
    '向 Perforce 查询工作区拉取到哪里。既用于「尚不知道」时，也用于重新核对一条较早的记录——范围大时可能要等一会儿。',
  'perforceGraph.syncPoint.querying': '正在查询同步点…',
  'perforceGraph.syncPoint.alreadyQuerying':
    '已经有一次同步点查询在进行中，Perforce 答复后就会更新。',
  'perforceGraph.syncPoint.answeredEmpty':
    'Perforce 答复：该范围里目前没有任何已同步的文件。点击可再查一次。',
  'perforceGraph.syncPoint.failed':
    '这次查询没有拿到答案（Perforce 失败或超时），同步点保持不变；详情见 Perforce 输出面板。',
  'perforceGraph.syncPoint.reveal': '跳转到同步点',
  'perforceGraph.syncPoint.landedBelow':
    '同步点 #{point} 没有修改这个范围，所以这里没有它那一行；已跳到 #{row}——本范围内被这次同步覆盖到的最新变更。',
  'perforceGraph.syncPoint.notInScope':
    '同步点 #{point} 没有修改这个范围，而这个范围里的变更全都比它新——这里的变更都还没拉下来。',
  'perforceGraph.reveal.missing': '#{id} 不在当前范围的历史里（它可能没有修改这里的内容）。',
  'perforceGraph.syncPoint.fromQuery': 'Perforce 于 {time} 答复。比它更新的变更尚未同步。',
  'perforceGraph.syncPoint.fromSync':
    '编辑器于 {time} 记录（那次拉取的就是这个 changelist）。此后在编辑器之外做的同步不会被反映——可用「查询同步点」重新查询。',
  'perforceGraph.syncPoint.fromExternal':
    '本机另一个同步工具于 {time} 写下的记录。本编辑器无法核实该范围实际拉到了哪里——可用「查询同步点」向 Perforce 核对。',
  'perforceGraph.syncPoint.widerScope':
    '该记录来自更大的范围，所以这只是上界：当前范围实际可能拉得更少。',
  'perforceGraph.syncPoint.partial':
    '那次拉取跳过了部分文件（本地已修改、已打开或需要合并），所以这只是上界。',
  'perforceGraph.search.placeholder': '搜索变更…',
  'perforceGraph.client': '工作区',
  'perforceGraph.forceGet': '强制拉取（覆盖本地文件）',
  'perforceGraph.getLatestRevision': '获取最新修订',
  'perforceGraph.getRevision': '获取修订…',
  'perforceGraph.getThisRevision': '获取此修订',
  'perforceGraph.header.change': '变更',
  'perforceGraph.loadMore': '加载更多变更',
  'perforceGraph.pendingCount': '待定变更（{count}）',
  'perforceGraph.pendingChanges': '待定变更',
  'perforceGraph.noPendingChanges': '没有待定的变更。',
  'perforceGraph.noChangeDetails': '没有变更详情。',
  'perforceGraph.copyId': '复制 changelist 编号',
  'perforceGraph.openChangelistChanges': '打开 changelist 更改',
  'perforceGraph.openChanges': '打开更改',
  'perforceGraph.scope.showFolder': '仅显示当前文件夹的更改',
  'perforceGraph.scope.showWholeRepo': '显示整个仓库的更改',
  'perforceGraph.scope.toggle': '切换仓库范围',
  'perforceGraph.scopedEmpty': '没有影响此路径的已提交更改。',
  'perforceGraph.scopedTitle': '历史：{label}',
  'perforceGraph.sync.confirm': '获取修订（{count}）',
  'perforceGraph.sync.description': '选择要同步到该变更列表状态的文件夹。',
  'perforceGraph.sync.noFolders': '无法列出工作区文件夹。请改用“获取此修订”同步整个显示范围。',
  'perforceGraph.sync.selectAll': '全选',
  'perforceGraph.sync.title': '获取修订 @{change}',
  'perforceGraph.commitChanges.hiddenFiles': ' · 该变更列表中还有 {count} 个文件不在所选范围内',
  'perforceGraph.scopedMultiClientEmpty':
    '所选路径不在同一个 Perforce 工作区中，因此无法合并它们的历史。',
  'perforceGraph.unavailable': 'Perforce Graph 不可用——当前目录是否位于 Perforce 工作区内？',

  // --- Swarm ---
  'swarm.addComment': '添加评论…',
  'swarm.apply.checkbox': '同时替换工作区之外的文件',
  'swarm.apply.checkbox.changelist': '在默认变更列表中打开已应用的文件',
  'swarm.apply.confirm': '将审核 #{0} 应用到本地文件？',
  'swarm.apply.detail': '这将用审核版本替换 {0} 个文件的本地内容。',
  'swarm.apply.detail.outside': '工作区之外的 {0} 个文件将被跳过：{1}',
  'swarm.apply.detail.skippedNote': '已打开或已过期的文件将被跳过并报告。',
  'swarm.apply.detail.unmapped': '{0} 个文件属于不同的流/分支，无法应用。',
  'swarm.apply.done': '已将 {0} 个文件应用到工作区。',
  'swarm.apply.done.noChangelist': '已将 {0} 个文件应用到工作区（未在变更列表中打开）。',
  'swarm.apply.doneWithSkipped': '已应用 {0} 个文件；跳过 {1} 个：\n{2}{3}',
  'swarm.apply.keptOpen': '{0} 个文件无法从默认变更列表中移除，仍保持打开：\n{1}{2}',
  'swarm.apply.nothing': '没有可应用的内容：该版本没有任何映射文件位于工作区内。',
  'swarm.apply.nothing.mismatch': '无法应用该审核：其文件所属的流/分支与当前工作区不同。',
  'swarm.apply.nothingApplied': '没有应用任何文件。',
  'swarm.baseVersion': '（基线）',
  'swarm.commands.unavailable': 'Swarm 不可用。请检查 Perforce 扩展与连接。',
  'swarm.comment': '评论',
  'swarm.comments': '评论',
  'swarm.commit.confirm': '批准并提交审核 #{0}？',
  'swarm.commit.detail': '这会将搁置的变更提交到 depot。此操作无法撤销。',
  'swarm.compare': '比较',
  'swarm.diff.spreadsheetTooLarge': '该表格过大（{0} MB），无法以表格形式对比。',
  'swarm.empty': '未找到审核。',
  'swarm.files': '文件',
  'swarm.files.collapseAll': '全部折叠',
  'swarm.files.toggleFolder': '切换 {name}',
  'swarm.files.viewAsList': '以列表查看',
  'swarm.files.viewAsTree': '以树查看',
  'swarm.filter.allAuthors': '所有作者',
  'swarm.filter.approvable.off': '显示全部（不只是可批准的）',
  'swarm.filter.approvable.on': '只显示我可以批准的审核',
  'swarm.filter.authored.hideApproved': '隐藏我已批准的审核',
  'swarm.filter.authored.showApproved': '显示我已批准的审核',
  'swarm.filter.authors.placeholder': '切换要包含的作者，然后按确定（留空 = 所有作者）',
  'swarm.filter.authors.selected': '已包含',
  'swarm.filter.authors.title': '按作者筛选"需要我处理"',
  'swarm.filter.needsAction.placeholder': '配置"需要我处理"',
  'swarm.filter.needsAction.tooltip': '筛选"需要我处理"',
  'swarm.filter.placeholder': '筛选审核…',
  'swarm.filter.setAuthors': '设置作者筛选…',
  'swarm.group.authored': '我发起的',
  'swarm.group.ignored': '已忽略',
  'swarm.group.needsAction': '需要我处理',
  'swarm.ignore.button': '忽略',
  'swarm.inline.asTask': '标记为任务',
  'swarm.inline.cancel': '取消',
  'swarm.inline.comment': '在此行添加评论…',
  'swarm.inline.commentBtn': '评论',
  'swarm.inline.reply': '回复…',
  'swarm.inline.replyBtn': '回复',
  'swarm.loading': '正在加载…',
  'swarm.menu.copyLink': '复制审核链接',
  'swarm.menu.copyName': '复制审核名称',
  'swarm.menu.ignore': '忽略审核',
  'swarm.menu.open': '打开审核',
  'swarm.menu.openBrowser': '在浏览器中打开审核',
  'swarm.menu.unignore': '取消忽略审核',
  'swarm.noComments': '暂无评论。',
  'swarm.noDescription': '（无描述）',
  'swarm.noFiles': '该版本没有文件。',
  'swarm.notConfigured': 'Swarm 未配置。请设置 perforce.swarm.url。',
  'swarm.notify.needsAction.title': '有新的 Swarm 审核待你处理',
  'swarm.notify.needsAction.one': '审核 #{0}：{1}',
  'swarm.notify.needsAction.oneNoDesc': '审核 #{0}',
  'swarm.notify.needsAction.many': '有 {0} 个新审核待你处理',
  'swarm.notify.needsAction.inAppOne': '有新的 Swarm 审核 #{0} 待你处理：{1}',
  'swarm.notify.needsAction.inAppOneNoDesc': '有新的 Swarm 审核 #{0} 待你处理',
  'swarm.notify.needsAction.open': '打开审核',
  'swarm.notify.needsAction.openList': '打开 Swarm 审核列表',
  'swarm.obliterate.action': '彻底删除审核',
  'swarm.obliterate.button': '彻底删除审核',
  'swarm.obliterate.confirm': '彻底删除审核 #{0}？',
  'swarm.obliterate.confirmDetail': '这将永久丢弃该审核，且无法撤销。',
  'swarm.obliterate.detail': '这将永久丢弃该审核。此操作无法撤销。',
  'swarm.openById.invalid': '请输入数字形式的审核 ID。',
  'swarm.openById.placeholder': '审核 ID，例如 100801',
  'swarm.openById.prompt': '输入要打开的 Swarm 审核 ID',
  'swarm.review.refresh': '刷新审核',
  'swarm.review.unavailable': '审核 #{0} 不可用。',
  'swarm.reviews.treeLabel': 'Swarm 审核',
  'swarm.transition.confirm': '{0}审核 #{1}？',
  'swarm.transition.confirmDetail': '该审核操作可能无法撤销。',
  'swarm.unignore.button': '取消忽略',
  'swarm.updateReview': '更新审核',
  'swarm.vote.clear': '清除投票',
  'swarm.vote.down': '投反对票',
  'swarm.vote.up': '投赞成票',
  'swarm.workspaceNotPerforce': '不是 Perforce 工作区。',
  'swarmChanges.empty': '选择一个审核以查看其更改的文件。',
  'swarmChanges.noFiles': '该审核没有更改的文件。',
  'swarmChanges.reviewTitle': '审核 #{0}',
  'swarmChanges.treeLabel': '更改的文件',
  'swarmChanges.version': 'v{0}',

  // --- Diff editor & Merge ---
  'diffEditor.nextChange': '下一处更改 ({key})',
  'action.diffEditor.openFile.title': '打开源文件',
  'diffEditor.previousChange': '上一处更改 ({key})',
  'mergeConflict.acceptBoth': '保留双方更改',
  'mergeConflict.acceptCurrent': '采用当前更改',
  'mergeConflict.acceptIncoming': '采用传入更改',
  'mergeConflict.compare': '比较更改',
  'mergeConflict.compareTitle': '{name}（当前 ↔ 传入）',
  'mergeConflict.next.title': '转到下一处合并冲突',
  'mergeConflict.previous.title': '转到上一处合并冲突',
  'mergeEditor.allResolved': '所有冲突已解决',
  'mergeEditor.complete': '完成合并',
  'mergeEditor.current': '当前更改',
  'mergeEditor.incoming': '传入更改',
  'mergeEditor.loading': '正在加载编辑器…',
  'mergeEditor.result': '结果',
  'mergeEditor.unresolved': '{count} 处冲突待解决',
  'diff.workingTreeTitle': '{label}（工作区）',

  // --- Terminal ---
  'terminal.close': '关闭终端',
  'terminal.empty': '没有终端。',
  'terminal.noTerminals': '没有终端',
  'terminal.profiles.default': '（默认）',
  'terminal.profiles.empty': '未检测到可用的 shell profile',
  'terminal.profiles.loading': '正在检测 shell…',
  'terminal.selectInstance': '选择终端实例',
  'terminal.selectShell': '选择 Shell',

  // --- Output & Logs ---
  'logs.currentLevel': '当前',
  'logs.noneFound': '未找到日志文件。',
  'output.active': '活动',
  'output.autoScroll.turnOff': '关闭自动滚动',
  'output.autoScroll.turnOn': '开启自动滚动',
  'output.channel.kind.log': '日志通道',
  'output.channel.kind.output': '输出通道',
  'output.editorLabel': '输出',
  'output.filter.ariaLabel': '筛选输出',
  'output.filter.clear': '清除筛选',
  'output.filter.level.debug': '调试',
  'output.filter.level.error': '错误',
  'output.filter.level.info': '信息',
  'output.filter.level.trace': '跟踪',
  'output.filter.level.warning': '警告',
  'output.filter.levels': '按日志级别筛选',
  'output.filter.placeholder': '筛选（例如 text, !exclude）',
  'output.noChannels': '没有通道',
  'output.selectChannel': '选择输出通道',
  'output.empty': '没有输出。',
  'logFiles.error.invalidId': '无效的日志文件 id：{id}',
  'logLevel.trace': '跟踪',
  'logLevel.debug': '调试',
  'logLevel.info': '信息',
  'logLevel.warning': '警告',
  'logLevel.error': '错误',
  'logLevel.off': '关闭',

  // --- Keybindings ---
  'keybindings.column.command': '命令',
  'keybindings.column.keybinding': '键盘快捷方式',
  'keybindings.column.source': '来源',
  'keybindings.edit': '编辑键盘快捷方式',
  'keybindings.empty': '没有匹配的键盘快捷方式。',
  'keybindings.record.cancelHint': '按 Esc 取消',
  'keybindings.record.placeholder': '按下按键…',
  'keybindings.reset': '重置为默认值',
  'keybindings.schema.args.description': '传递给命令处理程序的参数。',
  'keybindings.schema.command.description': '要调用的命令 ID，前缀 "-" 可移除默认绑定。',
  'keybindings.schema.key.description': '按键绑定序列，例如 "ctrl+shift+p" 或 "ctrl+k ctrl+s"。',
  'keybindings.schema.removeBinding.desc': '移除默认绑定（{desc}）',
  'keybindings.schema.when.description': '控制此绑定生效的上下文键表达式。',
  'largeFileOptimizations.warning':
    '{file}：为了减少内存占用并避免卡顿或崩溃，已为此大文件关闭语法着色、自动换行、折叠、CodeLens、单词高亮和粘性滚动。',
  'largeFileOptimizations.forceEnable': '强制启用功能',
  'largeFileOptimizations.reopenPrompt': '请重新打开文件以使该设置生效。',
  'notification.neverShowAgain': '不再显示',
  'keybindings.search.placeholder': '按此输入以搜索快捷键',
  'keybindings.source.default': '默认',
  'keybindings.source.user': '用户',
  'keybindings.action.addKeybinding.title': '添加键盘快捷方式',
  'keybindings.action.clearSearchResults.title': '清除搜索结果',
  'keybindings.action.copyCommandId.title': '复制命令 ID',
  'keybindings.action.copyCommandTitle.title': '复制命令标题',
  'keybindings.action.copyKeybindingEntry.title': '复制',
  'keybindings.action.defineKeybinding.title': '更改键盘快捷方式',
  'keybindings.action.defineWhenExpression.title': '更改 when 表达式',
  'keybindings.action.focusKeybindings.title': '聚焦键盘快捷方式',
  'keybindings.action.recordSearchKeys.title': '录制按键',
  'keybindings.action.removeKeybinding.title': '移除键盘快捷方式',
  'keybindings.action.resetKeybinding.title': '重置键盘快捷方式',
  'keybindings.action.searchKeybindings.title': '聚焦搜索',
  'keybindings.action.showSameKeybindings.title': '显示相同的键盘快捷方式',
  'keybindings.action.toggleSortByPrecedence.title': '按优先级排序',
  'keybindings.addKeybinding': '添加键盘快捷方式',
  'keybindings.aria.resultCount': '{count} 个键盘快捷方式',
  'keybindings.changeKeybinding': '更改键盘快捷方式',
  'keybindings.clearSearch': '清除键盘快捷方式搜索内容',
  'keybindings.column.when': '当',
  'keybindings.define.ariaLabel': '定义键盘快捷方式',
  'keybindings.define.chordTo': '组合键至',
  'keybindings.define.existing': '已有 {count} 个命令使用此键盘快捷方式',
  'keybindings.define.message': '按下所需的组合键，然后按 ENTER。',
  'keybindings.menu.addKeybinding': '添加键盘快捷方式…',
  'keybindings.menu.changeKeybinding': '更改键盘快捷方式…',
  'keybindings.menu.changeWhen': '更改 when 表达式',
  'keybindings.menu.copy': '复制',
  'keybindings.menu.copyCommandId': '复制命令 ID',
  'keybindings.menu.copyCommandTitle': '复制命令标题',
  'keybindings.menu.removeKeybinding': '移除键盘快捷方式',
  'keybindings.menu.resetKeybinding': '重置键盘快捷方式',
  'keybindings.menu.showSame': '显示相同的键盘快捷方式',
  'keybindings.recordKeys': '录制按键',
  'keybindings.recording.badge': '正在录制按键',
  'keybindings.recording.placeholder': '正在录制按键。按 Escape 退出。',
  'keybindings.sortByPrecedence': '按优先级排序（最高在前）',
  'keybindings.table.ariaLabel': '键盘快捷方式',
  'keybindings.whenInput.ariaLabel': 'when 表达式',
  'keybindings.whenInput.suggestions': '上下文键',
  'keybindingsEditor.extensionSource': '扩展',
  'keybindingsEditor.sourceSystem': '系统',
  'keybindingsEditor.sourceUser': '用户',
  'keybinding.chordPending': '已按下（{key}），等待第二个按键…',

  // --- Performance ---
  'performance.statusbar.tooltip': '启动耗时 {seconds}s - 点击查看报告',
  'startupPerformance.duration': '耗时',
  'startupPerformance.from': '从',
  'startupPerformance.marks': '标记',
  'startupPerformance.measuring': '正在测量启动性能…',
  'startupPerformance.offset': '距启动的偏移',
  'startupPerformance.phases': '阶段',
  'startupPerformance.title': '启动性能',
  'startupPerformance.to': '到',
  'startupPerformance.total': '总启动时间：',
  'interactionPerformance.title': '交互性能',
  'interactionPerformance.refresh': '刷新',
  'interactionPerformance.overview':
    '已采样交互：{count} · 慢交互：{slow}（{share}） · 长帧：{loaf}',
  'interactionPerformance.samplingNote':
    '仅采样耗时 ≥16ms 的交互（Event Timing 采样阈值）；分位数为这些样本的直方图桶下界估计。',
  'interactionPerformance.byType': '按交互类型',
  'interactionPerformance.noSamples': '尚未采样到交互。',
  'interactionPerformance.type': '类型',
  'interactionPerformance.count': '次数',
  'interactionPerformance.p95': 'p95',
  'interactionPerformance.p99': 'p99',
  'interactionPerformance.max': '最大',
  'interactionPerformance.slowest': '最慢交互',
  'interactionPerformance.noSlow': '暂无慢交互记录。',
  'interactionPerformance.duration': '耗时',
  'interactionPerformance.split': '输入 / 处理 / 呈现',
  'interactionPerformance.attribution': '归因',
  'interactionPerformance.context': '上下文',

  // --- Release notes ---
  'releaseNotes.empty': '没有可用的发行说明。',
  'releaseNotes.show': '显示发行说明',
  'releaseNotes.title': '发行说明',
  'releaseNotes.whatsNew': '{version} 中的新功能',

  // --- Schema viewer ---
  'schemaViewer.name': 'Schema：{name}',

  // --- Symbol kinds ---
  'symbolKind.arrays': '数组',
  'symbolKind.booleans': '布尔值',
  'symbolKind.classes': '类',
  'symbolKind.constants': '常量',
  'symbolKind.constructors': '构造函数',
  'symbolKind.enumMembers': '枚举成员',
  'symbolKind.enums': '枚举',
  'symbolKind.events': '事件',
  'symbolKind.fields': '字段',
  'symbolKind.files': '文件',
  'symbolKind.functions': '函数',
  'symbolKind.interfaces': '接口',
  'symbolKind.keys': '键',
  'symbolKind.methods': '方法',
  'symbolKind.misc': '其他',
  'symbolKind.modules': '模块',
  'symbolKind.namespaces': '命名空间',
  'symbolKind.nulls': 'null',
  'symbolKind.numbers': '数字',
  'symbolKind.objects': '对象',
  'symbolKind.operators': '运算符',
  'symbolKind.packages': '包',
  'symbolKind.properties': '属性',
  'symbolKind.strings': '字符串',
  'symbolKind.structs': '结构体',
  'symbolKind.typeParameters': '类型参数',
  'symbolKind.variables': '变量',

  // --- Color theme ---
  'colorTheme.current': '（当前）',
  'colorTheme.dark': '深色',
  'colorTheme.light': '浅色',
  'iconTheme.current': '（当前）',
  transparencyRequired: '此颜色必须为透明，否则会遮挡内容',
  useDefault: '使用默认颜色。',

  // --- Config location ---
  'configLocation.confirm.copy': '复制当前设置',
  'configLocation.confirm.empty': '不复制',
  'configLocation.confirm.message': '将配置目录切换为：',
  'configLocation.confirmNonEmpty.detail':
    '{dir} 中已有的 settings.json / keybindings.json 会保留；只会复制缺失的文件。',
  'configLocation.confirmNonEmpty.message': '目标目录非空。仍要将当前设置复制进去吗？',
  'configLocation.confirmNonEmpty.proceed': '仍然复制',
  'configLocation.locked': '配置目录已由命令行 / 环境变量覆盖固定，无法在这里更改。',
  'configLocation.reset': '配置目录已重置为默认值。',
  'configLocation.switched': '配置目录已切换到 {dir}。',

  // --- Remote ---
  'remote.addHost.placeholder': 'user@host[:port]',
  'remote.addHost.prompt': 'SSH 主机',
  'remote.addHost.title': '添加新的 SSH 主机',
  'remote.closeConnection.confirm': '关闭与"{authority}"的连接？这会关闭远程工作区。',
  'remote.closeConnection.confirmButton': '关闭',
  'remote.closeConnection.none': '没有可关闭的已连接主机。',
  'remote.closeConnection.placeholder': '选择要关闭的连接',
  'remote.connect.failed': '连接到 {authority} 失败：{message}',
  'remote.connectToHost.placeholder': 'user@host[:port]，或选择一个主机',
  'remote.connectToHost.sshConfig': 'SSH 配置',
  'remote.connectToWsl.placeholder': '选择 WSL 发行版',
  'remote.connection.close': '关闭连接',
  'remote.connection.retry': '重试连接',
  'remote.connection.stopServer': '停止远程服务器',
  'remote.group.ssh': 'SSH',
  'remote.group.wsl': 'WSL',
  'remote.menu.close': '关闭连接',
  'remote.menu.connectToHost': '连接到主机…',
  'remote.menu.connectToWsl': '连接到 WSL…',
  'remote.menu.local.placeholder': '选择打开远程窗口的方式',
  'remote.menu.openFolder': '在主机上打开文件夹…',
  'remote.menu.placeholder': '远程（{authority}）',
  'remote.menu.retry': '重试连接',
  'remote.menu.stopServer': '停止远程服务器',
  'remote.menu.title': '显示远程菜单',
  'remote.noConnected': '没有已连接的主机。请先运行"Remote-SSH: 连接到主机…"。',
  'remote.noFailed': '没有可重试的失败连接。',
  'remote.notConnected': '"{authority}" 未连接。',
  'remote.openFolder.placeholder': '选择一个已连接的主机',
  'remote.openFolder.title': '在 {authority} 上打开文件夹',
  'remote.progress.connect': '正在连接到 {authority}…',
  'remote.recent.remove': '从最近使用中移除',
  'remote.reconnect.closeRemote': '关闭远程工作区',
  'remote.reconnect.failed': '无法重新连接到 {authority}。',
  'remote.reconnect.failed.detail': '无法重新连接到 {authority}。{error}',
  'remote.reconnect.lost': '与 {authority} 的连接已断开。正在重新连接…',
  'remote.reconnect.reconnected': '已重新连接到 {authority}。',
  'remote.reconnect.retry': '重试',
  'remote.retryConnection.placeholder': '选择要重试的连接',
  'remote.section.targets': '目标',
  'remote.section.wslTargets': 'WSL 目标',
  'remote.status.connected': '$(remote) {label}',
  'remote.status.connecting': '{label}（正在连接…）',
  'remote.status.failed': '$(warning) {label}（失败）',
  'remote.status.local.tooltip': '打开远程窗口',
  'remote.status.reconnecting': '{label}（正在重新连接…）',
  'remote.status.step': '{label}（第 {index}/{total} 步：{step} · {elapsed}）',
  'remote.status.step.installing': '正在安装服务器',
  'remote.status.step.installingNode': '正在安装 Node.js 运行时',
  'remote.status.step.startingDaemon': '正在启动服务器',
  'remote.status.step.stoppingOld': '正在停止旧服务器',
  'remote.status.step.uploading': '正在上传服务器包',
  'remote.status.tooltip': '远程：{authority}',
  'remote.stopServer.confirm': '停止"{authority}"上的远程服务器？',
  'remote.stopServer.confirmButton': '停止服务器',
  'remote.stopServer.detail.none': '连接将被断开。',
  'remote.stopServer.detail.windows': '连接将被断开，{count} 个相关的工作区窗口将被关闭：{names}',
  'remote.stopServer.emptyWindow': '空窗口',
  'remote.stopServer.none': '没有可停止的已连接主机。',
  'remote.stopServer.placeholder': '选择要停止服务器的主机',
  'remote.target.connect': '连接到主机…',
  'remote.target.forget': '遗忘',
  'remote.target.openFolder': '在主机上打开文件夹…',
  'remote.targets.ariaLabel': '远程目标',
  'remote.targets.empty': '没有 SSH 目标。请添加一个，或使用 SSH 配置。',
  'remote.wsl.connect': '连接到 WSL…',
  'remote.wsl.default': '默认',
  'remote.wsl.noneDetected': '未检测到 WSL 发行版。请安装 WSL 及一个发行版后重试。',
  'remote.wsl.pickLabel': '{name}（WSL）',
  'remote.wsl.runningTooltip': '{name}（正在运行）',

  // --- Workspace trust ---
  'trust.manage.cancel': '取消',
  'trust.manage.restrict': '限制',
  'trust.manage.trust': '信任',
  'trust.manage.trustedDetail': '{folder}\n\n所有扩展均已启用。',
  'trust.manage.trustedMsg': '此工作区已受信任。',
  'trust.manage.untrustedDetail':
    '{folder}\n\n信任该文件夹即可启用所有扩展。在受限模式下，需要信任的扩展将被禁用。',
  'trust.manage.untrustedMsg': '你信任此工作区的作者吗？',
  'trust.noFolder': '请先打开文件夹以管理工作区信任。',
  'trust.revoke.confirm': '限制此工作区？',
  'trust.revoke.detail': '需要受信任工作区的扩展将被禁用，且扩展宿主将重启。',
  'trust.revoke.primary': '限制',
  'trust.status.restricted': '受限模式',
  'trust.status.tooltip': '此工作区处于受限模式。部分扩展已被禁用。点击管理工作区信任。',

  // --- Window, workspace & shutdown ---
  'jumpList.newWindow': '新建窗口',
  'jumpList.newWindowDesc': '打开一个新窗口',
  'jumpList.recentFolders': '最近的文件夹',
  'progress.openFolder': '正在打开文件夹…',
  'progress.openRecent': '正在打开 {name}…',
  'reload.leakDetected.message': '检测到 {count} 个未释放的 Disposable',
  'shutdown.action.closeWindow': '关闭窗口',
  'shutdown.action.quit': '退出',
  'shutdown.action.reload': '重启编辑器',
  'shutdown.action.switchWorkspace': '切换工作区',
  'shutdown.runningSessions.continue': '继续',
  'shutdown.runningSessions.detail': '是否继续？',
  'shutdown.runningSessions.message': '{count} 个会话正在运行，{action}将中断它们',
  'window.untitled': '未命名（窗口 {id}）',
  'workspace.recent.opened': '已打开',
  'workspace.recent.openedSuffix': '（已打开）',
  'crash.reload': '重新加载',
  'crash.close': '关闭窗口',
  'crash.title': '编辑器窗口已崩溃',
  'crash.detail': '渲染进程意外退出（{reason}）。重新加载可恢复窗口，正在进行的任务可能已中断。',
  'crashLoop.restore': '正常启动（恢复工作区）',
  'crashLoop.skip': '跳过恢复（打开空窗口）',
  'crashLoop.title': '连续异常退出',
  'crashLoop.message': '应用已连续 {count} 次异常退出。',
  'crashLoop.detail':
    '崩溃可能由上次打开的工作区触发（例如包含海量文件的目录）。可以跳过本次工作区恢复，以空窗口启动进行排查；之前的目录仍可从「最近打开」进入。',
  'abnormalExit.withDumps': '上次会话异常终止，已留下 {count} 个崩溃转储文件。',
  'abnormalExit.noDumps': '上次会话在 {time} 前后未正常退出，且未留下崩溃转储文件，具体原因未知。',
  'abnormalExit.openCrashes': '打开崩溃目录',

  // --- Update ---
  'update.availableMsg': '有新版本（{version}）可用。',
  'update.availableTooltip': '有新版本（{version}）可用 - 点击下载',
  'update.check': '检查更新',
  'update.checkFailed': '无法检查更新：{error}',
  'update.checking': '正在检查更新…',
  'update.checkingShort': '正在检查更新…',
  'update.download': '下载更新',
  'update.downloadedMsg': '版本 {version} 已准备好安装。',
  'update.downloadedTooltip': '版本 {version} 已下载 - 点击重启并安装',
  'update.downloadingShort': '正在下载更新… {percent}%',
  'update.later': '稍后',
  'update.restart': '重启以更新',
  'update.restartNow': '立即重启',
  'update.upToDate': '你正在运行最新版本（{version}）。',
  'update.availableLabel': '更新',
  'update.checkingLabel': '正在检查…',
  'update.downloadedLabel': '重启以更新',
  'update.downloadingLabel': '正在下载… {percent}%',
  'update.statusbar.available': '有可用更新',
  'update.statusbar.available.tooltip': '版本 {version} 可用 —— 点击下载',
  'update.statusbar.downloaded': '重启以安装',
  'update.statusbar.downloaded.tooltip': '版本 {version} 已下载 —— 点击重启并安装',

  // --- Workbench error & Monaco ---
  'monaco.commandSource': 'Monaco',
  'workbenchError.title': '出现错误',
  'monaco.needsActiveEditor': '此命令需要一个活动的文本编辑器',
  'monaco.command.columnSelectUp': '向上列选择',
  'monaco.command.columnSelectDown': '向下列选择',
  'monaco.command.scrollPageUp': '向上滚动一页',
  'monaco.command.scrollPageDown': '向下滚动一页',

  // --- AI: commands & model picker ---
  'ai.clearApiKey.confirm': '清除已存储的 {provider} API 密钥？',
  'ai.clearApiKey.clear': '清除',
  'ai.clearApiKey.done': '已清除 {provider} 的 API 密钥。',
  'ai.clearApiKey.none': '{provider} 没有已存储的 API 密钥。',
  'ai.commitMessage.pickModel.placeholder': '选择用于生成提交信息的模型',
  'ai.inlineCompletion.disabled': '内联补全已禁用。',
  'ai.inlineCompletion.enabled': '内联补全已启用。',
  'ai.inlineCompletion.noModel': '尚未配置内联补全模型。选择一个即可开始。',
  'ai.inlineCompletion.pickModel.placeholder': '选择用于内联补全的模型',
  'ai.inlineCompletion.pickNow': '选择模型',
  'ai.pickProvider.placeholder': '选择一个 provider',
  'ai.pickModel.manage': '打开 AI 设置…',
  'ai.pickModel.placeholder': '选择活跃的 AI 模型',
  'ai.quickSettings.chat': '对话',
  'ai.quickSettings.commit': '提交',
  'ai.quickSettings.inline': '内联',
  'ai.quickSettings.inlineCompletions': '内联补全',
  'ai.quickSettings.inlineScope.editor': '文本编辑器',
  'ai.quickSettings.inlineScope.session': '会话输入框',
  'ai.quickSettings.manageModels': '管理 AI 模型',
  'ai.quickSettings.noModel': '选择模型…',
  'ai.quickSettings.openSessions': '打开 Sessions',
  'ai.quickSettings.sessionTitle': '会话标题',
  'ai.quickSettings.title': 'AI 设置',
  'ai.sessionTitle.pickModel.placeholder': '选择用于生成会话标题的模型',
  'ai.setApiKey.done': '已为 {provider} 保存 API 密钥。',
  'ai.setApiKey.empty': 'API 密钥不能为空。',
  'ai.setApiKey.prompt':
    '输入 {provider} 的 API 密钥。以明文保存在 aiSettings.json（跨机器同步）。',
  'ai.statusbar.tooltip': 'AI',
  'ai.vendor.openai.label': 'OpenAI（兼容）',
  'ai.vendor.ollama.label': 'Ollama',
  'ai.verify.noProvider': '未注册此协议的提供方。',
  'ai.verify.noModels': '端点有响应，但没有可用的模型。',
  'ai.verify.unreachable': '无法连接到端点。',
  'ai.verify.timeout': '端点响应超时。',
  'ai.verify.unauthorized': '认证失败（{status}），请检查 API 密钥。',
  'ai.verify.serverError': '端点返回服务器错误（{status}）。',
  'ai.verify.httpError': '端点返回了意外状态码（{status}）。',
  'ai.verify.failed': '连接失败。',
  'ai.modelSettings.temperature': '采样温度（0–2）。',
  'ai.modelSettings.maxTokens': '生成的最大 token 数。',
  'ai.modelSettings.topP': '核采样概率（0–1）。',
  'ai.modelSettings.frequencyPenalty': '按 token 已有频率施加惩罚（−2 到 2）。',
  'ai.modelSettings.presencePenalty': '对已出现过的 token 施加惩罚（−2 到 2）。',
  'ai.modelSettings.seed': '用于（尽力而为的）确定性采样的种子。',
  'ai.modelSettings.reasoningEffort': '推理投入级别。',
  'ai.modelSettings.topK': '只从最可能的 k 个 token 中采样。',
  'ai.error.unauthorized': 'OpenAI 未授权（{status}）：{detail}',
  'ai.error.rateLimited': 'OpenAI 触发限流（{status}）：{detail}',
  'ai.error.serverError': 'OpenAI 服务器错误（{status}）：{detail}',
  'ai.error.requestFailed': 'OpenAI 请求失败（{status}）：{detail}',
  'ai.error.ollama.unauthorized': 'Ollama 未授权（{status}）：{detail}',
  'ai.error.ollama.rateLimited': 'Ollama 触发限流（{status}）：{detail}',
  'ai.error.ollama.serverError': 'Ollama 服务器错误（{status}）：{detail}',
  'ai.error.ollama.requestFailed': 'Ollama 请求失败（{status}）：{detail}',
  'ai.error.anthropic.unauthorized': 'Anthropic 未授权（{status}）：{detail}',
  'ai.error.anthropic.rateLimited': 'Anthropic 触发限流（{status}）：{detail}',
  'ai.error.anthropic.serverError': 'Anthropic 服务器错误（{status}）：{detail}',
  'ai.error.anthropic.requestFailed': 'Anthropic 请求失败（{status}）：{detail}',
  'ai.error.anthropic.outputLimit':
    '模型在输出任何文本前就达到了 max_tokens 上限——思考（推理）耗尽了全部输出预算。请打开 AI 设置，将该模型的 thinking 参数设为 disabled，或提高请求的 token 上限。',
  'ai.error.openai.outputLimit':
    '模型在输出任何文本前就达到了输出 token 上限——思考（推理）耗尽了全部输出预算。请打开 AI 设置，将该模型的 thinking 参数设为 disabled，或提高请求的 token 上限。',
  'ai.error.responsesNotSupported':
    'OpenAI Responses 协议仅供 agent 使用；请为编辑器功能选择 chat 协议的模型。',

  // --- AI: settings shell ---
  'aiSettings.group.agents': 'Agents',
  'aiSettings.group.ai': 'AI',
  'aiSettings.help.label': '帮助',
  'aiSettings.nav': 'AI 设置分类',
  'aiSettings.openJson': '打开 aiSettings.json',
  'aiSettings.title': 'AI 设置',
  'aiSettings.category.features': '功能模型',
  'aiSettings.category.models': '模型配置',
  'aiSettings.category.mcpServers': 'MCP 服务器',
  'aiSettings.category.providers': '供应商配置',

  // --- AI: model knowledge panel (Model Configuration) ---
  'aiKnowledge.addModel': '添加模型',
  'aiKnowledge.loading': '正在加载模型知识…',
  'aiKnowledge.loadFailed':
    '读取模型知识失败，为避免覆盖已有内容，编辑已锁定。请查看日志后重新打开设置页。',
  'aiKnowledge.empty':
    '还没有你自己的模型定义。可以为内置目录不认识的模型新建一条，或者覆盖下方的内置模型。',
  'aiKnowledge.legacy.banner':
    'aiSettings.json 仍是已废弃的两层格式且已被忽略，因此这里无法编辑模型知识。请先按新格式重建该文件。',
  'aiKnowledge.section.custom': '你的模型',
  'aiKnowledge.section.builtin': '内置模型',
  'aiKnowledge.section.builtin.note':
    '随编辑器发布并由编辑器持续更新。覆盖某条即可改写其中的字段；你没有设置的字段仍跟随内置定义。',
  'aiKnowledge.section.builtin.allOverridden': '所有内置模型都已在上方有覆盖条目。',
  'aiKnowledge.builtin.override': '覆盖',
  'aiKnowledge.builtin.contextBadge': '输入 {count}',

  // card header
  'aiKnowledge.badge.custom': '自定义',
  'aiKnowledge.badge.overridesBuiltin': '覆盖内置',
  'aiKnowledge.badge.usedBy': '{count} 个供应商在用',
  'aiKnowledge.card.rename': '重命名模型 key',
  'aiKnowledge.card.duplicate': '复制模型',
  'aiKnowledge.card.remove': '移除模型',
  'aiKnowledge.card.reset': '恢复为内置定义',

  // fields
  'aiKnowledge.field.name': '显示名称',
  'aiKnowledge.field.family': '模型系列',
  'aiKnowledge.field.vendor': '厂商',
  'aiKnowledge.field.nativeProtocol': '原生协议',
  'aiKnowledge.field.nativeProtocol.unset': '未设置',
  'aiKnowledge.field.nativeProtocol.builtin': '未设置（内置：{protocol}）',
  'aiKnowledge.field.nativeProtocol.note':
    '该模型在自家厂商端点上使用的协议。经由其它协议访问都属于转译。',
  'aiKnowledge.field.maxInputTokens': '最大输入 token',
  'aiKnowledge.field.maxOutputTokens': '最大输出 token',
  'aiKnowledge.field.number.invalid': '请输入整数 token 数。',
  'aiKnowledge.field.capabilities': '能力',
  'aiKnowledge.field.capabilities.clear': '清除能力覆盖',
  'aiKnowledge.field.capabilities.builtin': '来自内置知识。',
  'aiKnowledge.field.capabilities.default':
    '未声明——默认按支持流式处理。勾选任意一项都会完整记录整组能力。',
  'aiKnowledge.field.reasoningEffort': '推理强度档位',
  'aiKnowledge.field.reasoningEffort.note':
    '用逗号分隔，按模型文档给出的顺序填写。留空表示该模型没有推理强度设置。',
  'aiKnowledge.field.parameters': '请求参数',
  'aiKnowledge.field.parameters.edit': '编辑参数 schema',
  'aiKnowledge.field.parameters.clear': '清除参数 schema',
  'aiKnowledge.field.parameters.count': '{count} 个参数',
  'aiKnowledge.field.parameters.none': '没有可调参数——选模型时不会为它显示配置表单。',
  'aiKnowledge.capability.streaming': '流式处理',
  'aiKnowledge.capability.vision': '图片输入',
  'aiKnowledge.capability.promptCaching': 'Prompt 缓存',
  'aiKnowledge.capability.toolCalling': '工具调用',
  'aiKnowledge.inherit.from': '内置：{value}',
  'aiKnowledge.inherit.overrides': '覆盖内置：{value}',
  'aiKnowledge.inherit.builtin': '来自内置知识',

  // parameter schema dialog
  'aiKnowledge.parameters.title': '请求参数 schema',
  'aiKnowledge.parameters.hint':
    '每个参数一个 key，例如 { "temperature": { "type": "number", "default": 1 } }。填空对象即清除该 schema。',
  'aiKnowledge.parameters.save': '保存',
  'aiKnowledge.parameters.cancel': '取消',

  // add dialog
  'aiKnowledge.add.title': '添加模型',
  'aiKnowledge.add.key': '模型 key',
  'aiKnowledge.add.keyEmpty': '必须填写模型 key。',
  'aiKnowledge.add.keySlash': '模型 key 不能包含 “/”。',
  'aiKnowledge.add.keyExists': '该模型 key 已存在。',
  'aiKnowledge.add.keyHint':
    '供应商 protocolMap 解析知识时用的 key。若引用写成裸字符串，它同时也是发给端点的 wire 名称。',
  'aiKnowledge.add.overrideHint':
    '已有内置模型使用该 key。新建条目为空，你设置的每个字段都会覆盖内置定义。',
  'aiKnowledge.add.create': '创建',
  'aiKnowledge.add.override': '覆盖',
  'aiKnowledge.add.cancel': '取消',

  // remove / reset / rename
  'aiKnowledge.remove.confirm': '移除模型 {key}？',
  'aiKnowledge.remove.action': '移除',
  'aiKnowledge.remove.detail':
    '被这些供应商引用：{providers}。这些模型仍可调用，但会丢失元数据（名称、token 上限、能力）。',
  'aiKnowledge.reset.confirm': '把 {key} 恢复为内置定义？',
  'aiKnowledge.reset.action': '恢复',
  'aiKnowledge.rename.title': '重命名模型 key',
  'aiKnowledge.rename.invalid': '模型 key 不能包含 “/”。',
  'aiKnowledge.rename.exists': '已存在名为 {key} 的模型。',
  'aiKnowledge.rename.builtinExists':
    '{key} 是内置模型。改成这个名字会让本条目变成对那个模型的覆盖——请改用「内置模型」区里的「覆盖」。',
  'aiKnowledge.rename.confirm': '把 {from} 重命名为 {to}？',
  'aiKnowledge.rename.action': '重命名',
  'aiKnowledge.rename.rewritable': '将自动更新：{providers}。',
  'aiKnowledge.rename.bare':
    '无法自动更新：{providers}——它们按 wire 名称引用该模型，重命名不能改动 wire 名称。这些模型会丢失元数据。',

  // --- AI: provider configuration panel ---
  'aiModels.addProvider': '添加提供方',
  'aiModels.badge.keyed': '已配密钥',
  'aiModels.badge.modelCount': '{count} 个模型',
  'aiModels.badge.modelsLoading': '正在获取模型…',
  'aiModels.baseUrl': 'Base URL',
  'aiModels.baseUrl.placeholder': '提供方默认',
  'aiModels.filter.placeholder': '过滤模型…',
  'aiModels.configure': '配置模型',
  'aiModels.config.save': '保存',
  'aiModels.config.cancel': '取消',
  'aiModels.config.unset': '（默认）',

  // provider entry card
  'aiModels.instance.remove.remove': '移除',
  'aiModels.instance.status.idle': '未探测',
  'aiModels.instance.status.checking': '探测中…',
  'aiModels.instance.status.ok': '已连接 · {count} 个模型',
  'aiModels.instance.status.okReachable': '已连接',
  'aiModels.instance.status.fail': '连接失败',
  'aiModels.apiKey': 'API 密钥',
  'aiModels.apiKey.unset': '未设置',
  'aiModels.apiKey.show': '显示 API 密钥',
  'aiModels.apiKey.hide': '隐藏 API 密钥',
  'aiModels.apiKey.edit': '编辑 API 密钥',
  'aiModels.apiKey.clearBtn': '清除 API 密钥',
  'aiModels.apiKey.clearConfirm': '清除已存储的 {name} API 密钥？',
  'aiModels.apiKey.clearAction': '清除',
  'aiModels.rates.line': '{count} 个模型 · 更新于 {time}',
  'aiModels.usage.title': '账号额度',
  'aiModels.usage.refresh': '刷新用量',
  'aiModels.usage.unavailable': '不可用',
  'aiModels.usage.unavailableTooltip':
    '已配置用量来源但当前拉取不到权威数字，这里不会用本地估算值代替。',
  'aiModels.usage.used': '已用',
  'aiModels.usage.limit': '上限',
  'aiModels.usage.remaining': '剩余',
  'aiModels.usage.kind.quota': '配额',
  'aiModels.usage.kind.balance': '余额',
  'aiModels.usage.kind.subscription': '订阅',
  'aiModels.usage.fetchedAt': '获取于 {time}',

  // provider entry card
  'aiModels.section.providers': '提供方',
  'aiModels.providers.empty': '尚无提供方。添加一个以接入 AI 服务。',
  'aiModels.providers.loading': '加载提供方…',
  'aiModels.legacy.banner':
    'aiSettings.json 仍是旧的两层格式，已被忽略。请按新格式手工重建（密钥可从旧文件复制）。',
  'aiModels.entry.extends': '继承自 {id}',
  'aiModels.entry.defaultProtocol': '默认协议',
  'aiModels.entry.defaultProtocol.first': '首个协议',
  'aiModels.entry.noModels': '该协议下没有解析出的模型。',
  'aiModels.entry.agentOnly': '仅 Agent',
  'aiModels.entry.remove.confirm': '移除提供方 {name}？',
  'aiModels.entry.remove.remove': '移除',
  'aiModels.pricingSource.value': '{source} · 尚未获取到价目表。',
  'aiModels.pricingSource.none': '未配置价格来源——费率将显示为未知，而不会被估算。',
  'aiModels.rate.unknown': '费率未知',
  'aiModels.rate.unknownTooltip': '该提供方未配置价格来源（或该模型不在价目表中）。',
  'aiModels.issue.malformedEntry': '条目格式错误（缺少字符串 id）',
  'aiModels.issue.invalidId': "id 非法（为空，或包含 '/'）",
  'aiModels.issue.duplicateId': '重复的 id',
  'aiModels.issue.unknownExtends': '未知的继承目标',
  'aiModels.issue.extendsCycle': '继承成环',
  'aiModels.issue.extendsDepth': '继承链过深',
  'aiModels.issue.noProtocol': '未声明协议',
  'aiModels.issue.unknownDefaultProtocol': '未知的默认协议',
  'aiModels.issue.orphanBanner': 'aiSettings.json 中有条目无法读取：{details}',

  // --- AI: add-provider dialog ---
  'aiModels.addProvider.title': '添加提供方',
  'aiModels.addProvider.id': '提供方 id',
  'aiModels.addProvider.idEmpty': '提供方 id 不能为空。',
  'aiModels.addProvider.idSlash': "提供方 id 不能包含 '/'。",
  'aiModels.addProvider.idExists': '该提供方 id 已存在。',
  'aiModels.addProvider.protocol': '默认协议',
  'aiModels.addProvider.baseUrl': 'Base URL',
  'aiModels.addProvider.apiKey': 'API 密钥（可选）',
  'aiModels.addProvider.verify': '验证',
  'aiModels.addProvider.verifying': '验证中…',
  'aiModels.addProvider.verifyOk': '已连接 · {count} 个模型',
  'aiModels.addProvider.verifyFail': '验证失败。',
  'aiModels.addProvider.create': '创建',
  'aiModels.addProvider.cancel': '取消',
  'aiModels.addProvider.template': '模板',
  'aiModels.addProvider.baseUrlInvalid': '不是完整的 http(s) URL —— 仍可创建，但会跳过验证。',
  'aiModels.addProvider.revealApiKey': '显示 API 密钥',
  'aiModels.addProvider.hideApiKey': '隐藏 API 密钥',

  // --- AI: provider card — 保存反馈与继承标注 ---
  'aiModels.saved': '已保存',
  'aiModels.entry.duplicate': '复制提供方',
  'aiModels.filter.clear': '清除模型过滤',
  'aiModels.inherit.from': '继承自 {id}',
  'aiModels.inherit.overrides': '覆盖 {id}',
  'aiModels.inherit.revert': '清除覆盖，恢复继承',
  'aiModels.apiKey.overridePlaceholder': '继承自 {id} —— 输入密钥可覆盖',
  'aiModels.apiKey.inherited': '继承自 {id}',
  'aiModels.apiKey.save': '保存',
  'aiModels.apiKey.cancel': '取消',

  // --- AI: provider card — extends ---
  'aiModels.extends': '继承自',
  'aiModels.extends.none': '不继承（独立条目）',
  'aiModels.extends.rejected': '未应用：{reason}',
  'aiModels.extends.hint': '用于同一网关的第二个接入点 —— 协议映射整体替换，其余字段逐项覆盖。',
  'aiModels.extends.nothingInherited': '所有字段都在本地设置，当前没有继承任何内容。',
  'aiModels.extends.summary': '正在继承 {fields}',

  // --- AI: provider card — 问题修复引导 ---
  'aiModels.issue.fix.malformedEntry':
    'providers[] 的这一项不是带字符串 id 的对象。请在 aiSettings.json 中修正。',
  'aiModels.issue.fix.invalidId':
    "提供方 id 是每个模型 id 的第一段，因此不能为空、也不能包含 '/'。请在 aiSettings.json 中重命名。",
  'aiModels.issue.fix.duplicateId':
    '两个条目使用了相同的 id，只有第一个生效。请重命名或删除其中一个。',
  'aiModels.issue.fix.unknownExtends':
    '该条目继承自一个不存在的 id。请指向已有的提供方，或清除继承。',
  'aiModels.issue.fix.extendsCycle': '继承链绕回了该条目。请清除环上任意一个条目的继承。',
  'aiModels.issue.fix.extendsDepth': '继承链超过 8 层。请改为继承更靠近根部的条目以压平层级。',
  'aiModels.issue.fix.noProtocol':
    '提供方在声明至少一个协议之前不提供任何模型。请在「协议与模型」下添加一个。',
  'aiModels.issue.fix.unknownDefaultProtocol':
    '默认协议不在该提供方的协议映射里。请选择一个已声明的协议，或添加该协议。',
  'aiModels.issue.clearExtends': '清除继承',

  // --- AI: provider card — 模型声明编辑 ---
  'aiModels.ref.wireName': '线上模型名',
  'aiModels.ref.wireName.desc': '该端点期望的模型名。留空则使用知识库条目名。',
  'aiModels.ref.knowledgeRef': '知识库条目',
  'aiModels.ref.knowledgeRef.desc':
    '上下文窗口、厂商与能力的来源。当网关重命名了一个已知模型时选它。',
  'aiModels.ref.none': '（与线上模型名相同）',
  'aiModels.ref.capabilities': '能力',
  'aiModels.ref.capabilities.desc':
    '能力只能关闭、不能开启 —— 做协议翻译的网关会丢失特性，但无法凭空造出特性。',
  'aiModels.ref.capabilities.unknown': '该模型不在知识库里，因此没有可收窄的能力。',
  'aiModels.ref.save': '保存',
  'aiModels.ref.cancel': '取消',

  // --- AI: provider card — 端点探测 ---
  'aiModels.probe.title': '{protocol} 上的模型',
  'aiModels.probe.loading': '正在询问端点…',
  'aiModels.probe.failed': '无法连接到该端点。',
  'aiModels.probe.capped': '端点共报告 {total} 个模型，此处显示前 {shown} 个。',
  'aiModels.probe.largeList':
    '该端点提供 {total} 个模型。这么长的静态清单难以复核 —— 建议改为让该协议保持「从端点发现」。',
  'aiModels.probe.filter': '过滤…',
  'aiModels.probe.selectAll': '全选',
  'aiModels.probe.selectNone': '全不选',
  'aiModels.probe.empty': '没有匹配该过滤条件的模型。',
  'aiModels.probe.selected': '已选 {count} 个',
  'aiModels.probe.confirm': '使用所选',
  'aiModels.probe.cancel': '取消',

  // --- AI: provider card — 协议与模型 ---
  'aiModels.protocols.title': '协议与模型',
  'aiModels.protocols.empty': '未声明任何协议 —— 该提供方不提供模型。请在下方添加一个。',
  'aiModels.protocol.mode': '模型清单模式',
  'aiModels.protocol.discover': '从端点发现',
  'aiModels.protocol.static': '静态清单',
  'aiModels.protocol.discover.hint':
    '每次启动都会向端点索取模型清单。固化后可离线使用，也便于复核变化。',
  'aiModels.protocol.discover.loading': '正在向端点获取模型清单…',
  'aiModels.protocol.pin': '固化 {count} 个模型为静态清单',
  'aiModels.protocol.pin.loading': '固化模型为静态清单',
  'aiModels.protocol.probe': '探测端点…',
  'aiModels.protocol.add': '添加协议',
  'aiModels.protocol.addModel': '添加模型',
  'aiModels.protocol.addModel.placeholder': '线上模型名…',
  'aiModels.protocol.removeLabel': '移除协议 {protocol}',
  'aiModels.protocol.remove': '移除',
  'aiModels.protocol.toggle': '折叠/展开 {protocol} 模型',
  'aiModels.protocol.modelCount': '{count} 个模型',
  'aiModels.protocol.removeLastConfirm':
    '移除 {name} 的最后一个协议？在添加新协议之前，该提供方将不提供任何模型。',
  'aiModels.model.viaRef': '→ {ref}',
  'aiModels.model.unknown': '无元数据',
  'aiModels.model.unknownTooltip': '此处已声明，但不在知识库里 —— 仍可使用，只是没有元数据。',
  'aiModels.model.edit': '编辑模型声明',
  'aiModels.model.remove': '从清单中移除该模型',

  // --- AI: provider card — 价格与用量来源 ---
  'aiModels.pricingSource.title': '价格来源',
  'aiModels.pricingSource.refresh': '刷新价格',
  'aiModels.pricingSource.catalog': '内置目录',
  'aiModels.pricingSource.httpJson': 'HTTP JSON',
  'aiModels.pricingSource.catalog.vendor': '厂商',
  'aiModels.pricingSource.catalog.note':
    '费率按线上模型名查询，因此被重命名的渠道模型不会匹配到官方目录。',
  'aiModels.pricingSource.summary.catalog': '内置目录 · {vendor}',
  'aiModels.pricingSource.summary.httpJson': 'HTTP JSON · {path}',
  'aiModels.usageSource.title': '账号用量来源',
  'aiModels.usageSource.httpJson': 'HTTP JSON',
  'aiModels.usageSource.summary.httpJson': 'HTTP JSON · {path}',
  'aiModels.source.none.option': '无',
  'aiModels.source.inheritOption': '继承 {id}',
  'aiModels.source.summary.none': '无',
  'aiModels.source.summary.rates': '{count} 个模型',
  'aiModels.protocols.summary': '{count} 个协议 · {models} 个模型',
  'aiModels.protocols.summaryLoading': '{count} 个协议 · 获取中…',
  'aiModels.usage.badge.noValues': '未上报数值',
  'aiModels.cardSection.toggle': '折叠/展开 {title}',
  'aiModels.httpJson.path': '路径',
  'aiModels.httpJson.path.placeholder.pricing': '/v1/pricing',
  'aiModels.httpJson.path.placeholder.usage': '/api/user/self',
  'aiModels.httpJson.auth': '发送 API 密钥',
  'aiModels.httpJson.auth.desc': '把提供方的 API 密钥作为认证头附带发送（默认开启）。',
  'aiModels.httpJson.currency': '货币',
  'aiModels.httpJson.kind': '用量类型',
  'aiModels.httpJson.advanced': '高级',
  'aiModels.httpJson.authHeader': '认证头名称',
  'aiModels.httpJson.itemsPath': '条目路径',
  'aiModels.httpJson.itemsPath.placeholder': 'data',
  'aiModels.httpJson.modelField': '模型 id 字段',
  'aiModels.httpJson.unit': '单位',
  'aiModels.httpJson.field': '字段：{name}',
  'aiModels.httpJson.headers.note': '自定义请求头可通过「编辑原始 JSON」修改。',
  'aiModels.httpJson.editRaw': '编辑原始 JSON',
  'aiModels.httpJson.raw.title': '编辑原始 JSON',
  'aiModels.httpJson.raw.errorShape': 'JSON 必须是一个对象。',
  'aiModels.httpJson.raw.save': '保存',
  'aiModels.httpJson.raw.cancel': '取消',

  // --- AI: feature models panel ---
  'aiFeatures.chat': '对话',
  'aiFeatures.chat.desc': 'Agent 会话与对话补全使用的模型。',
  'aiFeatures.commit': '提交信息',
  'aiFeatures.commit.desc': '生成 Git 提交信息使用的模型。',
  'aiFeatures.inline': '内联补全',
  'aiFeatures.inline.desc': '编辑器幽灵文本补全使用的模型（可选更小更快的模型）。',
  'aiFeatures.sessionTitle': '会话标题',
  'aiFeatures.sessionTitle.desc': '为 Agent 会话生成友好标题使用的模型。',
  'aiFeatures.unavailable': '不可用',
  'aiFeatures.unset': '未设置',

  // --- AI: settings help (markdown) ---
  'aiSettings.help.features': [
    '## 功能模型',
    '',
    '为不同 AI 功能分别指定使用的模型，互不影响：',
    '',
    '- **对话**：Agent 会话与对话补全使用的主模型。',
    '- **内联补全**：编辑器幽灵文本补全（可选更小更快的模型）。',
    '- **提交信息**：生成 Git 提交信息使用的模型。',
    '',
    '点击任意一行会弹出模型选择器，选中后即时生效——与状态栏的模型选择体验一致。',
  ].join('\n'),
  'aiSettings.help.providers': [
    '## 供应商配置',
    '',
    '每个供应商条目就是一个网关端点——连接信息（网关地址 + API 密钥）加一张 `protocolMap`，声明它会说哪些协议、每种协议下暴露哪些模型。`protocolMap` 有三种状态：',
    '',
    '- **未声明**——该条目暂不提供任何协议（会标记为问题）。',
    '- **`[]`**——从端点自身枚举模型列表。',
    '- **非空列表**——只使用列出的这些模型，完全不访问网络。',
    '',
    '用 **extends** 给同一网关建多个入口（如不同账号）：子条目继承父条目的 `protocolMap`，只覆盖自己声明的字段。',
    '',
    '每个条目可选声明**价格来源**与**账号用量来源**：费率永远是（网关, 模型）的函数，用量跟着条目的密钥走。',
    '',
    '这里声明的模型可以**收窄**其知识库条目列出的能力——做协议翻译的网关只会丢特性、不会凭空造出特性——所以知识库里没有的能力在这里无法打开。',
    '',
    'API 密钥**明文**存放在 aiSettings.json 里——这是刻意的设计，方便配置跨机器同步（POSIX 下文件权限 0600）。界面一律掩码显示，绝不写入日志。',
    '',
    '模型的内在属性（token 上限、能力等）**不**在这里配置——它们属于 **模型配置** 分类。',
    '',
    '折叠与过滤状态都会被记住。需要直接编辑原始配置时，点 **打开 aiSettings.json**。',
  ].join('\n'),
  'aiSettings.help.models': [
    '## 模型配置',
    '',
    '模型**知识库**：每个条目对应一个逻辑模型 id，存放跨网关不变的内在属性。',
    '',
    '字段：`name` / `family` / `vendor` / `nativeProtocol` / `maxInputTokens` / `maxOutputTokens` / `capabilities` / `supportsReasoningEffort` / `parameters`。',
    '',
    '你的条目**按字段**合并覆盖内置目录——只写想改的字段即可；删掉某个字段即恢复内置值。',
    '',
    '刻意不含价格：费率是（网关, 模型）的函数，属于各供应商的 `pricingSource`。',
    '',
    '重命名 key 会影响 `protocolMap` 引用：显式 `ref` 的条目可自动改写，裸名引用会退化成无知识的裸模型。',
    '',
    '四项能力为 `streaming` / `vision` / `promptCaching` / `toolCalling`。勾选任意一项都会写入全部四项——只写一部分会静默丢掉内置条目声明的其余标志。',
  ].join('\n'),
  'aiSettings.help.mcpServers': [
    '## MCP 服务器',
    '',
    'MCP 服务器为 Agent 提供额外工具。每个服务器只出现一行，并为每个定义来源显示徽标：',
    '',
    '- **user**：`<userData>/settings.json`——在所有工作区可用。',
    '- **workspace**：`.universe-editor/settings.json`——仅对当前文件夹生效。',
    '- **.mcp.json**：工作区根目录的 Claude Code 兼容文件（此处只读）。',
    '- **vscode-user / vscode-ws**：VSCode 兼容设置文件（此处只读）。',
    '- **ext**：由已安装扩展贡献（不写入任何设置文件）。',
    '',
    '各来源**按服务器名**合并：生效来源的徽标正常显示，被覆盖的徽标弱化。点击徽标打开对应来源（user/workspace 打开编辑对话框，其余打开文件）。',
    '',
    '左侧两个开关是默认启停，与定义分开存放：人形图标是**用户级**默认（所有工作区生效），文件夹图标是**工作区级**默认（本项目优先）。工作区级开关是三态的——从开/关继续点击可回到「继承」虚线态。用户级开关只在服务器同时存在于用户级来源时显示。「disabled」徽标始终显示两级合成后的生效状态。',
    '',
    '输入框旁的会话级选择器仍可临时为当前会话启用或裁减服务器，但不会改动这里的默认。圆点显示活跃会话上报的实时连接状态。',
    '修改对新会话立即生效；活跃会话的服务器集合变化时会无缝重载。',
  ].join('\n'),

  // --- AI: MCP servers panel ---
  'aiMcp.add': '添加服务器',
  'aiMcp.empty.title': '尚未配置 MCP 服务器',
  'aiMcp.empty.desc': 'MCP 服务器为 Agent 提供额外工具（文件系统、文档等）。添加后即可用于新会话。',
  'aiMcp.scope.user': '用户（全局）',
  'aiMcp.scope.workspace': '工作区',
  'aiMcp.scope.vscodeWorkspace': 'VSCode 工作区（只读）',
  'aiMcp.scope.vscodeUser': 'VSCode 用户（只读）',
  'aiMcp.scope.extension': '扩展（只读）',
  'aiMcp.scope.claudeUser': 'Claude 用户配置（只读）',
  'aiMcp.scope.codexUser': 'Codex 用户配置（只读）',
  'aiMcp.scope.codexProject': 'Codex 项目配置（只读）',
  'aiMcp.row.affinityClaude': '仅对 Claude Code 会话生效',
  'aiMcp.row.affinityCodex': '仅对 Codex 会话生效',
  'aiMcp.row.claudeUserMerge': '合并自 ~/.claude.json 与 ~/.claude/settings.json',
  'aiMcp.remove.confirm': '从{scope}移除 MCP 服务器 {name}？',
  'aiMcp.remove.remove': '移除',
  'aiMcp.row.disabled': '已禁用',
  'aiMcp.row.shadowed': '被{scope}覆盖',
  'aiMcp.row.invalid': '运行时将被跳过：{reason}',
  'aiMcp.row.editScope': '编辑{scope}定义',
  'aiMcp.row.remove': '移除',
  'aiMcp.openJson.trigger': '打开配置文件',
  'aiMcp.openJson.user.label': '用户 settings.json',
  'aiMcp.openJson.user.detail': '全局 — 对所有工作区生效',
  'aiMcp.openJson.workspace.label': '工作区 settings.json',
  'aiMcp.openJson.mcpJson.detail': '工作区根目录，仅 Claude Code 会话',
  'aiMcp.openJson.vscodeUser.label': 'VSCode 用户 settings.json',
  'aiMcp.openJson.vscodeWorkspace.label': 'VSCode 工作区 settings.json',
  'aiMcp.openJson.readonly': '只读导入',
  'aiMcp.openJson.claudeUser.label': 'Claude settings.json',
  'aiMcp.openJson.claudeUser.detail': '仅 Claude Code 会话，只读导入',
  'aiMcp.openJson.codexUser.label': 'Codex config.toml',
  'aiMcp.openJson.codexUser.detail': '仅 Codex 会话，只读导入',
  'aiMcp.openJson.codexProject.label': 'Codex 项目 config.toml',
  'aiMcp.dialog.addTitle': '添加 MCP 服务器',
  'aiMcp.dialog.editTitle': '编辑 MCP 服务器',
  'aiMcp.dialog.scope': '保存到',
  'aiMcp.dialog.name': '名称',
  'aiMcp.dialog.nameEmpty': '名称不能为空。',
  'aiMcp.dialog.nameExists': '该作用域下已存在同名服务器。',
  'aiMcp.dialog.shadowsGlobal': '这将覆盖同名的用户全局服务器。',
  'aiMcp.dialog.shadowedByWorkspace': '工作区中的同名服务器优先于它。',
  'aiMcp.dialog.type': '类型',
  'aiMcp.dialog.command': '命令',
  'aiMcp.dialog.commandEmpty': 'stdio 服务器必须填写命令。',
  'aiMcp.dialog.args': '参数（每行一个）',
  'aiMcp.dialog.url': 'URL',
  'aiMcp.dialog.urlEmpty': 'http/sse 服务器必须填写 URL。',
  'aiMcp.dialog.env': '环境变量',
  'aiMcp.dialog.headers': '请求头',
  'aiMcp.dialog.enabled': '默认启用',
  'aiMcp.dialog.cancel': '取消',
  'aiMcp.dialog.save': '保存',
  'aiMcp.dialog.kvName': '名称',
  'aiMcp.dialog.kvValue': '值',
  'aiMcp.dialog.kvRemove': '删除',
  'aiMcp.dialog.kvAdd': '添加',

  // --- ACP: MCP server picker (prompt input) ---
  'acp.mcp.picker.sourceExtension': '扩展',
  'acp.mcp.picker.sourceAgentUserClaude': 'Claude 用户配置',
  'acp.mcp.picker.sourceAgentUserCodex': 'Codex 用户配置',
  'acp.mcp.picker.sourceAgentProjectCodex': 'Codex 项目配置',
  'acp.mcp.picker.affinityClaude': '仅 Claude Code 会话',
  'acp.mcp.picker.affinityCodex': '仅 Codex 会话',
  'acp.mcp.picker.sharedShort': '共享',
  'acp.mcp.picker.sharedHint': '当前 Agent 也定义了同名条目——显示的是更高优先级共享层的内容',
  'acp.mcp.ena.userTitle': '用户级默认（所有工作区生效）',
  'acp.mcp.ena.userTitleShadowed': '用户级默认（所有工作区生效）——当前被工作区设置覆盖',
  'acp.mcp.ena.wsTitleInherit': '工作区级默认（本项目优先）——继承：{state}',
  'acp.mcp.ena.wsTitleOn': '工作区级默认——本项目已启用',
  'acp.mcp.ena.wsTitleOff': '工作区级默认——本项目已禁用',
  'acp.mcp.ena.userAria': '切换用户级默认',
  'acp.mcp.ena.wsAria': '切换工作区级默认',
  'acp.mcp.state.enabled': '启用',
  'acp.mcp.state.disabled': '禁用',
  'acp.mcp.picker.defaultHint':
    '左侧复选框只作用于当前会话；人形/文件夹开关分别是用户级与工作区级默认（工作区优先，可回到继承）。',

  // --- ACP: config bar overflow ---
  'acp.config.more': '更多选项…',
  'acp.config.entryOutOfRange': '本会话有 {0} 个配置项。',

  // --- ACP: model switch context guard ---
  'acp.modelSwitch.shrink.message': '切换到 "{model}"？',
  'acp.modelSwitch.shrink.detail':
    '当前会话已使用约 {used} tokens 上下文，而 "{model}" 的上下文窗口只有约 {window} tokens。切换后下一条消息将立即触发上下文压缩，较早的细节可能丢失。如需保留完整上下文，请选择更大窗口的型号（如带 "1M context" 的选项）。',
  'acp.modelSwitch.shrink.confirm': '仍然切换',

  // --- ACP: permission card (plan review) ---
  'acp.permission.autoExecute': '此后自动执行计划',
  'acp.permission.autoExecute.tooltip':
    '计划完成后经短暂倒计时自动继续。可在设置 acp.plan.autoExecute 中调整模式。',
  'acp.permission.autoExecute.countdown': '{secs} 秒后自动执行',
  'acp.permission.steer.placeholder': '告诉 Claude 应该怎么做…',
  'acp.permission.steer.submit': '发送',
  'acp.session.forkTitle': '{title}（分叉）',
  'acp.session.cwd.tooltip': 'Agent 工作目录：{cwd}',
  'acp.session.toolCallOrphaned': '未收到结果（本轮已结束）。',
  'acp.session.toolCallUntitled': '工具调用',
  'acp.sessionTitle.noModelHint':
    '未配置会话标题模型，AI 会话将使用首条消息作为标题。选择一个模型可自动生成更贴切的标题。',
  'acp.sessionTitle.selectModel': '选择模型',
  'acp.sessionTitle.outputLimitHint':
    '会话标题生成失败：模型的思考（推理）耗尽了全部输出 token 预算，未产出任何文本。请打开 AI 设置，将该模型的 thinking 参数设为 disabled。',
  'acp.sessionTitle.openAiSettings': '打开 AI 设置',
  'acp.sessionStartFailed': '启动 Agent 会话失败：{msg}',
  'acp.sessionResumeFailed': '恢复 Agent 会话失败：{msg}',
  'acp.mcpTransportUnsupported':
    '{agent} 不支持为 {names} 配置的 MCP 传输方式，这些服务器已被跳过。',
  'acp.mcp.reloading':
    'MCP 服务器配置已变更——正在重启会话以应用。这会使模型提示缓存失效，下一轮对话可能变慢。',
  'acp.mcp.reloadFailed': '重启会话失败：{message}',
  'acp.modelContextWindowUnknown':
    '模型“{model}”的上下文窗口未知，当前按默认值管理，可能不准确。请在 aiSettings.json 的 models 下为它配置 maxInputTokens（命令：AI: Open Settings JSON）。',
  'acp.switchMode.keepPlanning': '已继续规划',
  'acp.switchMode.feedbackLabel': '你的反馈',
  'acp.syntheticDenial.badge': '上游中断',
  'acp.syntheticDenial.cardNotice':
    '上游响应异常导致工具调用被中断，并被误报为用户拒绝——这不是你的拒绝操作。请重新发起请求以继续。',
  'acp.syntheticDenialDetected':
    '上游响应异常导致工具调用被中断，并被误报为用户拒绝；子 Agent 已停止，需要重新发起请求才能继续。',
  'acp.toolCall.searchTitle': '搜索“{pattern}”',
  'acp.toolCall.openFile': '打开文件',
  'resourcePreview.openPreview': '打开预览',
  'acp.inlineDiff.openTooltip': '查看完整变更',
  'acp.inlineDiff.expandLines': '… 展开 {count} 行',
  'acp.subagentStats.costTooltip': '本地按 token 估算，实际计费可能不同',
  'acp.subagent.label': '子 Agent',
  'acp.subagent.triggerInherit': '子 Agent：继承',
  'acp.subagent.panelDesc':
    '子 Agent 使用此模型运行。该值在子进程启动时读取，修改后从下次会话开始生效。',
  'acp.subagent.inherit': '跟随主模型',
  'acp.subagent.nextSession': '下次会话生效',
  'acp.subagent.restartNow': '立即重启',
  'acp.subagent.writeFailed': '保存子 Agent 模型失败：{0}',

  // --- AI: debug ---
  'aiDebug.clear': '清空',
  'aiDebug.copyJson': '复制 JSON',
  'aiDebug.count': '{0} 个请求',
  'aiDebug.meta.duration': '耗时',
  'aiDebug.meta.modelId': '模型 ID',
  'aiDebug.meta.status': '状态',
  'aiDebug.meta.tokens': 'Tokens',
  'aiDebug.none': '尚未记录任何 AI 请求。',
  'aiDebug.replay': '重放',
  'aiDebug.replayRealtime': '实时重放',
  'aiDebug.section.error': '错误',
  'aiDebug.section.meta': '请求',
  'aiDebug.section.prompt': '提示词',
  'aiDebug.section.replay': '重放输出',
  'aiDebug.section.response': '响应',

  // --- Inline completion ---
  'inlineCompletion.error': '模型"{0}"的内联补全失败：{1}',
  'inlineCompletion.error.config': '补全模型需要配置',
  'inlineCompletion.error.disable': '禁用',
  'inlineCompletion.error.model': '所选补全模型缺失',
  'inlineCompletion.error.network': '连接提供方时发生网络错误',
  'inlineCompletion.error.quota': '额度或余额已用尽',
  'inlineCompletion.error.rateLimited': '请求频率受限，请稍后重试',
  'inlineCompletion.error.unauthorized': 'API 密钥被拒绝（请检查密钥）',

  // --- ACP: agent chat panel ---
  'acp.changes.none': '此会话尚未修改任何文件。',
  'acp.changes.noSession': '没有活跃的 Agent 会话。',
  'acp.changes.openFile': '打开文件',
  'acp.changes.openPreview': '打开预览',
  'acp.changes.viewAsList': '以列表查看',
  'acp.changes.viewAsTree': '以树查看',
  'acp.changes.inferred': '推测',
  'acp.changes.inferredTip':
    '本轮执行期间在磁盘上侦测到、但 agent 未上报的改动——可能并非 agent 所为。',
  'acp.changes.dismissInferred': '非 agent 改动——忽略',
  'acp.changes.treeLabel': '更改的文件',
  'acp.chat.forkFromTip': '从此处分叉',
  'acp.collapse.collapsed': '时间线：全部折叠 - 点击全部展开',
  'acp.collapse.default': '时间线：智能折叠 - 点击全部折叠',
  'acp.collapse.expanded': '时间线：全部展开（子 Agent 卡片最多展开一张）- 点击重置',
  'acp.confirmClose.close': '关闭',
  'acp.confirmClose.detail': '关闭将停止正在运行的 agent。',
  'acp.confirmClose.message': '会话 "{title}" 仍在运行。',
  'acp.context.remove': '移除上下文',
  'acp.contextRef.commit.description': '从提交历史中选择',
  'acp.contextRef.commit.label': 'Git 提交…',
  'acp.contextRef.commit.loadFailed': '加载 {repo} 的提交历史失败。',
  'acp.contextRef.commit.noRepo': '没有打开的 Git 仓库，无法选择提交。',
  'acp.contextRef.commit.placeholder': '从 {repo} 中选择一个提交',
  'acp.contextRef.docs.description': '位于 {path}',
  'acp.contextRef.docs.instruction':
    '{label}（文档目录，位于 {path}——请列出该目录并按需阅读其中相关的 markdown 文件）',
  'acp.contextRef.docs.label': '编辑器使用文档',
  'acp.contextRef.extensionDevDocs.label': '扩展开发文档',
  'acp.contextRef.empty': '没有匹配的上下文',
  'acp.contextRef.loading': '正在加载上下文…',
  'acp.contextRef.selection.description': '当前选区',
  'acp.cost.col.cost': '费用',
  'acp.cost.col.input': '输入',
  'acp.cost.col.model': '模型',
  'acp.cost.col.output': '输出',
  'acp.cost.estimateNote': '根据 token 用量在本地估算，实际计费可能不同。',
  'acp.cost.indicator': '会话费用 - 点击查看明细',
  'acp.cost.indicator.estimated': '预估会话费用 - 点击查看明细',
  'acp.cost.indicator.unknown': '费率未知 - 去 AI 设置为该模型填写费率',
  'acp.cost.noBreakdown': '未报告按模型拆分的明细。',
  'acp.cost.popover': '会话费用明细',
  'acp.cost.rate': '总计 {currency} {usd} · 汇率 1 USD = ¥{rate}{fallback}',
  'acp.cost.rateFallback': '（离线估算）',
  'acp.cost.rateUnknown': '总费用未知（未配置费率）',
  'acp.cost.title': '会话费用',
  'acp.cost.title.estimated': '预估会话费用',
  'acp.compaction.chip': '上下文 {pct}% · 接近自动压缩',
  'acp.compaction.chip.title': '上下文已用 {pct}% — 接近自动压缩阈值',
  'acp.compaction.failed': '上下文压缩失败',
  'acp.compaction.failedReason': '上下文压缩失败：{reason}',
  'acp.compaction.running': '正在压缩上下文…',
  'acp.compaction.success': '上下文已压缩',
  'acp.recovery.continueAfterInterrupt':
    '继续。注意：刚才连接中断导致上一轮被迫中止，并非我的操作；如果你当时有正在等待我回答的提问或确认，请不要视为我已回答、跳过或拒绝——请现在重新发起。',
  'acp.recovery.restarting': '正在重启 agent 进程…（{0}/{1}）',
  'acp.recovery.waking': '正在唤醒 Agent…（{0}/{1}）',
  'acp.recovery.retry': '重试',
  'acp.recovery.cancel': '取消',
  'acp.recovery.retrying': 'Agent 暂时不可用，正在重试…（{0}/{1}）',
  'acp.recovery.retryingIn': 'Agent 暂时不可用，{0} 秒后重试…（{1}/{2}）',
  'acp.recovery.reconnecting': '连接已断开，正在重新连接…（{0}/{1}）',
  'acp.recovery.reconnectingIn': '连接已断开，{0} 秒后重新连接…（{1}/{2}）',
  'acp.recovery.exhausted': '自动恢复失败，你可以手动重试。',
  'acp.resurrection.failed': '会话恢复失败',
  'acp.resurrection.failedReason': '会话恢复失败：{reason}',
  'acp.resurrection.running': 'Agent 无响应 — 正在从对话记录恢复会话…',
  'acp.resurrection.success': '会话已恢复',
  'acp.resurrection.successReplay': '会话已恢复 — 补发 {count} 条排队消息',
  'acp.elicitation.required': '「{field}」为必填项',
  'acp.aiFix.severity.error': '错误',
  'acp.aiFix.severity.warning': '警告',
  'acp.aiFix.severity.info': '信息',
  'acp.aiFix.severity.hint': '提示',
  'acp.aiFix.prompt.header': '修复 {path} 中的以下问题（相关代码已作为上下文附上）：',
  'acp.aiFix.prompt.more': '……以及另外 {count} 个问题',
  'acp.aiFix.prompt.instruction': '请直接对文件做最小修复。',
  'acp.aiFix.sessionTitle': 'AI 修复：{path}',
  'acp.aiFix.noCachedOptions':
    'AI 修复：该 Agent 还没有缓存的配置项——请先与它开一个会话，专用模型/思考深度才能生效。本次先使用 Agent 默认值。',
  'acp.aiFix.optionMissing':
    'AI 修复：该 Agent 不提供 {category} 配置项，配置的值「{value}」已被忽略。',
  'acp.aiFix.valueInvalid': 'AI 修复：「{value}」不是该 Agent 可选的 {category}——已忽略。',
  'acp.elicitation.minLength': '「{field}」至少需要 {limit} 个字符',
  'acp.elicitation.maxLength': '「{field}」最多允许 {limit} 个字符',
  'acp.elicitation.pattern': '「{field}」不符合要求的格式',
  'acp.elicitation.minimum': '「{field}」不能小于 {limit}',
  'acp.elicitation.maximum': '「{field}」不能大于 {limit}',
  'acp.elicitation.enumMembership': '「{field}」不是可选项之一',
  'acp.elicitation.minItems': '「{field}」至少选择 {limit} 项',
  'acp.elicitation.maxItems': '「{field}」最多选择 {limit} 项',
  'acp.elicitation.notANumber': '「{field}」必须是数字',
  'acp.elicitation.submit': '提交',
  'acp.elicitation.decline': '拒绝',
  'acp.elicitation.close': '关闭 (Esc)',
  'acp.elicitation.collapse': '折叠问题',
  'acp.elicitation.expand': '展开问题',
  'acp.elicitation.selectPlaceholder': '请选择…',
  'acp.elicitation.otherPlaceholder': '给所选选项加备注，或输入自定义回答…',
  'acp.elicitation.unsupportedMode': '不支持的信息收集模式：{mode}',
  'acp.elicitation.url.hint':
    'Agent 请求你在浏览器中打开以下链接。链接不会自动打开——请确认你信任该地址后再继续。',
  'acp.elicitation.url.open': '打开链接',
  'acp.elicitation.url.waiting': '已在浏览器中打开，等待 Agent 完成…',
  'acp.elicitation.url.done': 'Agent 已完成该流程。',
  'acp.empty': '没有活跃的 Agent 会话。',
  'acp.emptySession.chooseAgent': '选择 Agent',
  'acp.emptySession.collapse': '切换折叠状态',
  'acp.emptySession.commands': '命令',
  'acp.emptySession.context': '添加上下文',
  'acp.emptySession.focusInput': '聚焦输入框',
  'acp.emptySession.keyboard': '键盘',
  'acp.emptySession.mentions': '提及文件',
  'acp.emptySession.newSession': '新建会话',
  'acp.emptySession.nextPrevious': '下一项 / 上一项',
  'acp.emptySession.resumeSession': '恢复上一个',
  'acp.emptySession.start': '开始',
  'acp.emptySession.text': '在下方提问，或使用会话操作切换上下文。',
  'acp.emptySession.title': '此会话为空',
  'acp.emptySession.topBottom': '顶部 / 底部',
  'acp.find.close': '关闭',
  'acp.find.count': '{current} / {total}',
  'acp.find.next': '下一个匹配项',
  'acp.find.noResults': '无结果',
  'acp.find.placeholder': '查找',
  'acp.find.previous': '上一个匹配项',
  'acp.foreignSession.another': '另一个工作树',
  'acp.foreignSession.loading': '正在加载会话历史（只读）…',
  'acp.foreignSession.note': '该会话属于另一个工作树。请在其自身上下文中打开以继续工作。',
  'acp.foreignSession.openInNewWindow': '在新窗口打开工作树',
  'acp.foreignSession.readOnlyNote': '只读 —— 该会话属于 {branch}。请在其自身上下文中打开以继续。',
  'acp.foreignSession.switch': '切换到此窗口',
  'acp.history.popover.label': '输入历史',
  'acp.image.close': '关闭预览',
  'acp.image.copied': '已复制',
  'acp.image.copy': '复制图片',
  'acp.image.preview': '图片预览',
  'acp.image.reject.count': '附加的图片过多（最多 {n} 张）。',
  'acp.image.reject.size': '图片过大（最大 {mb} MB）。',
  'acp.image.reject.type': '不支持的图片类型。请使用 PNG、JPEG、WebP 或 GIF。',
  'acp.image.remove': '移除图片',
  'acp.image.unsupported': '当前 Agent 不支持图片输入。',
  'acp.deepLink.mcpMissing':
    '深链接请求了未配置的 MCP 服务器：{names}。会话已在不包含这些服务器的情况下创建。',
  'acp.mcp.none': '此会话没有配置 MCP 服务器。',
  'acp.mcp.noSession': '没有活跃的 Agent 会话。',
  'acp.mcp.openSettings': '配置 MCP 服务器',
  'acp.mcp.input': '输入',
  'acp.mcp.list': 'MCP 服务器',
  'acp.mcp.output': '输出',
  'acp.mcp.picker.cacheHint': '更改服务器会重启会话，并使模型的提示词缓存失效。',
  'acp.mcp.picker.custom': '自定义选择',
  'acp.mcp.picker.inherit': '跟随默认',
  'acp.mcp.picker.openSettings': '配置 MCP 服务器…',
  'acp.mcp.picker.reset': '重置',
  'acp.mcp.picker.sourceGlobal': '全局',
  'acp.mcp.picker.sourceProject': '项目',
  'acp.mcp.picker.title': '本会话启用的 MCP 服务器',
  'acp.mention.empty': '没有匹配的文件',
  'acp.mention.loading': '正在扫描文件…',
  'acp.mention.pickFile.open': '提及',
  'acp.mention.pickFile.title': '选择要提及的文件',
  'acp.mention.pickFolder.open': '提及',
  'acp.mention.pickFolder.title': '选择要提及的文件夹',
  'acp.newSession': '新建会话',
  'acp.newSession.titled': '新建 {name} 会话',
  'acp.newSession.withScope': '新建会话（选择作用域）',
  'acp.notify.completed.title': 'Agent 已完成任务',
  'acp.notify.errored.title': 'Agent 运行失败',
  'acp.notify.permission.title': 'Agent 需要你的许可',
  'acp.notify.question.title': 'Agent 有一个问题',
  'acp.plan.title': '计划',
  'acp.prompt.confirmShort.cancel': '继续编辑',
  'acp.prompt.confirmShort.detail': '你的第一条消息相当简短。确定要发送吗？',
  'acp.prompt.confirmShort.message': '发送这条短消息？',
  'acp.prompt.confirmShort.send': '发送',
  'acp.prompt.placeholder': '询问 Agent…',
  'acp.refreshSessions': '刷新会话列表',
  'acp.selectAgent': '选择 Agent…',
  'acp.selectAgent.titled': '选择 Agent…（当前：{name}）',
  'acp.send.idle': '发送 (Enter)',
  'acp.send.running': '运行中 · 发送以引导',
  'acp.session.authRequired': '此 Agent 需要先完成认证。',
  'acp.session.historyOverflow': '会话历史过大，仅加载了部分历史。',
  'acp.session.memoryTrimmed': '为保护内存已释放部分内容；最新的输出已保留。',
  'acp.session.openAuth': '打开 Agent 设置',
  'acp.session.resumeFailed': '恢复 Agent 会话失败：{error}',
  'acp.session.resumeWriterLock':
    '该会话正在被另一个 Codex 客户端使用，请先在官方 Codex 应用中关闭后再试',
  'acp.session.resuming': '正在恢复 Agent 会话...',
  'acp.session.retry': '重试',
  'acp.session.resumePausedAfterOom': '上次加载此会话时窗口因内存不足崩溃，已暂停自动恢复。',
  'acp.session.resumeAnyway': '继续加载',
  'acp.session.runningTime': '会话运行时间',
  'acp.session.wakeFailed': '唤醒 Agent 会话失败，改动未应用',
  'acp.sessions.empty': '还没有会话。',
  'acp.sessions.foreignWorktree': '属于另一个工作树',
  'acp.sessions.list': '会话',
  'acp.sessions.noMatch': '没有匹配的会话。',
  'acp.sessions.connecting': '正在连接…',
  'acp.sessions.dormant': '会话已休眠以节省内存 — 使用时会自动唤醒',
  'acp.sessions.startFailed': '启动失败',
  'acp.sessions.remove': '移除会话',
  'acp.sessions.removeConfirm': '删除这个会话？',
  'acp.sessions.removeConfirmDetail': '这将删除该会话及其历史记录。',
  'acp.sessions.removeConfirmOk': '删除',
  'acp.sessions.removeConfirmCancel': '取消',
  'acp.sessions.removeNeverAsk': '不再询问',
  'acp.sessions.removeMenu': '删除会话',
  'acp.sessions.rename': '重命名会话',
  'acp.sessions.renameMenu': '重命名会话',
  'acp.sessions.search': '搜索会话',
  'acp.sessions.searchClose': '关闭搜索',
  'acp.sessions.archive': '归档会话 (Del)',
  'acp.sessions.unarchive': '取消归档会话 (Shift+Del)',
  'acp.sessions.pin': '置顶会话',
  'acp.sessions.unpin': '取消置顶会话',
  'acp.sessions.pinned': '已置顶',
  'acp.sessions.archiveMenu': '归档会话',
  'acp.sessions.unarchiveMenu': '取消归档会话',
  'acp.sessions.pinMenu': '置顶会话',
  'acp.sessions.unpinMenu': '取消置顶会话',
  'acp.sessions.revealTranscript': '打开会话所在位置',
  'acp.sideTask.badge': '侧边任务',
  'acp.sideTask.bar': '侧边任务',
  'acp.sideTask.forkFailed': '创建侧边聊天失败：{message}',
  'acp.sideTask.forkUnsupported': '该 Agent 不支持创建侧边任务。',
  'acp.sideTask.open.placeholder': '打开一条侧边任务',
  'acp.sideTask.parent': '父会话',
  'acp.sideTask.quoteChip': '{count} 个已选文本片段',
  'acp.sideTask.remove': '删除侧边任务',
  'acp.sideTask.removeConfirm': '删除这个侧边任务？',
  'acp.sideTask.removeConfirmDetail': '这将删除该侧边任务及其历史记录。',
  'acp.sideTask.removeConfirmDetailNested': '同时会删除它的 {count} 个下级侧边任务。',
  'acp.sideTask.untitled': '新建侧边任务',
  'acp.filter.archived': '已归档',
  'acp.filter.menu': '筛选会话',
  'acp.filter.reset': '重置',
  'acp.filter.sortByCreated': '按创建时间排序',
  'acp.filter.sortByUpdated': '按更新时间排序',
  'acp.filter.status.completed': '已完成',
  'acp.filter.status.failed': '失败',
  'acp.filter.status.inProgress': '进行中',
  'acp.filter.status.inputNeeded': '需要输入',
  'acp.slash.empty': '没有匹配的命令',
  'acp.sticky.collapse': '折叠',
  'acp.stop': '停止',
  'acp.subscriptionUsage.extraUsage': '额外用量：已开启',
  'acp.subscriptionUsage.extraUsageSpend': '额外用量：{used} / {limit} {currency}',
  'acp.subscriptionUsage.expired': '已过期',
  'acp.subscriptionUsage.expiresInDays': '最早 {days} 天后过期',
  'acp.subscriptionUsage.expiresInHours': '最早 {hours} 小时后过期',
  'acp.subscriptionUsage.expiresInHoursMinutes': '最早 {hours} 小时 {minutes} 分后过期',
  'acp.subscriptionUsage.expiresInMinutes': '最早 {minutes} 分钟后过期',
  'acp.subscriptionUsage.freshFooter': '更新于 {at}',
  'acp.subscriptionUsage.indicator': '订阅用量：剩余 {pct} - 点击查看详情',
  'acp.subscriptionUsage.indicator.stale': '订阅用量（数据截止 {at}）：剩余 {pct} - 点击查看详情',
  'acp.subscriptionUsage.indicator.rolledOver': '订阅用量 —— 配额已重置；点击刷新',
  'acp.subscriptionUsage.noWindows': '未上报任何限额窗口。',
  'acp.subscriptionUsage.popover': '订阅用量详情',
  'acp.subscriptionUsage.popover.title': '订阅用量',
  'acp.subscriptionUsage.popover.titlePlan': '订阅用量 · {plan}',
  'acp.subscriptionUsage.reset.action': '重置限额',
  'acp.subscriptionUsage.reset.alreadyRedeemed': '该重置额度已被使用。',
  'acp.subscriptionUsage.reset.confirm': '要使用一张限额重置额度吗？',
  'acp.subscriptionUsage.reset.confirmButton': '使用',
  'acp.subscriptionUsage.reset.confirmDetail': '将消耗你现有 {count} 张额度中的一张，且不可撤销。',
  'acp.subscriptionUsage.reset.done': '限额已重置。',
  'acp.subscriptionUsage.reset.failed': '使用重置额度失败。',
  'acp.subscriptionUsage.reset.noCredit': '当前账户没有可用的重置额度。',
  'acp.subscriptionUsage.reset.nothingToReset': '当前没有触顶的限额，无需重置。',
  'acp.subscriptionUsage.reset.unavailable': '没有可用的 Agent 连接，请先发送一条消息。',
  'acp.subscriptionUsage.resetCredits': '重置额度：{count}',
  'acp.subscriptionUsage.resetCreditsExpiring': '重置额度：{count} · {when}',
  'acp.subscriptionUsage.resetsAt': '{at} 重置',
  'acp.subscriptionUsage.resetsInHours': '{hours} 小时后重置',
  'acp.subscriptionUsage.resetsInHoursMinutes': '{hours} 小时 {minutes} 分后重置',
  'acp.subscriptionUsage.resetsInMinutes': '{minutes} 分钟后重置',
  'acp.subscriptionUsage.resetsNow': '正在重置',
  'acp.subscriptionUsage.staleFooter': '数据截止 {at}（无活跃会话）',
  'acp.subscriptionUsage.window.daily': '每日',
  'acp.subscriptionUsage.window.days': '{days} 天',
  'acp.subscriptionUsage.window.generic': '用量',
  'acp.subscriptionUsage.window.hours': '{hours} 小时',
  'acp.subscriptionUsage.window.minutes': '{minutes} 分钟',
  'acp.subscriptionUsage.window.weekly': '每周',
  'acp.subscriptionUsage.window.weeklyOauthApps': '每周 · 第三方应用',
  'acp.subscriptionUsage.window.weeklyOpus': '每周 · Opus',
  'acp.subscriptionUsage.window.weeklySonnet': '每周 · Sonnet',
  'acp.accountUsage.fetchedAt': '更新于 {at}',
  'acp.accountUsage.indicator': '账号额度 - 点击查看详情',
  'acp.accountUsage.kind.balance': '余额',
  'acp.accountUsage.kind.quota': '配额',
  'acp.accountUsage.kind.subscription': '订阅',
  'acp.accountUsage.limit': '额度上限：{amount}',
  'acp.accountUsage.popover': '账号额度详情',
  'acp.accountUsage.refresh': '刷新',
  'acp.accountUsage.remaining': '剩余：{amount}',
  'acp.accountUsage.unavailable': '不可用',
  'acp.accountUsage.unavailable.tooltip':
    '该 provider 声明了账号用量来源，但当前拉取不到权威数字；这里不会用本地估算值代替。',
  'acp.accountUsage.used': '已用：{amount}',
  'acp.terminal.collapse': '折叠',
  'acp.terminal.expand': '展开',
  'acp.usage.cost': '费用：{amount} {currency}',
  'acp.usage.tokens': '上下文：{used} / {size} tokens（{pct}%）',
  'acp.usage.unavailable': '上下文用量不可用',
  'acp.userMessage.autoRetry': '自动重试',
  'acp.userMessage.collapse': '折叠',
  'acp.userMessage.expand': '展开',
  'acp.userMessage.fork': '从此处分叉',
  'acp.userMessage.rewind': '回退到此处',

  // --- Agent: state & placeholders ---
  'agent.addSelection.pickTarget': '选择要添加选区的 Agent 聊天',
  'agent.bookmark.gone': '（不可用）',
  'agent.configOption.current': '当前',
  'agent.configOption.failed': '应用选项失败：{error}',
  'agent.fork.failed': '分叉失败：{error}',
  'agent.fork.foreign': '请先在其所属工作树中打开该会话，再执行分叉。',
  'agent.fork.running': '请等待当前回合结束后再分叉。',
  'agent.fork.unsupported': '当前会话无法分叉（没有活跃会话，或该 Agent 不支持）。',
  'agent.history.clear.confirm': '清除全部 {count} 条会话记录？',
  'agent.history.clear.detail': '这只会移除本地历史索引。对话仍保存在 Agent 侧，直到它自行清理。',
  'agent.history.clear.done': 'Agent 会话历史已清除。',
  'agent.history.clear.primary': '清除',
  'agent.history.daysAgo': '{count} 天前',
  'agent.history.detail': '{time} · {cwd}',
  'agent.history.empty': '没有之前的 Agent 会话。',
  'agent.history.hoursAgo': '{count} 小时前',
  'agent.history.justNow': '刚刚',
  'agent.history.minutesAgo': '{count} 分钟前',
  'agent.listBookmarks.placeholder': '跳转到某个会话书签',
  'agent.noSession': '没有活跃的 Agent 会话。',
  'agent.newSessionWithScope.chooseFolder': '选择文件夹…',
  'agent.newSessionWithScope.dialogTitle': '选择 Agent 工作目录',
  'agent.newSessionWithScope.failed': '无法创建 Agent 会话：{message}',
  'agent.newSessionWithScope.placeholder': '为新会话选择工作目录',
  'agent.rename.empty': '标题不能为空',
  'agent.rename.prompt': '输入该会话的新标题',
  'agent.resumeSession.placeholder': '恢复之前的 Agent 会话',
  'agent.revealSession.noTranscript': '该会话没有可显示的记录文件。',
  'agent.rewind.cannot': '无法将本会话回退到该位置。',
  'agent.rewind.confirm': '将对话回退到这条消息？',
  'agent.rewind.detail.base': '这条消息之后的对话将被移除。',
  'agent.rewind.detail.files': '将回滚 {files} 个文件（+{insertions} / -{deletions}）。',
  'agent.rewind.detail.noFiles': '不会回滚任何文件更改。',
  'agent.rewind.discard': '放弃更改并回退',
  'agent.rewind.failed': '回退失败：{error}',
  'agent.rewind.keep': '保留更改并回退',
  'agent.rewind.previewFailed': '无法预览回退：{error}',
  'agent.rewind.primary': '回退',
  'agent.scope.detected': '检测到的项目',
  'agent.scope.configured': '已配置的项目',
  'agent.scope.workspace': '工作区',
  'agent.selectAgent.placeholder': '选择默认 ACP Agent',
  'agent.selectAgent.unavailable': '未安装（PATH 中找不到命令）',
  'agent.selectMode.notFound': '活跃 Agent 未提供会话模式选择器。',
  'agent.selectMode.placeholder': '选择会话模式',
  'agent.selectModel.notFound': '活跃 Agent 未提供模型选择器。',
  'agent.selectModel.placeholder': '选择模型',
  'agent.selectThoughtLevel.notFound': '活跃 Agent 未提供思考级别开关。',
  'agent.selectThoughtLevel.placeholder': '选择思考深度',
  'agent.configureAiFix.followDefault': '（默认）跟随 Agent 默认',
  'agent.configureAiFix.noOptions':
    '该 Agent 还没有缓存的配置项——请先与它开一个会话，然后在这里配置模型/思考深度/模式。',
  'agent.configureAiFix.pickAgent': '选择 AI 修复使用的 Agent',
  'agent.switchSession.placeholder': '切换到任意窗口中的会话',
  'agent.switchSession.untitled': '未命名',

  // --- Agent: settings (Claude) ---
  'agentSettings.alwaysThinking': '始终启用思考',
  'agentSettings.alwaysThinking.desc': '关闭时会禁用思考（Settings.alwaysThinkingEnabled）。',
  'agentSettings.auth.apiKey': 'Anthropic API 密钥',
  'agentSettings.auth.applyGatewayError':
    '无法应用此网关凭据 — 其 provider 缺少 base URL 或 API 密钥。',
  'agentSettings.auth.cancel': '取消',
  'agentSettings.auth.delete': '删除',
  'agentSettings.auth.edit': '编辑',
  'agentSettings.auth.form.kind': '类型',
  'agentSettings.auth.form.label': '名称',
  'agentSettings.auth.form.label.ph': '例如 Personal、Work gateway',
  'agentSettings.auth.form.model': '模型',
  'agentSettings.auth.form.model.none': '使用默认',
  'agentSettings.auth.form.oneM.tip':
    '请求 100 万 token 上下文变体 —— 写入 settings.json 的模型 id 会追加 [1m]。',
  'agentSettings.auth.form.provider': '供应商',
  'agentSettings.auth.form.provider.none': '选择一个 provider…',
  'agentSettings.auth.form.subagentModel': '子 Agent 模型',
  'agentSettings.auth.form.subagentModel.none': '未设置',
  'agentSettings.auth.gateway': '自定义网关 / 认证 token',
  'agentSettings.auth.externalCredential': '当前生效的是在编辑器之外配置的凭据，其费用无法归属。',
  'agentSettings.connectivity.checking': '正在检测连通性…',
  'agentSettings.connectivity.fail': '网关不可达',
  'agentSettings.connectivity.ok': '网关可连通',
  'agentSettings.auth.provider.add': '添加 provider…',
  'agentSettings.auth.provider.checking': '检测中…',
  'agentSettings.auth.provider.idle': '未检测',
  'agentSettings.auth.provider.missingApiKey':
    '此供应商没有 API 密钥。请在 AI 设置（供应商配置）中设置。',
  'agentSettings.auth.provider.missingBaseUrl':
    '此供应商没有 base URL。请在 AI 设置（供应商配置）中设置。',
  'agentSettings.auth.none.desc': '未选择 provider —— 可用时将使用下方的 Claude OAuth 登录。',
  'agentSettings.auth.provider.none':
    '还没有声明兼容协议的供应商。请在 AI 设置中添加一个，然后回到这里选择。',
  'agentSettings.auth.provider.noneCompatible':
    '你有 {count} 个供应商，但没有一个声明 {protocol} 协议。请在 AI 设置里调整它的协议映射（protocolMap），或给它补一个该协议的模型。',
  'agentSettings.auth.provider.configure': '打开 AI 设置…',
  'agentSettings.auth.provider.incompatibleOption': '不兼容',
  'agentSettings.auth.provider.incompatible':
    '此供应商没有声明 {protocol} 协议。请调整它的协议映射（protocolMap），或在 AI 设置里给它补一个该协议的模型。',
  'agentSettings.auth.provider.ok': '已连接 · {count} 个模型',
  'agentSettings.auth.provider.okReachable': '已连接',
  'agentSettings.auth.provider.test': '测试',
  'agentSettings.auth.provider.verifyFail': '连接失败',
  'agentSettings.auth.library': '已保存凭据',
  'agentSettings.auth.library.add': '添加凭据',
  'agentSettings.auth.library.empty':
    '还没有保存的凭据。添加 API 密钥或网关 token + URL 后，可随时在它们之间切换。',
  'agentSettings.auth.login': '登录 Claude',
  'agentSettings.auth.login.activated': '当前正在使用你的 Claude 登录。',
  'agentSettings.auth.login.console': '使用 Anthropic Console 登录',
  'agentSettings.auth.login.expired': '登录已过期 - 请重新登录。',
  'agentSettings.auth.login.hint':
    '打开终端并运行 Claude 登录流程。按提示完成后，再启动 Agent 会话。',
  'agentSettings.auth.login.overridden': '你已登录，但当前优先使用了已保存的凭据。',
  'agentSettings.auth.login.remoteHint':
    '在远程主机的终端中运行 `claude auth login`，要求其 PATH 中有 claude CLI。',
  'agentSettings.auth.login.refresh': '刷新',
  'agentSettings.auth.login.refreshing': '正在刷新…',
  'agentSettings.auth.login.setCurrent': '使用此登录',
  'agentSettings.auth.login.signedIn': '已登录',
  'agentSettings.auth.login.signedInPlan': '已登录（{plan}）',
  'agentSettings.auth.login.signedOut': '未登录',
  'agentSettings.auth.login.subscription': '使用 Claude 订阅登录',
  'agentSettings.auth.subscription': '使用 Claude 订阅登录',
  'agentSettings.auth.subscription.desc':
    '使用下方共享的 Claude OAuth 登录 —— 不会写入任何凭据环境变量。',
  'agentSettings.auth.title': '身份验证',
  'agentSettings.auth.path.prefix': '活跃凭据存储于',
  'agentSettings.auth.save': '保存',
  'agentSettings.auth.use': '使用',
  'agentSettings.autoCompact': '自动压缩窗口',
  'agentSettings.availableModels': '允许的模型',
  'agentSettings.availableModels.add': '添加模型 id 后按 Enter',
  'agentSettings.availableModels.desc':
    '将模型选择器限制为这些 id（Settings.availableModels）。留空表示允许所有模型。',
  'agentSettings.availableModels.remove': '移除 {model}',
  'agentSettings.category.auth': '认证',
  'agentSettings.category.binary': '二进制文件',
  'agentSettings.category.env': '高级（env）',
  'agentSettings.category.model': '模型与思考',
  'agentSettings.claude.nav': 'Claude 设置分类',
  'agentSettings.customEnv': '自定义环境变量',
  'agentSettings.customEnv.add': '添加变量',
  'agentSettings.customEnv.desc': '额外写入 settings.json env 区块的条目，会在 Agent 启动时应用。',
  'agentSettings.customEnv.key': 'KEY',
  'agentSettings.customEnv.remove': '移除 {key}',
  'agentSettings.customEnv.value': 'value',
  'agentSettings.effortLevel': '投入级别',
  'agentSettings.effortLevel.default': '（默认）',
  'agentSettings.language': '语言',
  'agentSettings.language.desc': 'Agent 回复使用的首选语言（例如 "japanese"、"chinese"）。',
  'agentSettings.login.binFailed': '准备 Claude 失败：{error}',
  'agentSettings.login.noTerminal': '无法打开终端。',
  'agentSettings.login.preparing': '正在准备 Claude…',
  'agentSettings.model': '默认模型',
  'agentSettings.model.desc': '覆盖 Agent 使用的模型（Settings.model）。留空则使用默认值。',
  'agentSettings.noSettings': '此 agent 暂无可在编辑器中配置的设置。',
  'agentSettings.openConfigFile': '打开 {path}',
  'agentSettings.promptCaching': '启用 1 小时提示词缓存',
  'agentSettings.showThinkingSummaries': '显示思考摘要',
  'agentSettings.showThinkingSummaries.desc':
    '显示摘要后的思考内容（Settings.showThinkingSummaries）。',
  'agentSettings.thinking': '思考',

  // --- Binary: Claude ---
  'binaryPanel.customPath': '可执行文件路径',
  'binaryPanel.remoteNotice':
    '远程工作区：此页面管理远端主机上的 Claude 二进制文件，将下载到远端服务器的数据目录。',
  'binaryPanel.source.custom': '自定义路径',
  'binaryPanel.source.custom.desc': '指向特定的 Claude 可执行文件，适用于测试或多版本场景。',
  'binaryPanel.source.download': '自动下载（推荐）',
  'binaryPanel.source.download.desc': '首次使用时自动下载 Claude 二进制文件到用户数据目录。',
  'binaryPanel.source.system': '系统 PATH',
  'binaryPanel.source.system.desc': '使用 PATH 中找到的 `claude` 可执行文件（需自行管理更新）。',
  'binaryPanel.source.title': '二进制文件来源',
  'binaryPanel.upgrade.error': '升级 Claude 二进制文件失败：{message}',
  'binaryPanel.upgrade.success': 'Claude 二进制文件已切换到 {version}。',
  'binaryPanel.version.chooseHint':
    '内置版本（{bundled}）已验证与此构建的 ACP SDK 匹配，最稳妥。最新版本（{latest}）包含新功能，但可能尚未随此构建充分测试。',
  'binaryPanel.version.download': '下载 {version}',
  'binaryPanel.version.downloadReady': '安装 {version}（本地已有，点击即切换）',
  'binaryPanel.version.downloadedLocally': '本地已有：{versions}',
  'binaryPanel.version.downloading': '正在下载 {version}…',
  'binaryPanel.version.downloading.mb': '正在下载 {version}… {mb} MB',
  'binaryPanel.version.downloading.pct': '正在下载 {version}… {pct}%',
  'binaryPanel.version.installed': '已安装：{version}',
  'binaryPanel.version.latest': '最新版本：{version}',
  'binaryPanel.version.latestUnavailable': '最新版本：不可用（网络错误）',
  'binaryPanel.version.loadError': '版本信息不可用：{message}',
  'binaryPanel.version.loading': '正在加载版本信息…',
  'binaryPanel.version.notDownloaded': '尚未下载',
  'binaryPanel.version.revert': '回退到 {version}',
  'binaryPanel.version.revertHint':
    '已安装版本与本构建 ACP SDK 验证过的版本不一致，若 agent 无法启动或行为异常，可回退到验证过的版本。',
  'binaryPanel.version.revertReady': '回退到 {version}（本地已有，点击即切换）',
  'binaryPanel.version.sdk': 'ACP SDK：{version}',
  'binaryPanel.version.title': '版本',
  'binaryPanel.version.upgrade': '升级到 {version}',
  'binaryPanel.version.upgradeReady': '切换到 {version}（本地已有，点击即切换）',
  'claudeBinary.error.unsupportedPlatform': '当前平台不支持 Claude 二进制文件：{platform}',
  'claudeBinary.error.noCustomPath': 'Claude 二进制文件：已选择自定义来源，但未配置路径。',
  'claudeBinary.error.customPathNotFound': '在配置的路径中找不到 Claude 二进制文件：{path}',
  'claudeBinary.error.notNativeExecutable':
    'Claude 二进制文件路径不是原生 Windows 可执行文件：{path}。请将 `acp.claude.executablePath` 指向包内的 bin/claude.exe。',
  'claudeBinary.error.noSystemBinary':
    'PATH 中找不到系统 `claude` 可执行文件。请安装 Claude Code，或将 `acp.claude.source` 切换为 "download"。',
  'claudeBinary.error.downloadNotAllowed':
    'Claude 二进制文件尚未下载——后台探测不会触发下载；请启动一个 Claude 会话或显式下载以获取。',
  'claudeBinary.error.tarballMissingBinary': '压缩包 {pkg}@{version} 中不包含 {binName}',
  'claudeBinary.error.fetchMetadataFailed': '获取 {pkg}@{version} 元数据失败：HTTP {status}',
  'claudeBinary.error.noTarballUrl': '{pkg}@{version} 的注册表元数据中没有压缩包 URL',
  'claudeBinary.error.connectTimeout': '连接 {url} 超时',
  'claudeBinary.error.downloadFailed': '下载 {url} 失败：HTTP {status}',
  'claudeBinary.error.integritySha512': '{url} 的完整性校验失败（sha512 不匹配）',
  'claudeBinary.error.integritySha1': '{url} 的完整性校验失败（sha1 不匹配）',
  'claudeBinary.error.missingSdkVersion': '{path} 处的 claude-binary.json 缺少 sdkVersion',

  // --- Binary: Codex ---
  'codexBinaryPanel.customPath': '可执行文件路径',
  'codexBinaryPanel.remoteNotice':
    '远程工作区：此页面管理远端主机上的 codex 二进制文件，将下载到远端服务器的数据目录。',
  'codexBinaryPanel.source.custom': '自定义路径',
  'codexBinaryPanel.source.custom.desc':
    '指向特定的 codex-acp 可执行文件，适用于测试或多版本场景。',
  'codexBinaryPanel.source.download': '自动下载（推荐）',
  'codexBinaryPanel.source.download.desc':
    '首次使用时自动下载 codex-acp 二进制文件到用户数据目录。',
  'codexBinaryPanel.source.system': '系统 PATH',
  'codexBinaryPanel.source.system.desc':
    '使用 PATH 中找到的 `codex-acp` 可执行文件（需自行管理更新）。',
  'codexBinaryPanel.source.title': '二进制文件来源',
  'codexBinaryPanel.upgrade.error': '升级 codex-acp 二进制文件失败：{message}',
  'codexBinaryPanel.upgrade.success': 'codex-acp 二进制文件已切换到 {version}。',
  'codexBinaryPanel.version.chooseHint':
    '锁定版本（{bundled}）是此构建跟随且已知可用的版本，最稳妥。最新版本（{latest}）包含新功能，但可能尚未随此构建充分测试。',
  'codexBinaryPanel.version.download': '下载 {version}',
  'codexBinaryPanel.version.downloadReady': '安装 {version}（本地已有，点击即切换）',
  'codexBinaryPanel.version.downloadedLocally': '本地已有：{versions}',
  'codexBinaryPanel.version.downloading': '正在下载 {version}…',
  'codexBinaryPanel.version.downloading.mb': '正在下载 {version}… {mb} MB',
  'codexBinaryPanel.version.downloading.pct': '正在下载 {version}… {pct}%',
  'codexBinaryPanel.version.installed': '已安装：{version}',
  'codexBinaryPanel.version.latest': '最新版本：{version}',
  'codexBinaryPanel.version.latestUnavailable': '最新版本：不可用（网络错误）',
  'codexBinaryPanel.version.loadError': '版本信息不可用：{message}',
  'codexBinaryPanel.version.loading': '正在加载版本信息…',
  'codexBinaryPanel.version.notDownloaded': '尚未下载',
  'codexBinaryPanel.version.pinned': '锁定的 codex-acp：{version}',
  'codexBinaryPanel.version.revert': '回退到 {version}',
  'codexBinaryPanel.version.revertHint':
    '已安装版本与本构建锁定跟随的版本不一致，若 agent 无法启动或行为异常，可回退到锁定版本。',
  'codexBinaryPanel.version.revertReady': '回退到 {version}（本地已有，点击即切换）',
  'codexBinaryPanel.version.title': '版本',
  'codexBinaryPanel.version.upgrade': '升级到 {version}',
  'codexBinaryPanel.version.upgradeReady': '切换到 {version}（本地已有，点击即切换）',
  'codexBinary.error.unsupportedPlatform': '当前平台不支持 codex 二进制文件：{platform}',
  'codexBinary.error.noCustomPath': 'Codex 二进制文件：已选择自定义来源，但未配置路径。',
  'codexBinary.error.customPathNotFound': '在配置的路径中找不到 Codex 二进制文件：{path}',
  'codexBinary.error.noSystemBinary':
    'PATH 中找不到系统 `codex` 可执行文件。请安装它，或将 `acp.codex.source` 切换为 "download"。',
  'codexBinary.error.downloadNotAllowed':
    'Codex 二进制文件尚未下载——后台探测不会触发下载；请启动一个 Codex 会话或显式下载以获取。',
  'codexBinary.error.tarballMissingBinary':
    '压缩包 {pkg}@{version} 中不包含 vendor/{triple}/bin/{binName}',
  'codexBinary.error.fetchMetadataFailed': '获取 {pkg}@{version} 元数据失败：HTTP {status}',
  'codexBinary.error.noTarballUrl': '{pkg}@{version} 的注册表元数据中没有压缩包 URL',
  'codexBinary.error.connectTimeout': '连接 {url} 超时',
  'codexBinary.error.downloadFailed': '下载 {url} 失败：HTTP {status}',
  'codexBinary.error.integritySha512': '{url} 的完整性校验失败（sha512 不匹配）',
  'codexBinary.error.integritySha1': '{url} 的完整性校验失败（sha1 不匹配）',

  // --- Codex settings ---
  'codexSettings.approval': '审批策略',
  'codexSettings.approval.default': '（默认）',
  'codexSettings.approval.desc':
    'config.toml `approval_policy` - Codex 在运行命令前何时暂停询问。交互式工作使用 "on-request"，无人值守运行使用 "never"。',
  'codexSettings.auth.apiKey': 'OpenAI API 密钥',
  'codexSettings.auth.applyGatewayError':
    '无法应用此网关凭据 — 其 provider 缺少 base URL 或 API 密钥。',
  'codexSettings.auth.cancel': '取消',
  'codexSettings.auth.delete': '删除',
  'codexSettings.auth.edit': '编辑',
  'codexSettings.auth.form.kind': '类型',
  'codexSettings.auth.form.label': '名称',
  'codexSettings.auth.form.label.ph': '例如 Personal、Work gateway',
  'codexSettings.auth.form.model': '模型',
  'codexSettings.auth.form.model.none': '使用默认',
  'codexSettings.auth.form.provider': 'Provider 实例',
  'codexSettings.auth.gateway': '兼容网关（密钥 + URL）',
  'codexSettings.auth.externalCredential': '当前生效的是在编辑器之外配置的凭据，其费用无法归属。',
  'codexSettings.auth.library': '已保存凭据',
  'codexSettings.auth.library.add': '添加凭据',
  'codexSettings.auth.library.empty':
    '还没有保存的凭据。添加 OpenAI API 密钥或兼容网关（密钥 + base URL）后，可随时在它们之间切换。',
  'codexSettings.auth.login': '登录 ChatGPT',
  'codexSettings.auth.login.activated': '当前正在使用你的 ChatGPT 登录。',
  'codexSettings.auth.login.expired': '登录已过期 - 请重新登录。',
  'codexSettings.auth.login.hint':
    '打开终端并运行 `codex login`（需要官方 Codex CLI 已在 PATH 中）。按提示完成后，再启动 Agent 会话。',
  'codexSettings.auth.login.overridden': '你已登录，但当前优先使用了已保存的凭据。',
  'codexSettings.auth.login.refresh': '刷新',
  'codexSettings.auth.login.refreshing': '正在刷新…',
  'codexSettings.auth.login.setCurrent': '使用此登录',
  'codexSettings.auth.login.signedIn': '已登录',
  'codexSettings.auth.login.signedInPlan': '已登录（{plan}）',
  'codexSettings.auth.login.signedOut': '未登录',
  'codexSettings.auth.login.start': '登录 ChatGPT',
  'codexSettings.auth.path.credentialsPrefix': '；凭据位于',
  'codexSettings.auth.path.settingsPrefix': '设置位于',
  'codexSettings.auth.save': '保存',
  'codexSettings.auth.subscription': '使用 ChatGPT 订阅登录',
  'codexSettings.auth.subscription.desc':
    '使用下方共享的 ChatGPT 登录 —— 内置的 openai provider 依赖这些 OAuth token 运行。',
  'codexSettings.auth.title': '身份验证',
  'codexSettings.auth.use': '使用',
  'codexSettings.category.advanced': '高级',
  'codexSettings.category.auth': '认证',
  'codexSettings.category.binary': '二进制文件',
  'codexSettings.category.model': '模型与推理',
  'codexSettings.category.safety': '审批与沙箱',
  'codexSettings.credentialStore': '凭据存储',
  'codexSettings.credentialStore.default': '（默认）',
  'codexSettings.credentialStore.desc':
    'config.toml `cli_auth_credentials_store` - Codex 保存凭据的位置："file"（auth.json）、"keyring"（系统存储）或 "auto"。',
  'codexSettings.customKeys': '其他配置键',
  'codexSettings.customKeys.add': '添加键',
  'codexSettings.customKeys.desc':
    '额外写入 config.toml 顶层的键。值以文本存储；嵌套表不会显示在这里。',
  'codexSettings.customKeys.key': 'key',
  'codexSettings.customKeys.remove': '移除 {key}',
  'codexSettings.customKeys.value': 'value',
  'codexSettings.effort': '推理投入',
  'codexSettings.effort.default': '（默认）',
  'codexSettings.effort.desc':
    'config.toml `model_reasoning_effort`。更高投入会花费更多思考成本。仅适用于 Responses API。',
  'codexSettings.hideReasoning': '隐藏 Agent 推理',
  'codexSettings.login.noTerminal': '无法打开终端。',
  'codexSettings.model': '模型',
  'codexSettings.model.desc': 'Codex 使用的模型（config.toml `model`）。留空则使用 Codex 默认值。',
  'codexSettings.nav': 'Codex 设置分类',
  'codexSettings.provider': '模型提供方',
  'codexSettings.provider.desc':
    '提供方 id（config.toml `model_provider`）。默认是 "openai"；必须匹配内置 id 或 `[model_providers.<id>]` 条目。',
  'codexSettings.reasoning': '推理',
  'codexSettings.sandbox': '沙箱模式',
  'codexSettings.sandbox.default': '（默认）',
  'codexSettings.sandbox.desc':
    'config.toml `sandbox_mode` - Codex 运行时使用的文件系统/网络沙箱。"danger-full-access" 会完全禁用沙箱。',

  // --- Editor resolver ---
  'editorResolver.fileEditor': '文本编辑器',
  'editorResolver.imagePreview': '图片预览',
  'editor.loading': '正在加载编辑器…',
  'editor.previewTitle': '预览 {name}',
  'editor.externallyModified.message': '文件 "{name}" 在外部已修改。',
  'editor.externallyModified.detail': '是否放弃当前更改并从磁盘重新加载？',
  'editor.externallyModified.reload': '重新加载',
  'editor.externallyModified.keepChanges': '保留当前更改',
  'editor.externalReloadTooLarge':
    '文件 "{name}" 过大，已停止自动同步磁盘上的改动。重新打开该文件可载入最新内容。',
  'languageMode.autoDetect': '自动检测',
  'languageMode.configured': '配置的语言',
  'languageMode.placeholder': '选择语言模式',
  'doc.notFound': '# 文档不存在\n\n文档 "{docId}" 不存在。',

  // --- Notifications center ---
  'notification.center.title': '通知',
  'notification.center.clearAll': '全部清除',
  'notification.center.closeAria': '关闭通知中心',
  'notification.center.empty': '没有通知',
  'notification.dismissAria': '忽略通知',
  'notification.time.justNow': '刚刚',
  'notification.time.minutesAgo': '{count} 分钟前',
  'notification.time.hoursAgo': '{count} 小时前',
  'notification.time.daysAgo': '{count} 天前',

  // --- Issue reporter ---
  'issueReporter.error.unknownProvider': '未知的问题上报提供方"{providerId}"',
  'issueReporter.error.notConfigured':
    '尚未配置问题追踪服务。请设置 issueReporter.tracker.serverUrl 与 issueReporter.tracker.appUrl。',
  'issueReporter.error.attachmentsNotSupported': '问题上报提供方"{providerId}"不支持附件',
  'issueReporter.error.uploadHttp': '诊断包上传失败：HTTP {status} {statusText}',
  'issueReporter.error.uploadNoPath': '诊断包上传响应中没有 path：{body}',
  'reportIssue.attachDiagnostics': '附带诊断包',
  'reportIssue.attachDiagnostics.description': '上传诊断 zip 作为附件（含系统信息与近期日志）',
  'reportIssue.skipDiagnostics': '不附带',
  'reportIssue.attachPrompt': '是否在问题报告中附带诊断包？（诊断摘要已复制到剪贴板）',
  'reportIssue.pasteHint': '（诊断信息较长，请从剪贴板粘贴）',
  'reportIssue.opened': '诊断摘要已复制到剪贴板，请在打开的上报页面中补充问题描述。',
  'reportIssue.failed': '打开问题上报页面失败：{message}',
  'reportIssue.uploadFailed': '诊断包上传失败：{message}',
  'reportIssue.openWithoutAttachment': '不附带诊断包直接打开',

  // --- Diagnostics ---
  'exportDiagnostics.done': '诊断包已导出：{path}',
  'exportDiagnostics.failed': '诊断包导出失败：{message}',

  // --- Memory diagnosis (controlled heap snapshots) ---
  'heapSnapshot.openFolder': '打开快照目录',
  'heapSnapshot.revealFailed': '无法打开快照目录：{message}',
  'heapSnapshot.commandFailed': '内存诊断执行失败：{message}',
  'heapSnapshot.unavailable': '当前版本未接入内存诊断，未启动任何轮次。',
  'heapSnapshot.confirm.message': '对当前窗口开启内存诊断？',
  'heapSnapshot.confirm.detail':
    '只诊断当前窗口，其它窗口不受影响。\n\n采集快照时窗口会暂停数秒至数十秒——这段时间编辑器不响应任何操作。\n\n快照内容是该窗口堆内存里的原始数据，可能包含文件内容、会话正文或凭据。它只写到本机（编辑器用户数据目录下），不会自动上传。\n\n本轮诊断最长 2 小时，重载或关闭窗口即停止；点「停止」不会取消已经开始的那一份快照。',
  'heapSnapshot.confirm.start': '开始诊断',
  'heapSnapshot.confirm.cancel': '取消',
  'heapSnapshot.stop.inFlight':
    '不会再采集新的快照。当前这一份正在写入，无法取消——窗口会一直卡住直到它完成。',
  'heapSnapshot.event.started': '内存诊断已开启：正在建立基线。采集时当前窗口会暂停数秒至数十秒。',
  'heapSnapshot.event.captured': '快照已写入：{name}',
  'heapSnapshot.event.capturedTrigger': '已写入{trigger}快照：{name}（{size}MB）',
  'heapSnapshot.trigger.baseline': '基线',
  'heapSnapshot.trigger.growth': '增长',
  'heapSnapshot.status.off': '内存诊断未开启。',
  'heapSnapshot.status.ended': '内存诊断已结束：',
  'heapSnapshot.status.capturing':
    '内存诊断：正在采集快照（窗口暂时无响应）。已尝试 {attempts} 次，已生成 {artifacts} 个文件。',
  'heapSnapshot.status.running':
    '内存诊断进行中（{phase}）。已尝试 {attempts} 次，已生成 {artifacts} 个文件。',
  'heapSnapshot.notice.heapLimitUnknown':
    '这个窗口没有上报可用的 V8 堆上限，无法推算快照大小，暂不采集。',
  'heapSnapshot.notice.baselineTooLarge': '当前堆已经超过基线快照允许的大小，先等它降下来。',
  'heapSnapshot.notice.baselineUnstable': '堆占用仍在波动，还不够稳定，继续等待基线。',
  'heapSnapshot.notice.sampleStale': '这个窗口不再上报内存读数，本轮诊断已失去依据。',
  'heapSnapshot.notice.holdersExplain': '占用上升已被已知缓存解释，快照不会提供新线索。',
  'heapSnapshot.notice.heapTooLarge': '当前堆已超过约定的拍摄上限，不再自动采集。',
  'heapSnapshot.notice.captureLimitReached': '基线之上已没有足够余量，增长快照无法保持在上限内。',
  'heapSnapshot.notice.physicalMemoryLow': '可用物理内存不足，跳过本次采集，避免加剧压力。',
  'heapSnapshot.notice.commitHeadroomLow': '系统提交内存余量不足，跳过本次采集，避免加剧压力。',
  'heapSnapshot.notice.commitUnknown': '读不到新的提交内存读数，保守跳过本次采集，不做猜测。',
  'heapSnapshot.notice.diskSpaceLow': '磁盘可用空间不足，跳过本次采集。',
  'heapSnapshot.notice.diskUnknown': '读不到磁盘可用空间，保守跳过本次采集，不做猜测。',
  'heapSnapshot.notice.directoryBudget': '快照目录已达上限，请手动清理后再开始新一轮。',
  'heapSnapshot.notice.captureFailed': '快照采集失败，本轮到此结束，不再重复冻结窗口。',
  'heapSnapshot.notice.captureStalled':
    '快照仍在进行（已超过 90 秒）。该调用无法取消，窗口可能继续无响应。',
  'heapSnapshot.notice.appQuotaExhausted':
    '本次运行允许的快照次数已用完，重启编辑器后才有新的额度。',
  'heapSnapshot.notice.roundQuotaExhausted': '本轮诊断的两次采集机会已用完。',
  'heapSnapshot.notice.roundExpired': '本轮诊断已达 2 小时上限，自动停止。',
  'heapSnapshot.notice.roundComplete': '两份快照（基线 + 增长）已写入磁盘。',
  'heapSnapshot.notice.windowClosed': '窗口已关闭，本轮诊断随之结束。',
  'heapSnapshot.notice.windowReloaded': '窗口已重新加载，本轮诊断结束：它测量的堆已不存在。',
  'heapSnapshot.notice.rendererUnavailable':
    '当前窗口的渲染进程已经不在运行，本轮诊断结束。重新加载窗口后可再开一轮。',
  'heapSnapshot.notice.stoppedByUser': '内存诊断已停止。',
  'heapSnapshot.notice.noTarget': '当前窗口没有可采集的渲染进程，无法开始诊断。',
  'action.reloadWindowForMemoryDiagnosis.title': '重载窗口并开始内存诊断',

  // --- Memory reminder (a heap that stays high while releasing fails to bring it down) ---
  'memoryReminder.message':
    '这个窗口已连续 {minutes} 分钟接近内存上限，释放缓存并没有把它降下来。现在重载会从干净的基线重新开始内存诊断——但重载会丢弃未保存的更改，而大约一分钟后采集第一份快照时，这个窗口最多会暂停约 {seconds} 秒。这是正常现象，不是崩溃。只诊断当前窗口；快照是该窗口堆内存里的原始数据，可能包含文件内容或会话正文；它只写到本机，不会自动上传。',
  'memoryReminder.action.reload': '重载并开始诊断',
  'memoryReminder.action.notNow': '暂不',
  'memoryReminder.armFailed': '窗口已重载，但内存诊断没能启动：{message}',
  'memoryReminder.armUnavailable': '窗口已重载，但当前版本未接入内存诊断，未启动任何轮次。',

  // --- Bug recording ---
  'bugRecording.statusTooltip': '正在录制 bug 证据。点击停止并导出。',
  'bugRecording.alreadyRecording': '已有一个 bug 录制在进行中。',
  'bugRecording.started': 'Bug 录制已开始。请复现问题，然后运行「停止 Bug 录制并导出证据包」。',
  'bugRecording.notRecording': '当前没有正在进行的 bug 录制。',
  'bugRecording.stopConfirm': '停止录制并导出证据包？',
  'bugRecording.stopDetail':
    '「保存证据包」保留全部信息，最有利于定位问题。\n\n「脱敏并保存」会掩码用户名、账号目录、项目路径与密钥 token——但也可能抹掉恰好解释这个 bug 的关键线索（例如只在特定路径或用户名下才复现的问题）。\n\n截图无法脱敏：它们是屏幕画面。如果画面可能含敏感信息，请自行检查。',
  'bugRecording.save': '保存证据包',
  'bugRecording.redactAndSave': '脱敏并保存',
  'bugRecording.keepRecording': '继续录制',
  'bugRecording.exported': 'Bug 证据包已导出：{path}（{events} 条事件，{shots} 张截图）',
  'bugRecording.exportFailed': '导出 bug 证据包失败：{message}',
  'bugRecording.orphanFound':
    '{time} 的一次 bug 录制被中断（崩溃或强制退出）。它记录的 {events} 条事件仍在磁盘上。',
  'bugRecording.orphanExport': '导出证据包',
  'bugRecording.orphanDiscard': '暂不导出',

  // --- User data ---
  'userData.error.readOnly': 'UserData：{file} 为只读',
  'userData.error.noWorkspace': 'UserData：未打开工作区（无法写入 {file}）',

  // --- Markdown ---
  'markdown.updateLinks.messageOne': '是否更新 1 个文件中的 Markdown 链接？',
  'markdown.updateLinks.messageMany': '是否更新 {count} 个文件中的 Markdown 链接？',
  'markdown.updateLinks.moreFiles': '，以及另外 {count} 个文件',
  'markdown.updateLinks.update': '更新',
  'markdown.updateLinks.dontUpdate': '不更新',
  'markdown.updateLinks.neverAskAgain': '始终如此（不再询问）',
  'markdown.linkFileNotFound': '文件不存在：{path}',
  'markdownPreview.help.title': '键盘快捷键',
  'markdownPreview.help.linkHintsCurrent': '链接提示（在当前页打开）',
  'markdownPreview.help.linkHintsSide': '链接提示（在侧边打开）',
  'markdownPreview.help.scrollLine': '向下 / 向上滚动一行',
  'markdownPreview.help.scrollHorizontal': '向左 / 向右滚动',
  'markdownPreview.help.scrollHalfPage': '向下 / 向上滚动半页',
  'markdownPreview.help.scrollPage': '向下 / 向上滚动一页',
  'markdownPreview.help.scrollTopBottom': '滚动到顶部 / 底部',
  'markdownPreview.help.backForward': '后退 / 前进',
  'markdownPreview.help.find': '在预览中查找',
  'markdownPreview.help.countPrefix': '数字前缀重复执行（如向下 3 行）',
  'markdownPreview.help.toggle': '显示 / 隐藏此帮助',

  // --- Drag & drop ---
  'dnd.replaceExisting.message': '目标文件夹中已存在名为"{name}"的文件或文件夹。是否替换？',
  'dnd.replaceExisting.detail': '此操作不可逆！',
  'dnd.copyFailed.message': '复制"{name}"失败。',
  'dnd.copyFailed.message.multiple': '{total} 个项目中有 {failed} 个复制失败。',
  'dnd.open.fileFailed': '无法打开"{name}"。',
  'dnd.open.fileMissing': '无法打开"{name}"。该文件可能已被移动、删除，或无法访问。',
  'dnd.open.folderFailed': '无法在新窗口中打开文件夹"{name}"。',
  'dnd.open.noResource': '拖放的项不是可在此打开的文件或文件夹。',

  // --- Undo & Redo (multi-file) ---
  cannotWorkspaceUndo: '无法在所有文件中撤销"{label}"。{message}',
  cannotWorkspaceUndoDueToChanges: '无法在所有文件中撤销"{label}"，因为 {files} 已被修改',
  cannotWorkspaceUndoDueToInMeantimeUndoRedo:
    '无法在所有文件中撤销"{label}"，因为期间发生了撤销或重做操作',
  cannotWorkspaceUndoDueToInProgressUndoRedo:
    '无法在所有文件中撤销"{label}"，因为 {files} 上已有撤销或重做操作正在运行',
  cannotWorkspaceRedo: '无法在所有文件中重做"{label}"。{message}',
  cannotWorkspaceRedoDueToChanges: '无法在所有文件中重做"{label}"，因为 {files} 已被修改',
  cannotWorkspaceRedoDueToInMeantimeUndoRedo:
    '无法在所有文件中重做"{label}"，因为期间发生了撤销或重做操作',
  cannotWorkspaceRedoDueToInProgressUndoRedo:
    '无法在所有文件中重做"{label}"，因为 {files} 上已有撤销或重做操作正在运行',
  cannotResourceUndoDueToInProgressUndoRedo: '无法撤销"{label}"，因为已有撤销或重做操作正在运行。',
  cannotResourceRedoDueToInProgressUndoRedo: '无法重做"{label}"，因为已有撤销或重做操作正在运行。',
  confirmDifferentSource: '要撤销"{label}"吗？',
  'confirmDifferentSource.no': '否',
  'confirmDifferentSource.yes': '是',
  confirmWorkspace: '要在所有文件中撤销"{label}"吗？',
  undoInFiles: '在 {count} 个文件中撤销',
  undoThisFile: '撤销此文件',
  externalRemoval: '以下文件已关闭且在磁盘上被修改：{files}。',
  noParallelUniverses: '以下文件已被以不兼容的方式修改：{files}。',

  // --- Focus scope ---
  'focusScope.dialog.confirm': '聚焦',
  'focusScope.dialog.skipped': '已跳过 {count} 个选择：聚焦项必须位于已打开的文件夹内。',
  'focusScope.dialog.title': '添加到聚焦',
  'focusScope.manage.add': '添加…',
  'focusScope.manage.exit': '退出聚焦模式',
  'focusScope.manage.placeholder': '聚焦项 —— 选择一个以移除',
  'focusScope.manage.placeholderEmpty': '聚焦模式已开启，但未聚焦任何项',
  'focusScope.manage.remove': '从聚焦中移除',
  'focusScope.noWorkspace': '请先打开文件夹 —— 聚焦文件夹是相对于已打开文件夹的。',
  'status.focusScope.tooltip': '聚焦于：\n{folderList}\n\n点击管理聚焦项。',
  'status.focusScope.tooltip.empty':
    '聚焦模式已开启但未聚焦任何项，因此不会过滤任何内容。点击添加一项或关闭聚焦。',

  // --- Timeline ---
  'timeline.filterBySource': '按来源筛选',
  'timeline.label': '时间线',
  'timeline.loadMore': '加载更多',
  'timeline.moreActions': '更多操作…',
  'timeline.noItems': '未提供时间线信息。',
  'timeline.noProvider': '当前编辑器无法提供时间线信息。',
  'timeline.openInGitGraph': '在 Git Graph 中打开',
  'timeline.openInPerforceGraph': '在 Perforce Graph 中打开',
  'timeline.pin': '固定时间线',
  'timeline.unpin': '取消固定时间线',
  'timeline.viewCommit': '打开提交',

  // --- Process Explorer ---
  'processExplorer.collapse': '折叠',
  'processExplorer.columnCpu': 'CPU (%)',
  'processExplorer.columnMemory': '内存 (MB)',
  'processExplorer.columnName': '进程名称',
  'processExplorer.columnPid': 'PID',
  'processExplorer.copyAllProcesses': '全部复制',
  'processExplorer.copyProcess': '复制',
  'processExplorer.expand': '展开',
  'processExplorer.forceKillProcess': '强制结束进程',
  'processExplorer.killFailed': '结束进程 {name}（pid {pid}）失败：{error}',
  'processExplorer.killProcess': '结束进程',
  'processExplorer.loading': '正在收集进程信息…',
  'processExplorer.title': '进程资源管理器',

  // --- Image editor ---
  'image.load.failed': '无法显示此图片。',
  'image.status.dimensions': '图片尺寸',
  'image.status.fit': '适应窗口',
  'image.status.size': '文件大小',
  'image.status.zoom': '缩放级别',

  // --- Theme errors ---
  'error.cannotParseJson': '解析 JSON 主题文件时出错：{errors}',
  'error.circularInclude': '主题文件中检测到循环 include：{key}',
  'error.externalTokenColors': "解析颜色主题文件时出错：不支持将 'tokenColors' 指向 tmTheme 文件。",
  'error.invalidColors': "解析颜色主题文件时出错：'colors' 不是对象。",
  'error.invalidFormat': 'JSON 主题文件格式无效：应为对象。',
  'error.invalidSemanticTokenColors': "解析颜色主题文件时出错：'semanticTokenColors' 不是对象。",
  'error.invalidTokenColors': "解析颜色主题文件时出错：'tokenColors' 应为规则数组。",
  'error.unsupportedThemeFormat': '不支持的主题文件格式（仅支持 .json）：{location}',

  // --- Workbench misc ---
  'aiDebug.list': '已记录的 AI 请求',
  'codeBlock.copied': '已复制',
  'codeBlock.copy': '复制代码',
  'commandPalette.configureKeybinding': '配置键盘快捷方式',
  'commandPalette.removeFromRecentlyUsed': '从最近使用中移除',
  'crash.detail.storm':
    '渲染进程意外退出（{reason}），最近几分钟内已发生 {count} 次。自动重载已暂停 —— 在你做出选择前，窗口不会自行恢复。',
  'customEditor.noProvider': '没有可打开此文件的扩展。',
  'editorGroup.unlock': '解锁组',
  'editorTitle.moreActions': '更多操作…',
  'errorToast.copyDetails': '复制详情',
  'extHost.unhandledRejection': '扩展宿主报告了未处理的 rejection：{error}',
  'extHost.unhandledRejection.showLog': '显示日志',
  'extensionTreeView.label': '扩展视图：{viewId}',
  'extensionTreeView.placeholder': '此视图由扩展提供。正在加载…',
  'findWordAtCursor.noMoreMatches': '没有更多匹配项。',
  'lsp.starting': '正在启动语言服务…',
  'quickInput.itemButton.ariaLabel': '项操作',
  'quickInput.toggleSelection.ariaLabel': '切换选中状态',
  'reportIssue.noProvider': '没有可用的上报目标。',
  'titlebar.remoteBadge.tooltip': '远程：{authority}',

  // --- Color descriptions (colorRegistry) ---
  'color.workbench.background': '工作台的整体背景色。',
  'color.foreground': '整体前景色。',
  'color.workbench.foregroundBright': '用于强调文本的亮色前景色。',
  'color.descriptionForeground': '用于补充说明的描述文本（例如标签）的前景色。',
  'color.workbench.border': '分隔工作台主要区域的边框色。',
  'color.focusBorder': '聚焦元素的整体边框色。',
  'color.workbench.accent': '主要操作与活动指示器的强调色。',
  'color.workbench.accentHover': '悬停在主要操作上时的强调色。',
  'color.list.highlightForeground': '搜索时匹配文本的前景色。',
  'color.workbench.surface': '卡片、浮层等浮起表面的背景色。',
  'color.agent.claudeAccent': 'Claude agent 品牌元素的强调色。',
  'color.titleBar.activeBackground': '窗口处于活动状态时的标题栏背景色。',
  'color.activityBar.background': '活动栏背景色。',
  'color.activityBar.inactiveForeground': '活动栏项处于非活动状态时的前景色。',
  'color.activityBar.hoverForeground': '悬停时活动栏项的前景色。',
  'color.activityBar.foreground': '活动栏项处于活动状态时的前景色。',
  'color.activityBar.hoverBackground': '悬停时活动栏项的背景色。',
  'color.activityBar.activeBorder': '活动栏中活动项的边框色。',
  'color.activityBarBadge.background': '活动通知徽标的背景色。',
  'color.activityBarBadge.foreground': '活动通知徽标的前景色。',
  'color.sideBar.background': '侧边栏背景色。',
  'color.sideBar.foreground': '侧边栏前景色。',
  'color.sideBarSectionHeader.background': '侧边栏分区标题的背景色。',
  'color.sideBarSectionHeader.foreground': '侧边栏分区标题的前景色。',
  'color.sideBarSectionHeader.hoverBackground': '悬停时侧边栏分区标题的背景色。',
  'color.sideBar.border': '侧边栏与编辑器分界一侧的边框色。',
  'color.fileIcon.defaultForeground': '默认文件图标的颜色。',
  'color.fileIcon.folderForeground': '文件夹图标的颜色。',
  'color.fileIcon.specialFolderForeground': '特殊文件夹图标的颜色。',
  'color.fileIcon.codeForeground': '代码文件图标的颜色。',
  'color.fileIcon.configForeground': '配置文件图标的颜色。',
  'color.fileIcon.markdownForeground': 'Markdown 文件图标的颜色。',
  'color.fileIcon.specialForeground': '特殊文件图标的颜色。',
  'color.editor.background': '编辑器背景色。',
  'color.editor.foreground': '编辑器的默认前景色。',
  'color.editor.mutedForeground': '编辑器区域中弱化的前景色。',
  'color.editor.blameForeground': '行内 git blame 标注的前景色。',
  'color.editorGroup.border': '编辑器区域及编辑器内部部件的边框色。',
  'color.editorWidget.background': '行内评论线程等编辑器部件的背景色。',
  'color.editor.findMatchBackground': '当前搜索匹配项的背景色。',
  'color.editor.findMatchForeground': '当前搜索匹配项的前景色。',
  'color.breadcrumb.foreground': '面包屑项的前景色。',
  'color.editor.lineHighlightBackground': '编辑器行高亮的背景色。',
  'color.editor.lineHighlightBorder': '编辑器行高亮的边框色。',
  'color.editor.findMatchHighlightBackground': '文件与视图中搜索匹配项的背景色。',
  'color.editorCodeLens.foreground': '编辑器 CodeLens 与行内操作分隔符的前景色。',
  'color.textLink.foreground': '文本中链接的前景色。',
  'color.editorGroupHeader.tabsBackground': '编辑器组标题栏的背景色。',
  'color.tab.activeBackground': '活动编辑器标签的背景色。',
  'color.tab.activeForeground': '活动编辑器标签的前景色。',
  'color.tab.activeBorderTop': '活动编辑器标签顶部的边框。',
  'color.tab.inactiveBackground': '非活动编辑器标签的背景色。',
  'color.tab.inactiveForeground': '非活动编辑器标签的前景色。',
  'color.tab.hoverForeground': '悬停时编辑器标签的前景色。',
  'color.tab.hoverBackground': '悬停时编辑器标签的背景色。',
  'color.tab.modifiedBorder': '标记已修改编辑器标签的边框色。',
  'color.tab.unfocusedActiveForeground': '编辑器组失焦时其活动标签的前景色。',
  'color.tab.unfocusedActiveBorderTop': '编辑器组失焦时其活动标签顶部的边框。',
  'color.statusBar.background': '状态栏背景色。',
  'color.statusBar.foreground': '状态栏前景色。',
  'color.statusBarItem.prominentForeground': '状态栏醒目项的前景色。',
  'color.statusBarItem.remoteBackground': '状态栏远程指示器的背景色。',
  'color.statusBarItem.remoteForeground': '状态栏远程指示器的前景色。',
  'color.statusBarItem.errorBackground': '状态栏项出错时的背景色。',
  'color.statusBarItem.errorForeground': '状态栏项出错时的前景色。',
  'color.panel.background': '面板背景色。',
  'color.panel.border': '面板与编辑器之间分界的边框色。',
  'color.panel.tabBarBackground': '面板标签栏的背景色。',
  'color.panel.toolbarBackground': '面板工具栏的背景色。',
  'color.quickInput.background': '快速输入框的背景色。',
  'color.quickInput.border': '快速输入框的边框色。',
  'color.list.hoverBackground': '悬停在列表项上时的背景色。',
  'color.list.focusOutline': '列表中聚焦项的轮廓色。',
  'color.list.focusBackground': '列表中聚焦项的背景色。',
  'color.list.activeSelectionBackground': '列表处于活动状态时所选项的背景色。',
  'color.list.activeSelectionBar': '列表处于活动状态时所选项的指示条颜色。',
  'color.list.activeSelectionForeground': '列表处于活动状态时所选项的前景色。',
  'color.list.inactiveSelectionBackground': '列表处于非活动状态时所选项的背景色。',
  'color.list.dropBackground': '将项目拖到放置目标上时列表的背景色。',
  'color.input.background': '输入框背景色。',
  'color.input.foreground': '输入框前景色。',
  'color.input.border': '输入框边框色。',
  'color.input.placeholderForeground': '输入框占位符的前景色。',
  'color.inputOption.activeBackground': '已激活的输入选项开关（如区分大小写）的背景色。',
  'color.picker.modifiedLabel': '标记为 `modified`（本地已修改内容）的快速选择项的标签颜色。',
  'color.picker.orphanLabel': '标记为 `orphan`（占用某路径的未跟踪文件）的快速选择项的标签颜色。',
  'color.picker.forceLabel': '标记为 `force`（覆盖本地文件的强制获取）的快速选择项的标签颜色。',
  'color.inputValidation.errorBorder': '错误级别的输入校验边框色。',
  'color.symbolIcon.defaultForeground': '大纲与面包屑中符号图标的默认前景色。',
  'color.symbolIcon.functionForeground': '可调用符号图标（函数与方法）的前景色。',
  'color.symbolIcon.variableForeground': '数据符号图标（变量与字段）的前景色。',
  'color.symbolIcon.classForeground': '类型符号图标（类与接口）的前景色。',
  'color.tooltip.background': '悬浮提示的背景色。',
  'color.tooltip.foreground': '悬浮提示的前景色。',
  'color.sash.hoverBorder': '悬停时拖拽分隔条的边框色。',
  'color.dropdown.background': '下拉框背景色。',
  'color.dropdown.border': '下拉框边框色。',
  'color.badge.background': '徽标背景色。',
  'color.badge.foreground': '徽标前景色。',
  'color.badge.progressBackground': '进行中徽标的背景色。',
  'color.badge.successBackground': '成功徽标的背景色。',
  'color.badge.warningBackground': '警告徽标的背景色。',
  'color.badge.errorBackground': '错误徽标的背景色。',
  'color.toolbar.hoverBackground': '悬停时工具栏操作的背景色。',
  'color.button.background': '按钮背景色。',
  'color.button.foreground': '按钮前景色。',
  'color.button.hoverBackground': '悬停时按钮的背景色。',
  'color.button.secondaryBackground': '次要按钮的背景色。',
  'color.button.secondaryForeground': '次要按钮的前景色。',
  'color.button.secondaryBorder': '次要按钮的边框色。',
  'color.button.secondaryHoverBackground': '悬停时次要按钮的背景色。',
  'color.menu.background': '上下文菜单背景色。',
  'color.menu.border': '上下文菜单边框色。',
  'color.menu.foreground': '上下文菜单项的前景色。',
  'color.menu.selectionBackground': '上下文菜单中所选项的背景色。',
  'color.menu.selectionForeground': '上下文菜单中所选项的前景色。',
  'color.menu.disabledForeground': '上下文菜单中禁用项的前景色。',
  'color.menu.separatorBackground': '上下文菜单分隔线的颜色。',
  'color.settings.modifiedItemIndicator': '值不同于默认值的设置项左侧指示条。',
  'color.agent.messageBackground': '聊天消息的背景色。',
  'color.agent.messageForeground': '聊天消息的前景色。',
  'color.agent.messageRoleForeground': '聊天消息角色标签的前景色。',
  'color.agent.userMessageBackground': '用户聊天消息的背景色。',
  'color.agent.thoughtBackground': 'Agent 思考块的背景色。',
  'color.agent.thoughtAccent': 'Agent 思考块的强调色。',
  'color.textCodeBlock.background': '文本中代码块的背景色。',
  'color.textInlineCode.background': '文本中行内代码的背景色。',
  'color.textInlineCode.foreground': '文本中行内代码的前景色。',
  'color.agent.resourceLinkBackground': '提示词输入框中资源引用标签的背景色。',
  'color.agent.commandBadgeBackground': '提示词输入框中命令徽标的背景色。',
  'color.agent.questionInputBackground': '提问输入框的背景色。',
  'color.agent.sendHoverForeground': '悬停时发送按钮的前景色。',
  'color.agent.sendTrackBackground': '发送按钮轨道的背景色。',
  'color.agent.sendHoverBackground': '悬停时发送按钮的背景色。',
  'color.agent.sendIconRing': '发送按钮图标的圆环颜色。',
  'color.errorForeground': '错误消息的整体前景色。',
  'color.error.background': '错误区块的背景色。',
  'color.success.foreground': '成功消息的前景色。',
  'color.danger.background': '危险操作的背景色。',
  'color.warning.foreground': '警告消息的前景色。',
  'color.warning.background': '警告区块的背景色。',
  'color.warning.border': '警告区块的边框色。',
  'color.notifications.infoBackground': '信息类通知区块的背景色。',
  'color.notificationsErrorIcon.foreground': '错误通知图标的前景色。',
  'color.notificationsWarningIcon.foreground': '警告通知图标的前景色。',
  'color.notificationsInfoIcon.foreground': '信息通知图标的前景色。',
  'color.progressBar.background': '进度条的前景色。',
  'color.progressBar.trackBackground': '进度条的轨道（背景）色。',
  'color.diffEditor.insertedTextBackground': '差异中新增文本的背景色。',
  'color.diffEditor.removedTextBackground': '差异中删除文本的背景色。',
  'color.diffEditor.insertedLineBackground': '差异视图中新增行的背景色。',
  'color.diffEditor.removedLineBackground': '差异视图中删除行的背景色。',
  'color.diff.insertedForeground': '差异视图中标记新增内容的前景色。',
  'color.diff.removedForeground': '差异视图中标记删除内容的前景色。',
  'color.editorGutter.addedBackground': '编辑器装订线中新增行的背景色。',
  'color.editorGutter.modifiedBackground': '编辑器装订线中修改行的背景色。',
  'color.editorGutter.deletedBackground': '编辑器装订线中删除行的背景色。',
  'color.minimapGutter.addedBackground': '小地图中新增行的背景色。',
  'color.minimapGutter.modifiedBackground': '小地图中修改行的背景色。',
  'color.minimapGutter.deletedBackground': '小地图中删除行的背景色。',
  'color.editorOverviewRuler.addedForeground': '概览标尺中新增内容的标记颜色。',
  'color.editorOverviewRuler.modifiedForeground': '概览标尺中修改内容的标记颜色。',
  'color.editorOverviewRuler.deletedForeground': '概览标尺中删除内容的标记颜色。',
  'color.gitDecoration.addedResourceForeground': 'Git 新增资源的前景色。',
  'color.gitDecoration.modifiedResourceForeground': 'Git 已修改资源的前景色。',
  'color.gitDecoration.conflictingResourceForeground': 'Git 冲突资源的前景色。',
  'color.gitDecoration.deletedResourceForeground': 'Git 已删除资源的前景色。',
  'color.gitDecoration.renamedResourceForeground': 'Git 重命名或复制资源的前景色。',
  'color.gitDecoration.ignoredResourceForeground': 'Git 已忽略资源的前景色。',
  'color.merge.currentContentBackground': '合并冲突中当前（我方）内容的背景色。',
  'color.merge.incomingContentBackground': '合并冲突中传入（对方）内容的背景色。',
  'color.merge.commonContentBackground': '合并冲突中共同基线的背景色。',
  'color.merge.headerBackground': '合并冲突中标记行的背景色。',
  'color.dialog.background': '对话框背景色。',
  'color.dialog.foreground': '对话框前景色。',
  'color.dialog.border': '对话框边框色。',
  'color.dialog.detailForeground': '对话框详情文本的前景色。',
  'color.widget.shadow': '浮层等部件的阴影颜色。',
  'color.card.shadow': '卡片的阴影颜色。',
  'color.workbench.scrollbarThumbBackground': '滚动条滑块的背景色。',
  'color.workbench.scrollbarThumbHoverBackground': '悬停时滚动条滑块的背景色。',
  'color.workbench.hoverBackground': '通用的悬停背景色。',
  'color.workbench.borderSubtle': '弱化的边框色。',
  'color.workbench.accentSoftBackground': '柔和的强调背景色。',
  'color.terminal.background': '终端背景色。',
  'color.terminal.foreground': '终端前景色。',
  'color.terminalCursor.foreground': '终端光标的前景色。',
  'color.terminalCursor.background': '终端光标的背景色（光标背后的不透明方块）。',
  'color.terminal.selectionBackground': '终端选区的背景色。',
  'color.terminal.ansiBlack': '终端中的 ANSI 黑色。',
  'color.terminal.ansiRed': '终端中的 ANSI 红色。',
  'color.terminal.ansiGreen': '终端中的 ANSI 绿色。',
  'color.terminal.ansiYellow': '终端中的 ANSI 黄色。',
  'color.terminal.ansiBlue': '终端中的 ANSI 蓝色。',
  'color.terminal.ansiMagenta': '终端中的 ANSI 品红色。',
  'color.terminal.ansiCyan': '终端中的 ANSI 青色。',
  'color.terminal.ansiWhite': '终端中的 ANSI 白色。',
  'color.terminal.ansiBrightBlack': '终端中的 ANSI 亮黑色。',
  'color.terminal.ansiBrightRed': '终端中的 ANSI 亮红色。',
  'color.terminal.ansiBrightGreen': '终端中的 ANSI 亮绿色。',
  'color.terminal.ansiBrightYellow': '终端中的 ANSI 亮黄色。',
  'color.terminal.ansiBrightBlue': '终端中的 ANSI 亮蓝色。',
  'color.terminal.ansiBrightMagenta': '终端中的 ANSI 亮品红色。',
  'color.terminal.ansiBrightCyan': '终端中的 ANSI 亮青色。',
  'color.terminal.ansiBrightWhite': '终端中的 ANSI 亮白色。',
} as const
