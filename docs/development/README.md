...
- [命令、菜单与上下文键](commands-and-context-keys.md) — `Action2` 的 `menu` 同时供给右键菜单与命令面板：`precondition` 会 AND 进每一行、面板打开时 `editorAreaFocus` 必为 false——两条会静默失效的判据与正确写法。
...
- [在 WSL2/Ubuntu 下跑 e2e](wsl-e2e.md) — 用 xvfb 离屏跑 Playwright e2e，避免测试窗口抢 Windows 前台焦点；含 AI agent 在 WSL 内工作的一键初始化与 xvfb 包装。
