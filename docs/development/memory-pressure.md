...
| 组 | 语义 | 名字 |
|---|---|---|
| `flow=` | **区间增量**，上报即清零；`name:calls、chars` | `mdparse` / `mdreseal` / `colorize` / `colorize.skip` / `materialize`（顶层）/ `childchunks`（子代理）/ `docpush`、`docdrop`（文档镜像出站）/ `extreload`（外部刷新读盘） |
...
**第六类缺口：出站聚合放大 × 行尾假全文编辑。** 2026-09-19 的 renderer OOM（`oom-lo-space-size=3294MB`）既不来自读也不来自渲染，而来自**同一份内容在出站方向被重新组装**：主线程一卡，`DocumentSyncContribution` 的 200ms debounce 就不跑，待发增量在 `entry.pending` 里无上限累加，恢复后一次性发出——`docSync.log` 里 43 次 ≥1M chars 的 `didChange` 合计 481M chars、单次最大 71.5M（次大 61.7M），而文档本身只有 10.2M。判读靠那行 `didChange <uri> changes=N chars=M`：`N≈1 且 M≈文档大小`说明每次都是整篇替换（口径在写入侧，见下）；`Σtext ≈ N 份文档`说明是积压合批（机制在此处）。**两个放大环节互相独立，缺一个都凑不出那 481M chars**：下面那段解释「攒」，紧接着这段解释「攒起来的为什么每一份都是整篇」。

> **证据口径（先读，别把结论当成现场读数）**：这次事故**没有堆快照**。现场只有三样东西：`docSync.log` 的 `didChange … changes/chars` 行、main 侧的 `large outbound ipc frame … (response fileSystem.readFileText)` 告警、以及崩溃行 `oom-lo-space-size=3294MB`。所以下面关于「谁把文本留在堆里」的机制结论**不是**从对象图里读出来的，而是三条合起来的：① **源码**（`pieceTreeBase` / `editStack` 里字符串怎么被持有）；② **复现**（`smoke.docSyncExternalReload.spec.ts` 在真窗口里重放这个形状）；③ **修前修后的曲线**（同一 spec 同一台机器的堆轨迹与比值）。三样缺一条，这类结论都只该写成假说。

**写入侧：每份载荷为什么都是整篇。** Monaco 的缓冲区只存**一种**行尾（`PieceTreeTextBufferFactory._getEOL`：CR+CRLF 过半即 CRLF，创建时把其余行尾统一重写），而盘上的字节可以混写——外部进程写的日志就是 CRLF 混少量裸 LF。旧代码拿 `getValue()` 与**原始盘上文本**逐字节比对，于是恒不相等，每次算出的差异从第一个异种行尾一直延伸到最后一个（近全文）：一次「盘上追加了几 KB」变成一次近全文 didChange，形状正是上面那行 `N≈1 且 M≈文档大小`。修法在 `services/editor/minimalModelEdit.ts`：`normalizeToModelEol` 只把**进来的盘上文本**统一成模型自己的行尾（绝不改模型的 EOL；`hasForeignEol` 先扫一遍，不需要重写时返回同一个字符串而不是复制一份），之后的比对与最小编辑都在**模型坐标系**里做（`getPositionAt` 读的也是模型）；相等即 no-op（连 version 都不动），不等才推真正变了的那一段。BOM 同理：`splitLeadingBom` 在读盘处就摘掉，`save()` 按 `_hasLeadingBom` 装回去。四条外部刷新路径（`FileEditorInput.checkExternalChange`、`ExternalChangeWatcher` 的预览 / 可编辑 diff / 快照 diff）共用这一套语义。

**读侧：同一份内容被读几次、读多快。** 单次成本由 `MAX_EXTERNAL_RELOAD_BYTES`（16MiB，见第四类缺口）挡住，频次由 `ExternalChangeWatcher._requestReconcile` 的**按资源单飞**挡住——一次对账在跑时后到的请求只置一个「排队」标志（不是计数），所以一份永不安静的文件只是拿到一次后续通过，而不是堆成一串读盘；批次本身在第一个 `await` 之前就被 `_handle` 同步过滤掉（只留下与已打开编辑器相关的变更），25k 条事件的风暴到不了对账层。`extreload` 记的就是这条路径真正读回来的字符数。

出站现在有三层约束：① `pending` 有**字符数与条数上限**（2Mi chars / 1000 条），越限即丢增量、只留「下次整篇」标记——**全文在推送时取，不在越限时取**（越限时的调用方通常正是卡住的主线程，读文档等于把成本挪个位置）；② 每份文档**同时在途至多一批**，且 open 与批串行（open 先等在途批落地，否则旧批的 ack 会记到新镜像上；open 期间再次到达的 resync 合并成一次后续 open），慢宿主只让积压有界地停在同一个地方；③ 一批失败即作废旧增量基线，下一次推送整篇恢复。小改动仍是 200ms debounce + 增量。另一处语义收紧：`PendingDocumentSync.flush`（补全/保存参与者用）现在等的是**调用时版本的 ack**——无 debounce 但在途时也等，且不等调用之后到达的输入。open 与 change 共用调用起点的 5 秒期限；超时记录 warn 并 reject，由保存参与者的异常隔离跳过本次扩展步骤，不继续向卡住的宿主请求保存前编辑。在途请求与载荷计数保持到真实落地，不能超时解锁后重发而重新形成并发放大。

读法：`flow=` 的 `docpush`（真正上线的字符数）与 `docdrop`（被丢弃的增量字符数）。同时涨=放大器仍在赢（文档在频繁整篇变更）；`docpush` 落、`docdrop` 涨=上限在挡。**`extreload` 与 `docpush` 并排读**才能分开两个环节：`extreload` 高而 `docpush` 与文件同阶 = 行尾口径又错了（写入侧回归）；两者同涨 = 真有人在整篇改文档。即时读数在 `DocumentSyncStats`（`services/extensions/documentSyncStats.ts`）：`pending` / `inflight` 由入队与发送处维护（**不读文档**），`openChars` 取 `getValueLength()`（长度，不是全文），文档 detach 即删记录，所以拆掉的管线读作 0。进堆报告已在 `main.tsx` 的 `createRendererHeapReporter` 里接上 holder（`measureDocumentSyncPayload`）与 count（`DocumentSyncStats.read().openDocs`）；holder **只报 pending+inflight**——镜像正文属于 `monaco` holder，重复计会让两者都不可归因。读回来的那份文本本身还有一条约束：**不许把它留在堆里**——差异段交给模型时必须自有字符数据，不能是父串的切片视图（机制、修法与实测见「验证」一节那条 `@regression` e2e 与 `detachView`）。
...
- e2e：`smoke.agentStreamMemory.spec.ts` 的 `acp sub-agent streaming render accounting` 组（`emit-subagent` / `emit-subagent-mixed` 夹具）。断言的是**算法形状**（`mdparse` 与 `mdreseal` 的比值、`childchunks.calls` 相对 chunk 数的量级、流式期 `colorize.chars === 0`），**不**断言墙钟或 MB——那些是机器的属性。`emit-subagent-mixed` 那条专守**追加子 tool call 的 seal**：在两次子 tool call 都已到达、回合仍在跑时取样，断言两条被打断消息均为 `live === false`；回合结束后另验全文长度。不能用回合结束后的快照验证中途 seal，因为 `_flushStream` 会统一清掉 live，掩盖追加时漏 seal 的缺陷。
- e2e：`@p1` `apps/editor/e2e/specs/smoke.agentStreamMemory.spec.ts`——把一条 300KB 的思考消息喂给真窗口（夹具 `emit-thought:<count>x<kb>[,fence]`，通过 `ECHO_AGENT_TURN_END_GATE` 保持回合开启，取得完整文本的流式渲染读数后才放行；子代理渲染用例另通过 `ECHO_AGENT_STREAM_CONTINUE_GATE` 在首块后等待卡片展开并挂载，确保后续块确实覆盖多次增量渲染，而不与点击耗时竞争；未配置门控的夹具仍保留末块后 500ms 等待），断言**可密封消息的 `mdparse.chars` 不超过 (长度 + 每次调用有界的尾部)**、sealed 缓存确实在增长、`colorize.chars === 0`；再断言**从不闭合的围栏在流式期间 `colorize.chars === 0`、seal 之后被着色**，两条都比对全文逐字符相等。刻意不断言 WS/RSS/墙钟：那是机器属性，而这里要守的是算法形状。
...
- e2e：`@regression` `apps/editor/e2e/specs/smoke.docSyncExternalReload.spec.ts`——真窗口 + **真 node fs 写入**（`workspaceSeeder` 造一份合成日志：行尾 CRLF 占多数、每 50 行混一条裸 LF、开头带 BOM，约 10M 字符，正文全是合成行）。三个形状断言：① **同内容原子重写**（临时文件 + `rename`，避免 watcher 读到写一半的截断内容）只产生一次 `extreload` 读盘、`docpush` **零**——行尾口径错了这里就会是一份近全文；② 24 轮各约 4KB 追加（4 轮一批），每批至少一次读盘，出站量与追加量**同阶**（实测逐批 `pushed == appended`，整段远小于一份文档）；③ 全程 `DocumentSyncStats` 载荷（积压 + 在途，settle 窗口内 40ms 连续采样取峰值）有界——载荷是瞬时量，settle 之后再读永远是 0，只有窗口内采样才看得见放大器攒起来的东西。内容一致性用**指纹**判：探针 `getActiveEditorTextDigest` 在渲染侧按 FNV-1a 算模型文本的 `{length, hash}`，spec 侧从磁盘字节按同一套「去 BOM + 归一到模型行尾」算出期望值，10MB 正文不过桥（两条用例：10MB 场景 + 一条手写字面量的小文件，后者同时把两侧的 FNV-1a 钉在一起）。**基线取在初始镜像之后**并断言 `openDocs`/`openChars` 是整篇：没有这一步，「出站量很小」会被「管道根本没起来」蒙混成绿色。计数值一律走 `readHeapFlowTotals()` 的**非破坏性累计读法**（`drain` 是采样器专用的），所以 spec 不会把真机的采样读数吃掉。
  堆这一节**是断言**（2026-09-20 收紧）：读数前经 CDP `HeapProfiler.collectGarbage` 强制回收（与 `smoke.outlineRetention` 同一条路），打印轨迹与 `growth / (2 × extreload.chars)` 比值，并断言该比值 < 0.5——「每读一次盘不许把这一份文本留在堆里」。放大器的机制是**交出去的差异段是父串的视图**（**源码**读出来的形状，由本条复现证实；原事故没有堆快照，口径见第六类缺口下的「证据口径」）：`String.prototype.slice` 对稍长的结果返回 `SlicedString`，它只记住父串上的偏移，父串在这个视图活着时无法回收；而 `pushEditOperations` 收到的字符串被 piece tree **原样**放进 `StringBuffer`（`pieceTreeBase.js` 的 `new StringBuffer(value, lineStarts)`，不做扁平化），活到文档关闭。于是「读一份 10.5M 字符、切出 4KB 追加段交出去」每轮留下一整份父串。**undo 栈不是持有者**：`editStack.js` 的 `SingleModelEditStackData.serialize()` 把每条 `TextChange` 写成定长字节（`Uint8Array`），栈里没有字符串。修法在 `minimalModelEdit.ts` 的 `detachView`：差异段按**码元**重建（`split('')` → `join('')`），结果里没有任何一片指向原串，代价是 O(差异段)、与父串大小无关；码元粒度是刻意的——代理对被拆成两半再按原序拼回、孤立代理项原样穿过，而 `TextEncoder`/`Buffer` 会把孤立代理项换成 U+FFFD。
  实测（同一 spec、同一台机器，即**修前修后曲线**）：修复前该比值 **1.01**、24 轮堆轨迹 `75→157→238→320→401→482→563MB`（增长 511MB ≈ 每轮 21MB，正好一份 10.5M 字符的文本）；修复后 **0.01**、`75→77→78→79→79→80→79MB`（增长 4.3MB，含镜像自身的合法增长）。**判据能截获原缺陷已复跑确认**（2026-09-20）：把 `detachView` 退回切片视图，该用例当场红（`511016290 < 252890112` 失败，轨迹与修复前逐点一致），并给判据留了约 48 倍余量（修后 5.2MB vs 线 253MB）。
- 单测（出站管道，假件见 `contributions/__tests__/documentSyncHarness.ts`）：`DocumentSyncContribution.pending.test.ts`（积压按字符/条数封顶、越限判定不读文档、每文档至多一批在线、批失败后下一次整篇）、`DocumentSyncContribution.flush.test.ts`（flush 等的是**调用时版本**的 ack：在途批也要等，但不被调用之后到达的输入拖住；open 没落地、宿主重启、通道失败各有有界出口）、`DocumentSyncContribution.lifecycle.test.ts`（激活期被摘除不发 open、迟到的批不得认领新镜像的读数、语言切换 close→open、工作区切换排在在途批之后、重启后按世代重开）、`services/extensions/__tests__/documentSyncStats.test.ts`（跨文档求和、过期句柄的写入被丢、detach 即删记录、拆管道后读作 0）。
- 单测（读侧与并发）：`FileEditorInput.externalChange.test.ts`（读盘期间用户输入 / 模型被替换 / 被 dispose 三种窗口都不写缓冲区，`_lastKnownMtime` 只在真的对上之后才记，EOL-only 差异不产生版本变更，`extreload` 计数）、`ExternalChangeWatcher.test.ts` 的 `per-resource single flight` 组（连发 200 条事件合并成一次后续通过、一个文件卡在确认框上不拖住别的文件、user-data 的 force 合并进在跑的那趟、一趟失败不吞下一条事件、dispose 后不再写盘）、`externalReload.test.ts`（读盘计数；被上限拒绝的读不计数）。
- 单测（真 GC 的回收验证）：`renderer/services/editor/__tests__/minimalModelEdit.retention.test.ts`——与上一条 e2e **同形状**（真文件追加 + 整读 + 差异段交给模型侧持有者的**替代品**，不是真 Monaco），但用真实 V8 GC 问「每轮多留了多少」，所以判据不随机器快慢漂移。两条用例：① 我们的实现每轮只留差异段（逐轮增长 < 8MB；把实现退回切片视图实测 253MB / 每轮 16.1MB，立即红）；② 守**前提**——切片视图确实会钉住父串（15 轮 ≥ 64MB），它红了说明 V8 的字符串表示变了、`detachView` 的取舍需要重新评估，那是结论要变而不是抖动。判据取**逐轮增长**而不是绝对增长：首轮有一次性的常数（`getValue()` 的 rope 扁平化、V8 为十几 MB 分配留下的页），它不属于「每轮留一份」。GC 拿不到即 fail loud（同 `Tree.callbackRetention`）。另一条踩坑：用例的模型侧持有者必须与 `FileEditorInput` **同口径去 BOM**——模型值去了 BOM 而喂进去的没去，差异段会退化成整篇，用例会失真（第一版正是这么红的，而它读起来像「修复无效」）。
