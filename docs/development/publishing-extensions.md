...
4. **发布进 stage**：调 `scripts/gallery/publish.mjs`（带上 `--signing-key-file`/`--key-id`），把 `.vsix` 落地到 `<stage>/gallery/assets/**`、对暂存字节签名，并 upsert `registry.json`（含 `sha256` + `signature`）。
