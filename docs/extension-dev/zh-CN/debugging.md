...
> 开发宿主、断点、日志与迭代循环的完整说明。以扩展 API 0.13.0 / uex 0.1.0 为准；基础流程（脚手架 → watch → `uex dev`）见 [快速上手](./getting-started.md)，本篇展开调试细节。
...
**扩展没激活**
两类典型原因：`activationEvents` 没触发（懒加载——先做出触发动作，如运行声明的命令）；或 `engines.universe` 区间不满足当前编辑器版本，扩展被**自动禁用**——在扩展视图里标为「已禁用」并显示原因（已禁用，需要 universe X，当前 Y），另有一次性通知。两者都可在扩展视图与宿主日志里分辨，不影响其他扩展。
