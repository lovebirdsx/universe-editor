...
> 一个 Universe Editor 扩展就是一个带 `package.json` 的 npm 目录：manifest 逐字段说明、激活事件的完整清单、扩展从扫描到 deactivate 的生命周期，以及 `activate` 收到的 `ExtensionContext`。以 API 0.13.0 为准。
...
| `engines` | **整个对象必填**，且必须含 `engines.universe`。它声明的是**编辑器版本**兼容区间（0.13.0 起 API 包版本与编辑器版本同空间）；推荐 `">=0.13.0 <1.0.0"`，**不要用 `^0.13.0`**。语义与理由见 [API 版本与 `engines.universe`](./versioning.md)，此处不重复。 |
...
```jsonc
{
  ...
  "engines": {
    "universe": ">=0.13.0 <1.0.0"      // 编辑器版本兼容区间；勿用 ^0.13.0
  },
  ...
  "devDependencies": {
    "@universe-editor/extension-api": "^0.13.0"
  }
}
```
