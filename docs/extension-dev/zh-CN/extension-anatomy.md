...
> 一个 Universe Editor 扩展就是一个带 `package.json` 的 npm 目录：manifest 逐字段说明、激活事件的完整清单、扩展从扫描到 deactivate 的生命周期，以及 `activate` 收到的 `ExtensionContext`。以 API 0.7.1 为准。
...
| `engines` | **整个对象必填**，且必须含 `engines.universe`。它声明的是**扩展 API 版本**区间，不是编辑器版本；推荐 `">=0.7.1 <1.0.0"`，**不要用 `^0.x`**。语义与理由见 [API 版本与 `engines.universe`](./versioning.md)，此处不重复。 |
...
| `contributes` | 静态贡献点（commands / menus / keybindings / configuration / jsonValidation / customEditors / themes / iconThemes / productIconThemes / grammars）。逐项语法见 [贡献点参考](./contribution-points.md)。 |
...
`activate(context)` 收到的对象（以 API 0.7.1 的类型定义为准）：
...
```jsonc
{
  ...
  "engines": {
    "universe": ">=0.7.1 <1.0.0"       // 扩展 API 版本区间（非编辑器版本）；勿用 ^0.x
  },
  ...
  "devDependencies": {
    "@universe-editor/extension-api": "^0.7.1"
  }
}
```
