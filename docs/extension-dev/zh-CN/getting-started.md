...
- `"engines": { "universe": ">=0.7.1 <1.0.0" }` —— 声明兼容的扩展 API 版本区间。**不是编辑器版本**；语义与推荐写法见 [API 版本与 `engines.universe`](./versioning.md)。
...
```
改 src/extension.ts → watch 自动重编 → 开发宿主里执行「重启扩展宿主」命令 → 再验证
```

「重启扩展宿主」（命令面板搜索 *Restart Extension Host*）会停掉扩展宿主进程并重启，重新加载磁盘上的新产物——不需要重开编辑器窗口。
