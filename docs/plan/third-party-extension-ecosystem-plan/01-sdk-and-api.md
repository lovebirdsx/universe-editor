...
扩展作者的 package.json 依赖：
  ...
  @universe-editor/extensions-common    ActivationEvents 等激活事件构造器    【必发】
                                        （COMPATIBILITY.md 明确推荐作者使用，
                                          手写激活事件字符串拼错即永不激活）
工具链（Phase C）的依赖：
  @universe-editor/extension-packaging  createVsix/readVsixManifest          【必发】
                                        （uex 依赖；它 dependencies 里已有
                                          extensions-common，构成同批发布理由②）
  @universe-editor/uex                  CLI（Phase C 交付，本阶段占位 scope）
  @universe-editor/create-extension     脚手架（Phase C 交付，同上）
```

**决策点：extensions-common 直接发布，不做内联拆分。** 备选是把"作者需要的那部分"（activation 构造器）拷进 extension-api、把"packaging 需要的那部分"（manifest/semver）内联进 packaging——拆分能减少对外包数量，但制造了双份真相（manifest 校验逻辑 host 与 CLI 各一份，漂移后果是"CLI 打包通过、宿主拒载"）。发布整包成本更低、真相单一。
...
5. **extensions-common / extension-packaging 同批处理**：补齐同样的元数据；`extension-packaging` 的 `@universe-editor/extensions-common: workspace:*` 与 `adm-zip: catalog:` 由 pnpm publish 自动替换为真实版本（发布后 `npm view` 核对替换结果，这是 pnpm catalog 首次对外，别盲信）。
...
- extensions-common / extension-packaging 从 `0.0.0` 起管理独立 semver（0.1.0 起步），只在有对外可见变更时发版。
...
机制已全部存在（`extensions-common/semver.ts` 的 `satisfies` fail-closed；scanner 跳过不满足的扩展并记日志），本阶段只做**表述层**工作，落点在 Phase E 的 `docs/extension-dev/zh-CN/versioning.md`，草稿随本阶段先行：
...
npm init -y && npm i @universe-editor/extension-api @universe-editor/extensions-common typescript
