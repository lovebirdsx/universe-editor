...
编辑器内置的「用户指南 / 帮助中心」文档源文件树（`docs/user/<locale>/**/*.md`）：随发布产物落盘、运行时读盘，在编辑器里以格式化 markdown tab 呈现。本文是用户帮助文档子系统的上下文地图（处理相关任务前通读）。
...
- **渲染 / 键盘导航**：`DocEditor` **复用 markdown 预览的渲染器与 vimium 式导航**——`MarkdownView`（渲染）、`useMarkdownReaderNav`（f/F link hints、滚动、find、help）、`MarkdownReaderOverlays`、`MarkdownPreviewHelp`。改这些属于 `apps/editor/src/renderer/workbench/markdown/CLAUDE.md` 线②的共享面，动它要同时顾及 markdown 预览与聊天两个消费方。`DocEditor` 自身只是「又一个 markdown 阅读面」。
