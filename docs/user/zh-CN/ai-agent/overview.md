...
- 位置：[辅助侧边栏](../reference/glossary.md#辅助侧边栏)（界面最右侧）的 **"Sessions"** 容器，图标是两个对话气泡。
- 容器内有两个视图：**Sessions**（会话列表与输入框）和 **MCP 服务器**。
- 找不到时，从[命令面板](../reference/glossary.md#命令面板)搜"打开 Sessions 视图"唤出；用 `Ctrl+Alt+B` 可显隐辅助侧边栏。

## 聊天可以换位置

Agent 聊天默认以标签页的形式在**编辑器区**打开——适合一边看文件一边对话。此时辅助侧边栏的 **Sessions** 视图只显示会话列表。
...
| 命令面板名称 | 快捷键 | 作用 |
| --- | --- | --- |
| 打开 Sessions 视图 | 无 | 唤出 Sessions 面板 |
