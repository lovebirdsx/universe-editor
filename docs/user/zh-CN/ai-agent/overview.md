...
- 容器内有两个视图：**Sessions**（会话列表）和 **MCP 服务器**。
- 列表只是目录：点某一行，该会话的聊天会在**编辑器区**以标签页打开——和文件标签页排在一起，适合一边看文件一边对话。输入框就在这个标签页里。
- 找不到列表时，从[命令面板](../reference/glossary.md#命令面板)搜"打开 Sessions 视图"唤出；用 `Ctrl+Alt+B` 可显隐辅助侧边栏。
