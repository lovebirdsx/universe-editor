...
**可视化配置**：AI 设置的"MCP 服务器"面板按来源分组列出所有服务器——User（全局 `settings.json`）与 Workspace（项目 `.universe-editor/settings.json`）两组可以直接在界面里添加、编辑、启停、删除；`.mcp.json` 一组只读展示、可一键打开文件；Extensions（扩展）一组同样只读——要关掉扩展提供的服务器，请禁用该扩展或使用它自己的设置项。同名条目按"项目覆盖全局、全局覆盖扩展"合并，被覆盖的行会标注"overridden by …"而不是悄悄消失；格式有误、运行时会被跳过的条目也会就地给出警告。每行右侧的状态点反映当前会话里该服务器的实时连接状态（已连接/连接中/失败）。
