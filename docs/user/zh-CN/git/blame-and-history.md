...
**点击这个状态栏项，会打开那次提交 / changelist 的全部改动**——在源代码管理侧栏的"提交变更"视图里列出该次提交改动的所有文件（含提交标题、作者、时间），并自动聚焦到当前文件那一行；点击任一文件行即可查看该文件在这版与上一版之间的差异（空格预览、回车打开并聚焦编辑器），方便你顺藤摸瓜看这次改动的来龙去脉。想改为在[历史图谱](./git-graph.md)里定位这次提交，用悬停卡片里的 **"Open in Graph"** 链接（见下节）。
