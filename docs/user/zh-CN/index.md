# 文档中心

欢迎来到 Universe Editor 用户文档。选择下方分区，开始学习你需要的内容。

## 目录

- [快速上手](#快速上手)
- [AI Agent](#ai-agent)
- [编辑与文件](#编辑与文件)
- [搜索与导航](#搜索与导航)
- [版本控制](#版本控制)
- [定制](#定制)
- [参考与排障](#参考与排障)

---

## 快速上手

第一次使用？从这里开始，5 分钟跑通第一个项目。

- [下载与安装](./getting-started/installation.md)
- [界面导览](./getting-started/interface-tour.md)
- [打开第一个项目](./getting-started/first-project.md)
- [命令面板](./getting-started/command-palette.md)

## AI Agent

编辑器的核心能力——让 AI 帮你阅读、编写和管理内容。

- [概览](./ai-agent/overview.md)
- [开始第一个会话](./ai-agent/first-session.md)
- [采纳与回退改动](./ai-agent/reviewing-changes.md)
- [模型与成本](./ai-agent/models-and-cost.md)
- [技能、记忆与 MCP](./ai-agent/skills-memory-mcp.md)
- [模式与思考等级](./ai-agent/modes-and-thinking.md)
- [会话管理](./ai-agent/managing-sessions.md)

## 编辑与文件

日常编辑：文件浏览、标签页、Markdown、书签、大纲、内联补全。

- [资源管理器与文件操作](./editing/explorer-and-files.md)
- [标签页与分屏](./editing/tabs-and-split.md)
- [Markdown 编辑与预览](./editing/markdown.md)
- [编号书签](./editing/bookmarks.md)
- [大纲](./editing/outline.md)
- [内联补全](./editing/inline-completion.md)

## 搜索与导航

在大型项目里快速找到任何内容。

- [全局搜索与替换](./search-navigation/global-search.md)
- [文件内查找](./search-navigation/find-in-file.md)
- [快速打开文件](./search-navigation/quick-open.md)
- [符号与定义跳转](./search-navigation/symbols-and-definitions.md)
- [历史导航](./search-navigation/history.md)

## 版本控制

Git 集成：提交、分支、GitGraph、Blame、冲突解决。

- [概览](./git/overview.md)
- [提交与 AI 提交信息](./git/commit.md)
- [分支与合并](./git/branches-and-merge.md)
- [GitGraph 可视化](./git/git-graph.md)
- [Blame 与历史](./git/blame-and-history.md)
- [会话更改](./git/session-changes.md)
- [冲突解决](./git/conflicts.md)

## 定制

设置、快捷键、主题、插件、AI 供应商密钥。

- [设置](./customization/settings.md)
- [快捷键](./customization/keybindings.md)
- [主题与语言](./customization/themes-and-language.md)
- [插件管理](./customization/extensions.md)
- [AI 供应商配置](./customization/ai-providers.md)

## 参考与排障

快捷键速查、命令速查、术语表、FAQ、排障指南。

- [快捷键速查表](./reference/keyboard-shortcuts.md)
- [命令速查表](./reference/command-reference.md)
- [术语表](./reference/glossary.md)
- [FAQ 常见问题](./reference/faq.md)
- [排障指南](./reference/troubleshooting.md)

---

## 下一步

- [快速上手：下载与安装](./getting-started/installation.md)
- [AI Agent：Agent 是什么，能帮你做什么](./ai-agent/overview.md)
