...
在源代码管理面板里按住 `Ctrl` / `Shift` 可选中多个文件行，随后的行内操作（签出、标记删除、还原、移动到 Changelist、解决冲突等）会作用于**整个选择**，不必逐个执行。破坏性操作（还原、标记删除）会在确认框里显示涉及的文件数量。**资源管理器里的 Perforce 右键命令同样支持多选**：选中多个文件再右键，签出 / 新增 / 删除 / 还原 / 收集 / 丢弃未收集的改动都会作用于整个选区（收集与丢弃对目录按 `<dir>/...` 递归，其余命令只作用于选区中的文件）。
