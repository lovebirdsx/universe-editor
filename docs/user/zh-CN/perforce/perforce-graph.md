...
也可以"带着目标 changelist 来"：从[时间线](./daily-workflow.md#时间线文件历史)历史行的行内按钮 / 右键菜单，或 Blame 悬停卡片里的 **"Open in Graph"** 链接——图谱会直接**选中并滚动定位到该 changelist**，未加载时自动向前翻页查找（最多补 20 页）。
...
- **单击某个 changelist**，源代码管理侧栏的"提交变更"视图会列出它改了哪些文件，并按状态着色（已添加 / 已修改 / 已删除 / 已重命名）。点击文件行会打开该文件这次改动的差异。
...
图谱会自动跟随服务器上的新变化刷新；需要手动拉一遍最新历史时，在图谱激活时按 `Ctrl+R`，或运行命令 **Refresh**（刷新，分类 "Perforce Graph"），与工具栏的 ↺ 按钮等效。
