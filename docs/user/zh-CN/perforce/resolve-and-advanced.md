...
- **改了文件但面板里没有**：普通改动会出现在面板的 **Working Tree Changes** 组；若该组隐藏、资源管理器也没有 RC 徽标，说明文件已签出且没有未收集改动，或已被[收集改动](./daily-workflow.md#收集改动reconcile)。想把它收进某个 changelist，在资源管理器右键 **收集改动** 即可。
