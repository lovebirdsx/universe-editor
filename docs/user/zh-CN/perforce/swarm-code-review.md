...
顶部有关键词过滤框，输入即筛选：关键词会**直接下推到服务器查询**（而非先拉取全部再在本地过滤），并**自动刷新**结果，无需手动重载。**单击**某行在主编辑区打开审核详情；**右键**可直接打开审核、在浏览器中打开、执行服务器允许的状态操作、复制审核名称或网页链接，以及丢弃审核。
...
- **过滤「Needs My Action」**（漏斗+列表图标）：点开一个小菜单，可配置两条规则——
  - **发起者集合**：从当前看板出现过的作者里勾选（支持多选，空集=不过滤），只显示发起者在集合中的审核；
