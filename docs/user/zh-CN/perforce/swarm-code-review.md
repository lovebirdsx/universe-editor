...
审核列表是一棵可用键盘完整操作的树，运行 **"Show Swarm Reviews"** 或点活动栏图标后，焦点直接落在列表上（无选中行时自动落到第一条审核）：
...
| `Ctrl+Enter` | 把焦点跳到 **Swarm Changes** 子视图 |
...
**Swarm Reviews** 下方还有一个 **Swarm Changes** 子视图，随列表当前选中的审核**自动显示它的变更文件**，交互与源代码管理里的 **Commit Changes** 完全一致：
...
未选中任何审核时该视图显示「Select a review to see its changes.」；选中的审核没有变更文件时显示对应提示。
...
| `perforce.swarm.notifications.enabled`     | `true`           | 有新审核进入「Needs My Action」时通知：窗口未聚焦（或你已离开电脑——锁屏 / 约 2 分钟无键鼠输入）弹系统桌面通知，聚焦且在用时改为应用内通知。需先开启 `perforce.swarm.backgroundPoll.enabled`。 |
