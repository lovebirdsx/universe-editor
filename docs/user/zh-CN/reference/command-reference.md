...
| 将选区添加到已有 Agent 聊天   | 把选中文本附到已打开的聊天；有多个会话时先选一个                                                        | `Ctrl+K Ctrl+L`                                   |
| 将选区添加到新建 Agent 聊天   | 新建一个聊天，把选中文本附上去                                                                          | 无                                                |
| 恢复 Agent 会话…              | 打开历史会话                                                                                            | `Ctrl+Shift+H`                                    |
| 切换会话…                     | 跨窗口切换会话；按住 `Alt` 连点 `S` 逐行移动，松开 `Alt` 直接切换                                       | `Alt+S`                                           |
| 切换到上一个会话…             | 同「切换会话…」，反向开局                                                                                | `Alt+Shift+S`                                     |
| 新建侧边任务                | 基于当前会话直接开一条只读侧边任务（在右侧分屏打开）；划选后创建会自动带上引用块                      | 无                                              |
| 打开侧边任务…                 | 列出当前会话的直接下级侧边任务，选中即在右侧分屏打开                                                    | 无                                                |
| 转到父会话                    | 从侧边任务跳回它所属的父会话；父会话标签页已打开则直接聚焦                                              | 无                                                |
