...
| Refresh（英文） | 刷新 Git 图谱；仅图谱激活时生效，中文释义"刷新" | 图谱内 `Ctrl+Shift+R` |
...
> Perforce 侧的等价功能为 **View Perforce Graph**（查看 Perforce Graph）、图谱内 `Ctrl+F` 的 **Focus Search** 与图谱内 `Ctrl+Shift+R` 的 **Refresh**，同样只显示英文。更多见 [提交历史图（Perforce 图谱）](../perforce/perforce-graph.md)。
...
| --- | --- | --- |
| 检查更新 | 手动检查有无新版本 | 无 |
| Report Issue...（英文） | 报告问题：生成诊断摘要（版本 / 系统信息 / 扩展 / 近期错误聚合）并复制到剪贴板，可选择附带诊断包（自动上传为附件），随后打开预填好的上报页面（默认 iLoop，可切换 GitHub） | 无 |
