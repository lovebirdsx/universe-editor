...
| 向上 / 向下复制行 | `Shift+Alt+↑` / `Shift+Alt+↓`（Linux 不同，见下） | Copy Line Up / Down |

> 按词删除用 `Ctrl+Backspace` / `Ctrl+Delete`，移动行请看[被本编辑器占用的键](#被本编辑器占用的键)——`Alt+↑` `Alt+↓` 在本编辑器里是别的功能。
...
> `Ctrl+K Ctrl+L`（切换折叠）被本编辑器占用了，见下节。
>
...
Mac 只做 `Ctrl` → `Cmd`、`Alt` → `Option` 的换算。**Linux 不同**，因为编辑器内核按平台注册键位，而系统里显示的是 Windows 一套。Linux 上以这张表为准：

| 操作 | Windows | Linux |
| --- | --- | --- |
| 向上 / 向下复制行 | `Shift+Alt+↑` / `Shift+Alt+↓` | 无可用默认键（`Ctrl+Alt+Shift+↑` / `↓` 被「调整视图高度」占用，见下节） |
| 在下方 / 上方加光标 | `Ctrl+Alt+↓` / `Ctrl+Alt+↑` | `Shift+Alt+↓` / `Shift+Alt+↑` |
| 切换块注释 | `Shift+Alt+A` | `Ctrl+Shift+A` |
| 格式化文档 | `Shift+Alt+F` | 无可用默认键（`Ctrl+Shift+I` 被「开发者工具」占用） |

> **为什么**：编辑器内核的默认键在 Windows 与 Linux 上并不完全一致，而本编辑器只镜像了 Windows 那一套。所以「键盘快捷方式」页面（`Ctrl+K Ctrl+S`）里显示的可能是 Windows 键；**以本页为准**。想固定下来，按[定制 · 键盘快捷方式](../customization/keybindings.md)自己绑一个。

## 被本编辑器占用的键

下面这些键在编辑器内核里本来另有含义，但本编辑器把它们给了更常用的功能。表格里写的是**在本编辑器里按下去实际会发生什么**：

| 键 | 实际作用 | 内核里本来的含义 |
| --- | --- | --- |
| `Alt+↑` / `Alt+↓` | 查找光标处单词的上一处 / 下一处 | 上移 / 下移行 |
| `Ctrl+K Ctrl+L` | 把选区添加到已有 Agent 聊天 | 切换折叠 |
| `Alt+PageUp` / `Alt+PageDown` | 文件有未提交改动时：上一处 / 下一处更改；没有改动时是滚动一页 | 滚动一页 |
| `Ctrl+0` … `Ctrl+9` | 跳转到编号书签 | — |
| `Ctrl+Shift+I` | 打开开发者工具（Linux 上这会挡住「格式化文档」） | — |
| `Ctrl+Alt+Shift+↑` / `↓` | 调整聚焦视图的高度 | 列选择 / （Linux）复制行 |
| `F1` | 命令面板（与 `Ctrl+Shift+P` 相同） | 编辑器内快速命令 |
| `Ctrl+F` / `Ctrl+H` | 编辑器内查找 / 替换（同一个功能） | 编辑器内查找 / 替换 |

> 行被占用的键想要回来，可以自己重新绑定，见[定制 · 键盘快捷方式](../customization/keybindings.md)。
