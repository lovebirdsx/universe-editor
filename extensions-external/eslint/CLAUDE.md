...
对等 vscode-eslint 的**外置插件**（以 `.vsix` 经市场/本地安装，**不在 pnpm workspace 内**，对齐 `extensions-external/pdf`）。本文是在本插件里开发/调试的上下文地图（处理相关任务前通读）。

## 在 ESLint 外置插件里开发/调试功能

`extensions-external/eslint` 是**独立 LSP server 架构**：client 跑在扩展宿主内，server 是宿主 spawn 的 Electron-as-node 子进程，两者用**自定义精简协议**（非标准 LSP 全集）over stdio 通信。server 在运行时解析**工作区自己的 eslint**（`node_modules` 里那份，绝不打包），把 ESLint 的 offset-based 结果映射成 LSP 诊断 / TextEdit。
