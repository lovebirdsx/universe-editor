...
- **回归护栏**：这条 phantom delete `@regression` 已迁进 `e2e/specs/perforceWorkingTreeHint.spec.ts`——盘上改过的未签出文件 → Explorer 出 `RC` 徽标 → `perforce.openChange` 打开 diff → 断言 modified 侧 == 真实盘上内容（不是空）。改坏 `clientToLocalPath` 会红。单测见 `pathUtil.test.ts`/`openedParser.test.ts`/`reconcileParser.test.ts`。
...
- **还原（统一入口）**：`perforce.revert` 按打开状态分流——已打开走 `revert(paths)` = `p4 revert`（离开 changelist + 丢本地改动）；未打开走 `revertReconcile(paths)` = `p4 clean`（旧「丢弃未收集的改动」）。确认框在含已打开文件时列出将离开 changelist 的文件（`revertPlan.ts`）。Explorer 目录去掉 `!explorerResourceIsFolder`，`openedInTree(dir)` 做 live `p4 opened dir/...` 预检后始终 `p4 clean dir/...`，有 opened / fail-open 才叠加 `p4 revert dir/...`。`client.revertReconcile` 只作执行原语，不再有独立菜单/命令面板入口。
...
- **目录级命令**：explorer 右键传 `{ resource, isDirectory }`（见 `ExplorerContextMenu` args）；handler 读 `isDirectory` 决定是否把路径转成 p4 递归语法 `<dir>/...`（见 `extension.ts` `perforce.reconcile` / `perforce.revert`：目录 → `${path}/...`，复用同一命令，不新增命令）。要支持目录版的其它 p4 操作照此办：菜单项去掉 `!explorerResourceIsFolder`，handler 分叉 `<dir>/...`。**判定必须叠 `selectionPaths(args[1]).length === 0`**：SCM 树文件夹行也会带 `isDirectory:true`，但其 selection 已是子树文件，不能再 glob。
- 行选择靠 `scmResourceState`（单字母，来自 `p4Decoration.ts` `contextValue`：E/A/D/B/I/M，未 resolve=U，搁置=S）。组选择靠 `=~ /^cl:/` / `=~ /^shelved:/`（正则）。
