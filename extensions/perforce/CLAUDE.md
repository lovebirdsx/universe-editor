...
## ⚠️ sync 拒绝有**三个**形态，只解析一个就会谎报「已是最新」（踩过）
...
- **sync 成功尾部要做 drift 收缩（`_removeDriftForSyncRun`）**：sync 不改 `_driftFiles` 就走不到 `_mutate` 的 `_invalidateAfterMutation`（它只做减法），watcher 又被 `_suppressExternalChanges()` 抑制，所以一个被 force get 覆盖的文件（磁盘已 == 新 have rev）会**永远留在 Changes 组直到下个会话**。修法是**只做减法**：sync 逐文件吐的 applied 行（`<depot>#<rev> - <verb>[ as] <local>`，verb ∈ updated/added/deleted/refreshing/refreshed/updating）就是 p4 自报的「实际覆盖了哪些文件」清单——`parseSyncApplied` 把它解析出来（流式路径在 `onStdoutLine` 闭包里顺手收集，非流式路径从 buffered stdout re-parse），`_removeDriftForSyncRun(applied, refused, refusedOverwrite)` 对 applied 行减掉 `_driftFiles`（先差集排除 refused——clobber run 会先打 `- updating <local>` 再失败，那行被 APPLIED_LINE 计数但其实没落盘）。**只减不扫**：sync 不制造漂移只消灭漂移，重扫整个 scope（常是 `//...`，几十万文件）绝不可接受。**解析缺口兜底**：`appliedFiles.length < summary.applied`（行被计数但提取不出本地路径）时跑 `_revalidateDriftAfterSync()`——对剩余漂移行做一次窄 `reconcile -n`（watcher flush 同款），超 `MAX_EXTERNAL_NARROW_PATHS` 预算就留给下会话扫描。取消分支同样收缩（abort 前已 applied 的确实落了盘）。
...
- `sc.count` = 已签出文件数 + 未收集漂移数（「Changes」组的真实总数 `_driftTotal`，截断前），对齐 git 的 staged+working 角标语义；统一由 `_updateScmCount()` 收口（`_applyDriftGroup`/`_doRefresh`/`_goOffline` 三处接线）；`acceptInputCommand`/`acceptInputActions` 在默认组有文件时挂 Submit / Revert Unchanged。
