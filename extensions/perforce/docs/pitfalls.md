...
- [已消失的路径不是文件 spec：裸路径查不出被删目录的子树](#-已消失的路径不是文件-spec裸路径查不出被删目录的子树真机实测)
...
- **`-f` 是逃生阀，不是默认补救**。对 refusedModified，p4 拒绝的唯一原因就是文件里有未收集的工作，而 `sync -f` 会永久销毁它（§11.2 已实测覆盖本地草稿）；对 refusedOverwrite，`-f` 是**唯一**能挪走孤儿占位的途径。所以三条硬约束缺一不可：① 它只能作为**显式按钮**出现（绝不自动重跑成 `-f`）；② 点击后弹**勾选式 per-file force 多选列表**（`pickForceGetFiles`）而非盲目全量重跑；③ 对 refusedModified，默认路径仍是先收集——按钮顺序固定为 `收集改动 → 查看差异 → 强制拉取`（收集是无损的、p4 会排合并，故领先；force 排在看过 diff 之后）。按钮集由纯函数 `refusedSyncButtons({refusedModified, refusedOverwrite, mustResolve, allowForce})` 拼装（`extension.ts`，带单测）：**`refusedModified > 0` 时给 collect/diff/force（orphan 计数只在 message 里呈现，refusedModified 语义优先）；纯 `refusedOverwrite > 0` 时只给 force**（不给 collect/diff，语义不符）。`allowForce` 在本次 run 已经是 `-f` 时为 false——同样的拒绝再给一次 force 只会原地打转。clobber 分支同样给 force，但**不给「查看差异」**：它走 stderr/exit 1，`refusedFiles` 从 stdout 解析故为空，没有 per-file 本地路径可 diff（也无 per-file 列表可勾选，故 clobber 的 force 仍是原 scope 重跑 + `confirmForceGet(spec, scopeText)`）。
- **`-f` 的四个显式入口，两种确认形态**：① Explorer/命令面板 `perforce.sync` 的 quick pick **后四项**（label 前缀「强制拉取：」+ `labelColor: 'force'` 整行红；`#head` 只是其中一档，changelist/日期/修订号各有一档强制版）——事前，走共享的 `confirmForceGet(spec, scopeText)` 弹窗（**正文点名真实会跑的 spec 与 scope**，见下）后按选区 scope 全量 `-f`；② 上述 refusal 补救（事后，per-file picker）；③ **图谱行右键 "强制拉取（覆盖本地文件）"**（事前，同一个 `confirmForceGet`）。第 ③ 个是唯一**同时**携带两层破坏性的入口——它既是 `-f`（覆盖未收集改动）又是时间旅行（重置未签出文件），故把两层语义**合成一个弹窗**而不是叠两个 modal（Explorer 侧的 ① 现在也共用这句文案；正文用「目标版本」而非「changelist」，因为同一个 spec 位可能是 `@4521` / `@2026/08/01` / `#4` / `#head`，写死 changelist 会撒谎）。它走纯函数 `graphSyncConfirmKind`（三态，`graphSync.ts`），**`force` 压过 `graphSyncNeedsConfirm` 的全部三条豁免**（`isLatest` / `confirmed` / 单文件 scope）——`force` 刻意不进 `GraphSyncConfirmInput`，传进去即编译错误；少了这条，对最新行（`isLatest: true`）右击强制拉取会**静默**覆盖本地工作。测试双守卫：`graphSync.test.ts` 的纯函数真值表 + `PerforceGraphEditor.test.tsx` 对 payload 的 `toEqual` 精确断言（多带一个 `isLatest` 就红）；picker 那四档则由 `syncSpec.ts` 的真值表守（**档位顺序即屏幕顺序**，force 组必须在后——e2e 直接按 Enter 选第一项）。
- **`quick pick` 的单选形态没有 title 位**（`MainThreadWindow.$showQuickPick` 只透传 `placeHolder`，`title`/`okLabel` 仅多选生效），且渲染端的 `labelColor` 是**闭集**（`QuickInputPanel.labelColorClass` 只认 `modified`/`orphan`/`force`，未知 id 静默渲染默认色）。所以给单选 picker 加一档「危险项」需要三处一起动：色值注册（`universeColorIds.ts` 的 `picker.forceLabel`，被 `cssVarCoverage.test.ts` 当契约守）+ CSS 类（`QuickInput.module.css`）+ 映射分支；而**红色行的含义只能写在 label 与 placeHolder 里**（没有图例位），故 `perforce.syncPick.placeholder` 现在带一句「红色项为强制拉取，会覆盖本地文件」。
...
- **UI 侧 150ms 节流**（`extension.ts` 的 `PROGRESS_REPORT_INTERVAL_MS`）：p4 每文件一行，不节流就是上万条 RPC。本扩展**首次用** `window.withProgress`（`ProgressLocation.Notification`，不定形态 + `cancellable`）；取消按钮与状态栏 spinner **统一经 `extension.ts` 的 `confirmAndCancelBusy` 二次确认**（先 `showWarningMessage`，确认后**复查 `cancellableEpoch` 未变**才 `target.cancelBusy()`——确认框是异步缺口，期间本次 run 可能已结束、后续 mutation（get 后的 collect）可能已起来，不复查就会误杀用户没看到的操作）；abort 机制仍然只有一套。**拒绝确认时 token 已经 trip 且不可撤销**：`runSync` 的 `res.cancelled` 来自 `target.sync()` 的返回值、只由 p4 子进程是否被 kill 决定，所以同步照常跑完、ledger 照常记账；通知上那个取消按钮因此需要平台层 re-arm（`NotificationService.cancelProgress` 在通知未 dismiss 时重新武装 handler），否则拒绝后按钮变哑。状态栏侧另有一条 200ms 节流（`client.ts` `_bumpSyncProgress`，镜像 `_setScanProgress`）。
...
- **对策 C（已落地）**：**读 p4 进程自己的 OS IO 计数器，状态栏显示读写合计速率**（`processIo.ts` + `SyncProgress.ioReadBytes`/`ioWriteBytes`）。真机观察确认了对策 A 填不了的正是最需要填的那段：sync 早期 p4 在服务端比对文件列表 / 把内容先收进临时区，**一个工作区文件都还没写**，所以 `done` 与 `disk +N` 同时冻结在 0——对策 A 的计数只覆盖「已经开始落盘」之后。进程计数器则从第一次服务器往返就在涨，用 `p4Service.P4ExecOptions.onSpawn(pid)` 拿到 pid 后起采样器。
  - **为什么只有这条路**：Node 没有跨平台的子进程 IO 计数器（`@vscode/windows-process-tree` 不含 IO 字段）；Windows 上 `GetProcessIoCounters` 只能经 WMI（`Win32_Process.ReadTransferCount`，任务管理器同源，**含网络接收**）→ 常驻 `powershell.exe -NoProfile -NonInteractive -EncodedCommand`（脚本 base64 编码，不碰 Windows argv 引号雷区）每秒轮询一次并按 pid 表输出**每拍增量**（新出现的 `--parallel` worker 取其当轮值，worker 退出不产生负增量/假峰值，pid 复用因差值为负被钳 0）；Linux 直接读 `/proc/<pid>/io` 的 `rchar`（**不是** `read_bytes`——那是块设备读，p4 从网络收）；macOS 不实现。
  - **`p4 -I` 不在选项内**：进度协议会让 `--parallel` 静默失效（见本节开头），大仓库不可接受。
  - **降级与红线**：采样器起不来（macOS / PowerShell 被策略禁用 / WMI 挂）只写一行日志，状态栏回落 `disk +N`，**绝不 toast、绝不 throw**——采样回调跑在异步 stdout/定时器里，宿主崩溃红线同 `onStdoutLine`。失效分两档（`P4IoProbeFailureKind`）：**`missing`**（spawn 失败 / 一行都没执行就退出——启动行 `S 0 0` 是判据）按 **session 锁存**，之后不再白起进程；**`transient`**（15s 无输出判卡死、WMI 连续 5 拍失败、闲置超时）只作废**本次**运行，下次 sync 照常重试——一次卡顿升级成「整个 session 没速率」是过度反应（WMI 服务重启、机器高负载都能触发）。采样器自己也会在 p4 消失或**连续 15 分钟无字节流动**时退出（宿主崩溃不留残留）；**刻意不设挂钟 deadline**：整仓拉取本来就跑几十分钟，中途砍掉采样器正好把状态栏冻在 `000KB/s`——正是本对策要消除的假「卡死」信号。`UNIVERSE_P4_IO_PROBE=off` 关闭、其它值 = 覆盖采样命令（e2e 缝，脚本形态经 `process.execPath` 跑并**补回 `ELECTRON_RUN_AS_NODE`**，否则起成第二个编辑器窗口——与 fake p4 同一坑）。
  - **定宽是硬要求**：状态栏是一行，速率若随数字变长会每秒把相邻条目推来推去。故 3 位补零尾数 + 4 字符单位 = 恒 7 字符（`000KB/s` / `042MB/s` / `999MB/s`），配状态栏 CSS `font-variant-numeric: tabular-nums`。**刻意不用空格类填充**：普通空格被 HTML 折叠、NBSP 宽度 ≠ 数字宽度、U+2007 有缺字回退风险，前导零是唯一零字体风险的定宽手段。
  - **速率在渲染时算**（`RateWindow`，与 `elapsed` 同一范式）：心跳每 1s push 一次当前累计字节，写入停止后窗口自然滑走 → 衰减回 `000KB/s`，不需要 p4 再产生任何事件。有采样器时速率**替换**计数占位（计数进 tooltip，不重复显示）。
  - **喂进窗口的是 `ioReadBytes + ioWriteBytes`，不是只喂读**：两侧在不同阶段动（读在拉取、写在落盘），只喂读会在后段——**正好是磁盘最忙的时候**——衰减成 `000KB/s`，把本对策要消除的假「卡死」信号原样还回来（真机复现过；e2e `perforceSyncIoRate` 的写入阶段断言是这条的回归护栏）。合计**单调**（任一侧增长即增长）所以窗口差分仍是一个速率；但**合计不是传输量**：同一个字节可能先作为 socket 读、再作为中转临时文件写各记一次，数字高于实际传输的量。tooltip 保留读写分列，正文只出现合计。
  - **语义提醒**：读数含网络接收与中转临时文件，是「进程在干活」的活性信号，**不等于落盘速度**；tooltip 里明写这句。
...
- **诊断法**：状态栏数字冻结 ≠ UI 卡死。看 Perforce 输出频道：sync 期间持续有 reconcile 批 spawn（洪流）或完全没有新行（`--parallel` 静默期）都指向这里；真机验证判据=冻结期间 `磁盘 +N`（对策 A）或速率段 `042MB/s`（对策 C）是否还在跳动。两者都不动 = 早期服务端比对阶段（对策 C 尚未起飞或已降级）；对策 C 降级时输出频道有 `sync I/O rate unavailable` 一行——**整个 session 只出现一次 = 这台机器没有可采样的源（`missing`，已锁存不再重试）；每次 sync 都出现一次 = 每次都被卡掉（`transient`，多半是 WMI 负载/服务重启），功能仍在重试**。
...
- **scope 推算按时间取、不按深度取**。对目标 scope，候选 = 所有**包含**它的记录（自身 + 更宽祖先，**不含更窄的**），取 `at` 最新那条。按「最具体优先」是错的：先 `sync A/B@4520`、再 `sync A@4560`，A/B/C 的答案应是 **4560**（后一次把整个 A 推到了 4560），按深度会答 4520 = **低报**。而反过来「更窄的记录回答更宽的问题」是**高报**——和上面第 2 条发现的 fake-p4 bug 是同一个错误方向。
- **答案来自更宽 scope 时是上界**（`widerScope`），UI 必须标注；同理 `complete: false`（有文件被拒绝 / 留在打开态 / 需要 resolve）= 部分同步，也只能算上界。

## ⚠️ 已消失的路径不是文件 spec：裸路径查不出被删目录的子树（真机实测）

**现象**：在工作区里删掉**一个目录**，Perforce 的「Changes」组不出现其下文件的删除（应出 `RD`）；而新增文件、修改文件、删除**单个文件**都正常。

**根因**：`stat` 抛错被当成「这是被删除的**文件**」——`_flushExternalChanges` 原先把 pending 路径二分为 `directories`（只失效 checkpoint）与 `files`（逐文件窄查），判据是 `_isDirectoryPath` 的 catch 分支。而 Windows 回收站删除 = **同卷 rename**，parcel watcher 对「整棵子树被移出监视树」只产生**一条**目录级 `delete` 事件（子树内文件**没有**逐文件事件）。于是消失的目录落进 `files` 桶，发出 `reconcile -n <目录裸路径>`。

**三条真机实测判据**（只读，`-ztag`）：

| 命令 | 结果 |
|---|---|
| `reconcile -n -a -e -d "<磁盘上不存在的路径>"` | **exit 0** + stdout 空 + stderr `<path> - no file(s) to reconcile.` |
| `reconcile -n -a -e -d "<同一目录>/..."` | 正确返回子树内所有「在 have 列表但磁盘缺失」的文件，`action delete` |
| 混合批 `["<不存在的裸路径>", "<目录>/..."]` | **exit 0**，delete 记录照常返回，stderr 空——裸 spec 匹配不到**不污染整批** |

**p4 的裸路径永远是单文件 spec**：目录展开按文件系统判断，路径没了就不再是目录——所以「路径不存在」推不出「它是个文件」。反过来 `<dir>/...` 的 `...` 展开走 depot/have 列表，**与磁盘上目录是否存在无关**（用一个「磁盘上是空目录、have 里有 2 个文件」的目录验证过）。

**修法红线**：
- 三态回答（`_pathKind`：`dir` / `file` / `gone`），`gone` **成对**发 `[<path>, <path>/...]`——裸 spec 覆盖「删的是文件」，`/...` 覆盖「删的是目录」。只发 `<path>/...` 会让删单文件丢失答案。
- 含 exclude 子目录时**只能 carve，绝不退化成 `<dir>/...`**（同扫描那条红线）；carve 失败退化为「只失效 + 日志」。
- 归属不能只靠精确回显：`<dir>/...` 的答案回显**文件**路径，与请求的目录路径对不上，须按子树包含关系认领。
- **`covered` 必须含被问路径本身**：扫描轮 settle 按字符串精确比对决定是否丢父 checkpoint，被问路径缺席会白丢覆盖它的检查点。
- stderr 分支**不能丢 `res.records`**：新路径的批含匹配不到的裸 spec，旧写法会把 `/...` 吐出的真实 delete 记录整批丢掉——等于在新路径上复刻同一个 bug。同款教训见 `checkIgnore`。

**连带发现（同源，已一并修）**：目录被**改名/移入**工作区时同样只有一条目录级事件，而 `directories` 桶只失效不查询 → 其内容本会话也不显示。（旧注释断言「新目录里的文件会各自来事件」只对**新建**成立。）

**回归护栏**：`clientReconcileScan.test.ts` 的「删除的目录按子树回答」「目录事件按子树回答且 checkpoint 就地修正」「carve 不退化成 `<dir>/...`」等用例，以及 `e2e/specs/perforceDirectoryDelete.spec.ts`（见 `e2e/CLAUDE.md`）。
