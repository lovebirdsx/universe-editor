...
## 14. 2026-09-18 删除目录后 SCM 不显示（只读实测，裸路径 vs `/...` 的三态）

验收 bug：在编辑器里删掉工作区的一个**目录**，Perforce 面板的「Changes」组不列出其下文件的删除；新增/修改/删单文件都正常。真机只读探测（`p4 info` / `fstat` / `-ztag reconcile -n`，**未跑任何写操作**，也未改动工作区）。

### 14.1 核心判据：裸路径永远是单文件 spec

对一个**磁盘上已不存在**的路径（用「磁盘上是空目录、have 里有 2 个文件」的对照目录验证 `...` 展开，再对不存在的路径验证裸 spec）：

| 命令 | stdout | stderr | exit |
|---|---|---|---|
| `reconcile -n -a -e -d "<不存在的路径>"` | 空 | `<path> - no file(s) to reconcile.` | **0** |
| `reconcile -n -a -e -d "<同一目录>/..."` | 该目录 have 列表中「磁盘缺失」的文件，`action delete` | 空 | 0 |
| `reconcile -n -a -e -d "<不存在的裸路径>" "<目录>/..."`（混合批） | 只有 `/...` 的 delete 记录 | 空 | **0** |

两条结论：**①** p4 的裸路径不做目录展开，目录展开按文件系统判断——路径没了就不再是目录，所以「不存在」推不出「它是个文件」；**②** `<dir>/...` 的展开走 depot/have 列表，**与磁盘上目录是否存在无关**。③ 匹配不到的裸 spec **不污染整批**（混合批仍 exit 0 且正常返回记录），所以成对发 spec 是安全的。

### 14.2 与产品行为的连接

Windows 回收站删除 = 同卷 rename → parcel watcher 对「整棵子树移出监视树」只给**一条目录级 delete**（子树内文件无逐文件事件）。原实现把 `stat` 抛错当成「被删除的文件」，于是发出裸路径查询、拿到 exit 0 + 空 stdout，读成「查过且干净」，还把「干净」写进 24h 有效的 checkpoint。修法见 `../docs/pitfalls.md` 的「已消失的路径不是文件 spec」条。

### 14.3 fake-p4 对齐（本轮改了两处）

- **dry-run 空结果 `return 1` → `return 0`**：真机是 **exit 0 + stderr `<spec> - no file(s) to reconcile.`**，fake 原来返回 1，会把「匹配不到」在 e2e 里伪装成失败——而新路径的批**必然**含一个匹配不到的裸 spec。
- **`targetsFromArgs` 的匹配语义**：原来对任意 spec 做 `<path>/...` 前缀匹配，等于让裸路径也能前缀命中（真机拒绝的查询在 fake 里有了答案）。现在**并集**：精确匹配（本地路径 / depot 路径）∪ 仅对 `<path>/...` 形态做前缀匹配。
- 新增 **`UNIVERSE_P4_FAKE_ARGV_LOG`**（照 `UNIVERSE_P4_FAKE_READBACK_LOG` 先例）：每次 reconcile 的 argv 追加一行，spawn 后立刻写、try/catch 吞异常、绝不失败命令。它是「查询**形态**」类断言唯一的证据来源（见 `../CLAUDE.md`）。

### 14.4 未验证

- **`<file>/...` 的语义**：本轮只测了目录。对文件路径加 `/...` 真机会怎么答（空？报错？退化成文件？）**没有实测**——产品对 `file` 三态只发裸 spec，不依赖这个答案。
- **服务器版本差异**：以上是 P4D 2024.2 的行为；文档化语义如此（裸 = 单文件 spec），但未在其它版本上复测。
- **Linux/macOS 的 watcher 事件形状**：Windows 上删除必现「一条目录级事件」；其它平台可能退化成逐文件事件（症状本就较轻，不走这条路径）。e2e 用「同卷 rename + 形态不变量」两头钉，见 `../CLAUDE.md`。
