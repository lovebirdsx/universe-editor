...
- 判读套路：先看 renderer 有没有 tick 进来（无 → host poller / RPC 问题，看 `extensionHost.log` 有没有 `poll driver every` 与连续的 `not acknowledged`——**连续 `not acknowledged` = renderer 被节流/挂起；`tick gap` = 停摆时长实锤**）；有 tick 但全是 dropped（→ 上一次 refresh 卡住，看相位计时卡在哪个阶段 + host `api`/`auth` scope 是否有挂起/超时请求）；有 poll ok 但无 notifying（→ 过滤口径问题：authors 白名单 / approvable / ignore）；有 notifying 但系统没弹（→ main 侧 host.log 看 shown/skipped 决策，focused+present 被吞属焦点门控语义，见上节）。
