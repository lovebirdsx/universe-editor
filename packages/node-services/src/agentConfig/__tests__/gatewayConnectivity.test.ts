...
describe('probeGatewayConnectivity', () => {
  it('resolves true when the server answers with any HTTP status', async () => {
    ...
    await expect(probeGatewayConnectivity('http://192.0.2.30:8080', fetchImpl)).resolves.toBe(true)
  ...
  it('resolves false on network errors', async () => {
    ...
    await expect(probeGatewayConnectivity('http://192.0.2.30:8080', fetchImpl)).resolves.toBe(false)
  })
