...
/**
 * Per-model usage breakdown for the current day, from the Claude API usage
 * endpoint. Monetary fields keep the provider's raw integer scale (divide by
 * 10000 for the CNY amount); formatting is the renderer's job.
 */
 ...
 * integer scale; the renderer formats them for display.
 */
...
/**
 * Result of a Claude API-usage query. Discriminated union so the renderer can
 * react distinctly: `disabled` (credentials missing — hide the indicator),
