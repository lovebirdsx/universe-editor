 ...
 *  the next token as its value. `string[]` items collect EVERY occurrence (repeatable
 *  flags like `--extension-development-path`), preserving argv order; other types
...
export class CliConfigSource implements IConfigSource {
  ...
  getRawValue(item: ConfigItem): RawConfigValue {
    ...
    for (let i = 0; i < this._argv.length; i++) {
      ...
      if (eqForm) {
        const value = arg.slice(eqForm.length + 1)
        if (item.type === 'string[]') {
      ...
      if (forms.includes(arg)) {
        ...
        // yields no usable value. For string[] that occurrence is dropped but
        // later occurrences may still supply values, so keep collecting.
        if (item.type !== 'string[]') return undefined
      }
    }
    if (item.type === 'string[]') return collected.length > 0 ? collected : undefined
    return undefined
  }
}
