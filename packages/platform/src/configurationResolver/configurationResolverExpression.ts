 ...
 *  Ported from VSCode's ConfigurationResolverExpression
 *  (workbench/services/configurationResolver/common/configurationResolverExpression.ts).
 *
 *  Parses `${name}` / `${name:arg}` replacements out of a string or an arbitrary
 *  object tree using brace-counting (so nested braces and args containing spaces
 *  parse correctly), tracks every location a given replacement occurs, and lets a
 *  resolver substitute values back in — re-parsing freshly substituted text so a
 *  variable that expands to another `${...}` is picked up in the same pass.
 *
 *  Trimmed vs upstream: no `ConfiguredInput` coupling and no VSCode `Iterable`
 *  helper dependency (plain arrays/generators instead).
 *--------------------------------------------------------------------------------------------*/
...
type PropertyLocation = {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any -- arbitrary config object tree
  object: any
  propertyName: string | number
  replaceKeyName?: boolean | undefined
}
...
export class ConfigurationResolverExpression<T> {
  ...
  private constructor(object: T, platform: HostPlatform) {
    this.platform = platform
    // If the input is a string, wrap it in an object so we can use the same logic.
    if (typeof object === 'string') {
      this.stringRoot = true
      // eslint-disable-next-line @typescript-eslint/no-explicit-any -- string root is boxed
   ...
   */
  public static parse<T>(object: T, platform: HostPlatform): ConfigurationResolverExpression<T> {
    if (object instanceof ConfigurationResolverExpression) {
      return object
    }

    const expr = new ConfigurationResolverExpression<T>(object, platform)
  ...
  private applyPlatformSpecificKeys() {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any -- already cloned by ctor, safe to change
    const config = this.root as any
    const key =
      this.platform === 'win32'
        ? 'windows'
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any -- walks an arbitrary config tree
  ...
  private parseString(
    // eslint-disable-next-line @typescript-eslint/no-explicit-any -- container in an arbitrary config tree
    object: any,
    propertyName: string | number,
    value: string,
  ...
  /** Gets the replacements which have not yet been resolved. */
  public *unresolved(): Iterable<Replacement> {
    ...
    while (true) {
      const next = newReplacements.entries().next()
      if (next.done) {
        break
      }
  ...
  public *resolved(): Iterable<[Replacement, IResolvedValue]> {
    for (const l of this.locations.values()) {
      if (l.resolved) {
        yield [l.replacement, l.resolved]
      }
    }
  }

  /**
   * Resolves a replacement into the string value. If the value is undefined,
   * the original variable text will be preserved.
   */
  public resolve(replacement: Replacement, data: string | IResolvedValue): void {
    ...
    if (data.value !== undefined) {
      for (const l of location.locations) {
  ...
  private _resolveAtLocation(
    ...
    // note: in nested `this.parseString`, parse only the new substring for any
    // replacements, don't reparse the whole string
  ...
  public toObject(): T {
    ...
    if (this.stringRoot) {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any -- unbox the string root
