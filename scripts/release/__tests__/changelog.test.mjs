...
test('parseCommit includes bang commits of user-facing types', () => {
...
test('parseCommit drops commits without bang regardless of type', () => {
  assert.equal(parseCommit('feat: add remote workspace'), null)
  assert.equal(parseCommit('fix: stop spinner overlap'), null)
  ...
  assert.equal(parseCommit('docs!: tweak readme')?.group, 'other')
})
...
test('buildGroups orders groups and aggregates items', () => {
  ...
  assert.deepEqual(groups, [
    ...
    { type: 'other', title: '其他变更', items: ['skip-not', 'r'] },
  ])
})
