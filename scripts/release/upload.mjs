...
// 更新日志 release-notes.json 不在 release/ 下，用源路径一并同步；
// 下载页 index.html 已改由 server:deploy 随服务器版本同步（首装由 setup 落地），不在此同步。
...
// 上传清单（{ label 仅用于日志, src 本地源路径 }），顺序即上传顺序：先 payload，最后清单类。
