...
| `serverEnv.mjs` | **两边** | 运行时配置的单一事实源：白名单、默认值派生、`server.env` 读写 |
| `setup.mjs` | 服务器 | 装服务：拷产物 / 写 `server.env` / 注册服务 / 自动生成机密 / 防火墙 / 启停卸载 |
...
`server.mjs` 启动自检命中即拒绝启动，别把这个检查删了。
...
`deploy` 的远端安装命令新增了 `cp ~/server.env.v* <appDir>/server.env` 这条通道，
**老部署的 sudoers 规则只覆盖 `server.js`，不更新会在安装步骤失败**（deploy 已带精确 hint）。
急着发版可先用 `--skip-env` 跳过配置上传。
